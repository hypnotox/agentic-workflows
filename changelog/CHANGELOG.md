...
- Em-dashes are removed from the shipped template prose across every skill,
  ...
  punctuation (colons, semicolons, commas, parentheses). The rendered wording is
  unchanged in meaning; only the punctuation reads less machine-set. Adopters get
  the reworded output on their next `awf sync`.
