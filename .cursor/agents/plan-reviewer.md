...
Independent, lens-diverse reviewer for plans under `docs/plans/`, dispatched in fresh context. Produces structured findings, classifies each as **mechanical / reasoned / user-decision**, applies fixes with a 3-round soft cap, and emits a digest.
...
1. **executability** — no placeholder language ("TBD", "similar to Task N", "implement later", "as needed"); file paths are absolute; verify steps have exact commands with expected output or observable change; each phase ends with a commit step, and each phase's closing commit passes the project's gate on its own — flag any definition whose first production use lands in a later phase; tasks are bite-sized (~2–5 min); code changes shown as exact diff not prose.
...
1. Apply mechanical and reasoned fixes directly; note rationale for reasoned fixes.
