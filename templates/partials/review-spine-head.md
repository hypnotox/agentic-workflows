...
When the brief carries pasted consensus entries (user-provenance decision-log entries, including whatever `Record:` blocks exist), check the {{ with .data.reviewSubject }}{{ . }}{{ else }}artifact{{ end }} against each one. A deviation from a user entry is always a `user-decision` finding, never silently absorbed: `location` cites the deviating {{ with .data.reviewSubject }}{{ . }}{{ else }}artifact{{ end }} passage, `issue` names the deviation, and `suggested_fix` carries the escalation phrasing "we decided X; during <phase> we found Z; recommend Y, approve?". A brief without consensus entries leaves this check idle.
