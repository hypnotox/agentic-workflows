...
func TestRunUpgradeAddsExploringAtSchemaThirteen(t *testing.T) {
	...
	if upgradedLock.SchemaVersion != 16 {
		t.Errorf("lock schema = %d, want 16", upgradedLock.SchemaVersion)
	}
...
func TestSyncReportsIndexOwnershipTakeover(t *testing.T) {
	...
	if err := (&manifest.Lock{AWFVersion: project.Version, SchemaVersion: 16, Files: map[string]manifest.Entry{}, ADRFormatV1From: 1, ADRFormatV2From: 1, LegacyADRGaps: []int{}}).Save(config.LockPath(root)); err != nil {
		t.Fatal(err)
	}
	testsupport.SwapVar(t, &getwd, func() (string, error) { return root, nil })
	var out, errb bytes.Buffer
