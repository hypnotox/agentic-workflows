...
func TestRunUpgradeExistingV1ToCurrentIsAtomicAndIdempotent(t *testing.T) {
	...
	if got.SchemaVersion != 18 || got.AWFVersion != "0.22.0" || got.ADRFormatV1From != 1 || got.ADRFormatV2From != 4 {
	...
	if !strings.Contains(out.String(), "already at schema 18") {
