...
func TestDocsAvailableWithoutProjectStateOrMutation(t *testing.T) {
	...
	pages := []struct {
		...
		{name: "knowledge", args: []string{"docs", "knowledge"}},
	...
	for _, page := range pages {
		...
		if !ok {
			t.Fatalf("missing embedded page %q", page.name)
		}
		...
		if code != 0 || stderr != "" || stdout != wantOutput {
			t.Errorf("%v = code %d, stdout %q, stderr %q, want %q", page.args, code, stdout, stderr, wantOutput)
		}
	}
...
func TestDocsHelpAndUsage(t *testing.T) {
	for _, args := range [][]string{{"docs", "--help"}, {"help", "docs"}} {
		...
		if code != 0 || stderr != "" || !strings.Contains(stdout, "awf docs [integration|knowledge|agents|topics|effort|changes|adr|completion]") {
...
func TestArtifactCreationCLI(t *testing.T) {
	root := t.TempDir()
	if code, _, stderr := runCLI(t, root, "init"); code != 0 {
		t.Fatal(stderr)
	}
	for _, kind := range []string{"intent", "spec"} {
	...
	code, stdout, stderr = runCLI(t, root, "check")
	if code != 0 || stdout != "check: ok\n" || stderr != "" {
		t.Fatalf("check with conforming starters = code %d, stdout %q, stderr %q", code, stdout, stderr)
	}
	...
	artifacts := map[string][]byte{
		...
		filepath.Join("docs", "plans", "existing.md"):             []byte("existing tracked plan\n"),
	...
	code, stdout, stderr = runCLI(t, root, "check")
	if code != 1 || !strings.HasPrefix(stdout, "check: failed\n") || stderr != "" {
		t.Fatalf("check with invalid authored metadata = code %d, stdout %q, stderr %q", code, stdout, stderr)
	}
	for relative, want := range artifacts {
		if !strings.Contains(stdout, filepath.ToSlash(relative)+": ") {
			t.Errorf("check omitted invalid concept %s: %s", relative, stdout)
		}
...
func TestEffortCLI(t *testing.T) {
	root := t.TempDir()
	...
	memoryBody := []byte{'o', 'p', 'a', 'q', 'u', 'e', 0, 0xff, '\n'}
	if err := os.WriteFile(filepath.Join(root, ".awf", "efforts", "simple", "memory.md"), memoryBody, 0o644); err != nil {
		t.Fatal(err)
	}
	...
	wantShow := "memory: .awf/efforts/simple/memory.md\n\n" + string(memoryBody)
	if code != 0 || stdout != wantShow || stderr != "" {
		t.Fatalf("effort show = code %d, stdout %q, stderr %q, want %q", code, stdout, stderr, wantShow)
	}
...
func TestTrackedDocumentCommandsRejectWrongArgumentCounts(t *testing.T) {
