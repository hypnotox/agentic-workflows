...
func TestLockVsBinaryCorruptLockErrors(t *testing.T) {
	...
	// config layout exists, gate(ctx, )'s GateState check errors first; without one
...
func TestGateCorruptLockWithoutConfigLayout(t *testing.T) {
	...
	// and never loads the lock, so gate(ctx, )'s version sub-check is the first
	...
	if err := gate(ctx, root); err == nil || !strings.Contains(err.Error(), "unreadable .awf/awf.lock") {
