...
func TestEffortRefusalLeaseCoversDiagnosticWrite(t *testing.T) {
	...
	result := handlers["effort"](&cmdCtx{ctx: testContext(t), root: root, sub: "new", inv: invocation{positionals: []string{"too long"}, bools: map[string]bool{"--no-worktree": true}, values: map[string]string{"--slug": strings.Repeat("s", 33)}}, stdout: &stdout})
