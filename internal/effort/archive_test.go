...
func TestFinishRefusesActiveArchiveCollisionWithoutMutation(t *testing.T) {
	...
	result, err := service.Finish(testContext(t), record.Slug)
	if err == nil || result != (FinishResult{}) || !strings.Contains(err.Error(), "archive destination already exists") {
...
func TestFinishReportsExpectedMarkerCompositionFailure(t *testing.T) {
	...
	if _, err := service.New(testContext(t), NewInput{Slug: "marker-error", Title: "Marker error"}); err != nil {
		t.Fatal(err)
	}
	if _, err := service.Finish(testContext(t), "marker-error"); err == nil || !strings.Contains(err.Error(), "render expected") {
