...
func TestEffortFocusedFailureBranches(t *testing.T) {
	...
	service, err := Open(testContext(t), root, Options{UUID: func() (string, error) { return "bad", nil }})
	if err != nil {
		t.Fatal(err)
	}
	...
	service, err = Open(testContext(t), root, Options{UUID: func() (string, error) { return "", errors.New("entropy") }})
	if err != nil {
		t.Fatal(err)
	}
