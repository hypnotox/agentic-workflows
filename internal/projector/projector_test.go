...
func TestResolveIsLexicalAndSorted(t *testing.T) {
	root := t.TempDir()
	writeTestFile(t, filepath.Join(root, ".awf", "project.md"), []byte(validProject("body\n")), 0o644)
	writeTopicForTest(t, root, "z-all.md", []string{"**"})
	writeTopicForTest(t, root, "a-go.md", []string{"src/**/*.go"})
	writeTopicForTest(t, root, "m-docs.md", []string{"docs/**"})

	matches, err := Resolve(root, []string{`src\new\future.go`, "unknown/file.txt"})
	if err != nil {
		t.Fatal(err)
	}
	want := []TopicMatch{
		{ID: "a-go", SourcePath: ".awf/topics/a-go.md"},
		{ID: "z-all", SourcePath: ".awf/topics/z-all.md"},
	}
	...
	writeTestFile(t, filepath.Join(root, ".awf", "topics", "z-all.md"), []byte("---\npaths: [elsewhere/**]\n---\nbody\n"), 0o644)
	matches, err = Resolve(root, []string{"no/match"})
