...
// invariant: adr-system/adr-lifecycle:renumber-digest-paired (TestRenumberPairsAcrossAnOccupiedNumber)
//
...
// invariant: adr-system/adr-lifecycle:renumber-digest-paired (TestRenumberPairsWhenTheOldNumberIsVacant)
//
...
// invariant: adr-system/adr-lifecycle:renumber-digest-paired (TestRenumberRequiresByteIdenticalHistory)
//
...
// invariant: adr-system/adr-lifecycle:renumber-digest-paired (TestRenumberRefusedWhenDigestAmbiguous)
//
...
// invariant: adr-system/adr-lifecycle:renumber-digest-paired (TestAmendedRecordStillPairsOnItsNumber)
//
...
// invariant: adr-system/adr-lifecycle:renumber-digest-paired (TestUnchangedSluglessRecordsPairOnTheirNumbers)
//
...
// slug is present on either side is outside slugless digest pairing, so an
// unrelated slug-carrying record with the same body cannot make the digest
// ambiguous.
...
// invariant: adr-system/adr-lifecycle:renumber-digest-paired (TestRenumberRefusesNewlySluggedAfterSide)
func TestRenumberRefusesNewlySluggedAfterSide(t *testing.T) {
	before := renumberBefore()
	after := renumberAfter()
	for i := range after.ADRs {
		if after.ADRs[i].Number == "0202" {
			after.ADRs[i].Slug = "newly-added-slug"
			after.ADRs[i].Format = adr.CurrentStateV3
		}
	}
	got := messages(currentstate.CheckPair(before, after, currentstate.AuthoredCommit))
	if !strings.Contains(got, "ADR-0202 adds claim d/t:x, which already existed") {
		t.Fatalf("a newly slugged after-side record must not form a digest pair:\n%s", got)
	}
}

// invariant: adr-system/adr-lifecycle:renumber-digest-paired (TestRenumberIgnoresAnUngovernedTwin)
//
