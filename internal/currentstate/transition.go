...
// the updating ADRs (ADR-0189), and change a canonical field that is neither
	...
	for _, id := range unionKeys(ops, beforeClaims, afterClaims) {
		...
		case hasOp && op.verb == adr.OpUpdate:
			if removedInUniverse(after.ADRs, id) {
				// An applied remove absorbed this claim (ADR-0189): the update
...
// ascending ADR number then intra-ADR history position (ADR-0189).
...
// so a net remove or net no-op is attributed to the removing ADR (ADR-0189).
...
// update chain as dominated history (ADR-0189).
...
// and the updating ADRs (ADR-0189).
...
// number is legal (ADR-0189 item 5). Ascending order over the result is owned by
...
// is derived (ascending ADR number, ADR-0189), so the migration's reordering of
