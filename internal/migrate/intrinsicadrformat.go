...
func applyIntrinsicADRFormat(ctx context.Context, root string, out io.Writer) error {
	return applyIntrinsicADRFormatWithSave(ctx, root, out, func(lock *manifest.Lock, path string) error {
		return lock.Save(path)
	})
}

func applyIntrinsicADRFormatWithSave(_ context.Context, root string, out io.Writer, save func(*manifest.Lock, string) error) error {
