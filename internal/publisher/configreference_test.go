...
func TestTemplateSourceRootCurrentValue(t *testing.T) {
	...
	if got := currentValueResolvers(newRenderInputs(p, cfg, nil, "test"))["render.templateSourceRoot"](); got != "`templates`" {
...
func TestConfigReferenceNoBareVars(t *testing.T) {
	...
	cref, ok, err := generateConfigReference(renderInputsForTest(p), files)
	if err != nil {
		t.Fatal(err)
	}
	if !ok {
