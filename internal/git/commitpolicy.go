	...
	for _, target := range targets {
		...
		if !isRangeTarget(target) {
			...
			if peelErr != nil {
				...
				if errors.As(peelErr, &policyErr) && policyErr.Kind == CommitPolicyTagPeelError {
					return nil, peelErr
				}
...
func (r *Repo) VerifySSH(ctx context.Context, id string, signers []commitpolicy.Signer) (verdict commitpolicy.SignatureVerdict, retErr error) {
	...
	create := r.createTemp
	if create == nil {
		create = func(dir, pattern string) (trustFile, error) { return os.CreateTemp(dir, pattern) }
	}
	...
	defer func() {
		...
		if cleanupErr := remove(name); cleanupErr != nil {
			if retErr != nil {
				cleanupErr = errors.Join(retErr, cleanupErr)
			}
			verdict = commitpolicy.SignatureMissing
			retErr = policyGitError(CommitPolicyTrustError, name, cleanupErr)
		}
	}()
	...
	for _, signer := range signers {
		if _, err := fmt.Fprintln(f, signer.Principal, signer.Key); err != nil {
			_ = f.Close()
			return commitpolicy.SignatureMissing, policyGitError(CommitPolicyTrustError, name, err)
		}
	}
...
func (r *Repo) PeelCommit(ctx context.Context, rev string) (string, string, error) {
	_, outerType, err := r.FullOID(ctx, rev)
	if err != nil {
		return "", "", err
	}
	peeled, err := r.runner.run(ctx, "rev-parse", "--verify", rev+"^{}")
	if err != nil {
		peelErr := fmt.Errorf("peel %q: %w", rev, err)
		if outerType == "tag" {
			return "", "", policyGitError(CommitPolicyTagPeelError, rev, peelErr)
		}
		return "", "", peelErr
	}
