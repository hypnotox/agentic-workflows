...
`,
	...
	if !strings.HasPrefix(got, "<!-- GENERATED by internal/adrtools") {
...
// TestGenerateActiveMD is the staleness-gate test: computes expected ACTIVE.md
// from ../../docs/decisions and compares to on-disk ACTIVE.md.
// If stale: writes the regenerated file and fails.
// If identical: passes.
func TestGenerateActiveMD(t *testing.T) {
	decisionsDir := filepath.Join("..", "..", "docs", "decisions")
	activeMDPath := filepath.Join(decisionsDir, "ACTIVE.md")

	generated, err := adrtools.GenerateActiveMD(decisionsDir)
	if err != nil {
		t.Fatalf("GenerateActiveMD: %v", err)
	}

	onDisk, err := os.ReadFile(activeMDPath)
	...
	// Stale (or missing): write it and fail so the caller re-stages.
	if err := os.WriteFile(activeMDPath, []byte(generated), 0644); err != nil {
