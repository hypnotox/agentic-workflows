...
func parseStateChanges(body string) (ops []Operation, none bool, err error) {
	...
	for _, line := range lines {
		...
		id := m[2]
		if seen[id] {
			return nil, false, fmt.Errorf("state changes names %s more than once; amend an unapplied operation, use Reapplied to correct an applied add or update while Implementing, or use a follow-up ADR", id)
		}
		seen[id] = true
