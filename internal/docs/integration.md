...
5. Reconcile `docs/` with `awf docs knowledge`, using the manual metadata upgrade below for existing documents. Run `render` and `check`, review the full diff, and run the repository's tests or gate. Commit the author-owned guidance, topics, and generated outputs together.
...
The older v0.50 layout kept authored agent guidance in `.awf/agents-doc.yaml` and `.awf/parts/agents-doc/` overrides, and split topic selectors and prose under `.awf/topics/metadata/` and `.awf/topics/parts/`. Follow `MIGRATING-v0.50.md` in the release archive or the [repository copy](https://github.com/hypnotox/agentic-workflows/blob/main/MIGRATING-v0.50.md) to combine them and preserve other authored overrides. Do not move that split layout as though each file were a complete topic. Inspect legacy hooks and remove obsolete AWF calls while retaining unrelated behavior. Change or unset hook configuration only through an explicit repository decision.

`init`, `render`, `check`, and topic resolution report an actionable migration error while `.awf/project.md`, `.awf/agents-doc.yaml`, `.awf/parts/agents-doc/`, or `.awf/topics/` remains; they never silently ignore that guidance. Documentation and artifact/effort commands remain available. There is no old-location fallback or format parser.
...
1. Inventory existing Markdown under `docs/`, including files not created by AWF. Add a descriptive string `type` and a meaningful string `description` to every concept. Add `title` where useful, but it is optional. Preserve existing frontmatter fields, bodies, links, and repository-relative topic selectors. Starter types are documented in `awf docs knowledge`; custom types remain valid.
2. For each ADR concept in `docs/decisions/`, move legacy `status: pending|accepted|active` to `decision_status`. Add the corresponding OKF status using the mapping in `awf docs adr`: pending and accepted map to draft, active maps to stable. Renaming a field does not establish new agreement or activation. Keep active predecessors governing until existing supersession and retirement conditions are met.
3. If both status fields already exist, or metadata conflicts with the record's documented authority, stop and reconcile that record explicitly using established agreement and implementation evidence. Do not guess, silently overwrite a value, or treat a previously absent state as approval.
4. Inspect `index.md` and `log.md` at every depth. They are reserved files, not concepts or topics. If a legacy concept uses one of those names, deliberately rename it to a non-reserved basename and repair references; preserve its body and selectors. Do not simply drop its metadata and lose the concept's role. Reconcile genuine indexes and logs with their structural conventions; neither is required.
5. Run the target binary's `render` and `check`, inspect repository-relative diagnostics and the diff, and run the repository's tests. Render refreshes only AWF-owned outputs; repair authored metadata manually. Commit reconciled documents and generated outputs together.

Root README and agent instructions, generated skills, internal guides outside `docs/`, and local effort memory are not part of this migration. Files under `docs/` remain author-owned; conformance does not attest their truth, completeness, or approval.

## Connect checks to automation

Add `./awf check` to the repository's existing gate and CI after checkout. It validates the `docs/` knowledge contract (including ADR state pairs), topic selectors, generated bytes, executable state, and retired ownership markers in the working tree. It does not validate an isolated staged snapshot or prove overall integration. Keep source edits, rendering, review, staging, and commits explicit.
