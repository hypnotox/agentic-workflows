...
Project knowledge lives in `docs/`: topics in `docs/topics/`, change documents in `docs/changes/<slug>/`, and ADRs in `docs/decisions/`. Each document is author-owned, with no generated copy. `.awf/` retains project-level agent guidance, local effort memory, and launch infrastructure.
...
awf docs effort       use effort memory, change documents, ADRs, and manual worktree conventions
```
...
Use `awf new` to create effort, intent, spec, plan, ADR, or topic starters. Intent, specification, and plan share a change folder, but each is optional and independent of effort memory. ADRs remain organized by decision area; memory references these tracked documents. See `awf docs effort` for their distinct lifecycles and `awf docs topics` for topic creation and informational coverage inspection.
