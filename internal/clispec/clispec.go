...
var Commands = []Command{
...

	{
		...
		MaxPos: 0, Gating: Gated,
		Help: Help{Usage: []string{"awf read <subcommand>"}, Description: "Read a bounded executable projection from a parsed project artifact.", Positionals: []HelpItem{{Name: "<subcommand>", Description: "operation selected from this command group"}}},
		Children: []Command{
	...
	},
	{
		...
		MaxPos: 0, Gating: Gated,
		Help: Help{Usage: []string{"awf effort <subcommand>"}, Description: "Create, inspect, finish, integrate, and remove immutable slugged efforts, which get a managed worktree by default.", Positionals: []HelpItem{{Name: "<subcommand>", Description: "operation selected from this command group"}}},
		Children: []Command{
			...
			{Name: "show", Summary: "Show one effort", BoolFlags: []string{"--json"}, MinPos: 1, MaxPos: 1,
				Help: Help{Usage: []string{"awf effort show <slug> [--json]"}, Description: "Show one schema-2 effort and its owned memory path.", Positionals: []HelpItem{{Name: "<slug>", Description: "immutable effort slug"}}}},
			...
			{Name: "worktree", Summary: "Add or remove a managed worktree", ValueFlags: []string{"--base"}, MinPos: 2, MaxPos: 2,
				Help: Help{Usage: []string{"awf effort worktree add <slug> [--base <ref>]", "awf effort worktree remove <slug>"}, Description: "Manage the fixed .awf/worktrees/<slug> checkout and awf/<slug> branch without stored attachment state.", Positionals: []HelpItem{{Name: "<slug>", Description: "immutable effort slug"}, {Name: "<ref>", Description: "Git revision used as the worktree base"}}}},
			...
			{Name: "memory", Summary: "Update bounded effort memory metadata", MaxPos: 0,
				Help: Help{Usage: []string{"awf effort memory update <slug> [--phase <text>] [--next <text>]"}, Description: "Update one or both mutable memory metadata fields. At least one of --phase and", Details: []string{"--next is required."}, Positionals: []HelpItem{{Name: "<slug>", Description: "immutable effort slug"}, {Name: "<text>", Description: "replacement effort metadata text"}}},
				Children: []Command{
	...
	},
	{
		...
		Help: Help{Usage: []string{"awf adr <subcommand>"}, Description: "Perform an ADR lifecycle operation that the corpus, not the author, owns.", Positionals: []HelpItem{{Name: "<subcommand>", Description: "operation selected from this command group"}}},
		Children: []Command{
			{Name: "number", Summary: "Number pending ADRs at integration", MinPos: 0, MaxPos: -1,
				Help: Help{Usage: []string{"awf adr number [<slug>...]"}, Description: "Number pending ADRs after merging the integration branch in and before merging", Details: []string{"back. Bare invocation numbers a single pending ADR; several pending ADRs require", "an explicit list naming every pending slug, in the intended add-before-revise", "order."}, Positionals: []HelpItem{{Name: "<slug>", Description: "immutable effort slug"}}}},
		},
	},
	{
	...
	},
	{
		...
		StateExempt: true,
		Help:        Help{Usage: []string{"awf changelog [--version <v> | --since <v> | --range <from>..<to>]"}, Description: "Print the embedded awf changelog. With no flags, print the whole file. The three", Details: []string{"flags are mutually exclusive."}, Positionals: []HelpItem{{Name: "<v>", Description: "release version"}, {Name: "<from>", Description: "inclusive lower release version"}, {Name: "<to>", Description: "inclusive upper release version"}}, Options: []HelpItem{{Name: "--version", Description: "<v>          print only version v's entry"}, {Name: "--since", Description: "<v>            print every version released after v (exclusive)"}, {Name: "--range", Description: "<from>..<to>   print every version in [from, to] (inclusive both ends)"}}},
	},
	{
