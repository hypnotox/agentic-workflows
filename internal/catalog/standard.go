...
var Standard = &Catalog{
	Skills: map[string]SkillSpec{
		...
		"reviewing-plan": {Profile: WorkflowProfile{Kind: WorkflowChain, Purpose: "Independently review an implementation plan.", Trigger: "Use when a written plan needs review before execution.", UsuallyFollows: []string{"writing-plans"}, CommonFollowUps: []string{"reviewing-plan-resync", "executing-plans"}}, RequiresAgent: "plan-reviewer", Sections: []string{
		...
		"reviewing-plan-resync": {Profile: WorkflowProfile{Kind: WorkflowChain, Purpose: "Reconcile a plan after review findings.", Trigger: "Use when review findings require a plan revision and re-review.", UsuallyFollows: []string{"reviewing-plan", "reviewing-adr"}, CommonFollowUps: []string{"executing-plans", "subagent-driven-development"}}, RequiresAgent: "plan-reviewer", Sections: []string{
			"when-fires", "dispatch-subagent-narrowed", "classify-route-findings",
			"apply-fixes-commit", "re-review-loop", "hand-off-to-impl", "notes",
		}},
		"reviewing-adr": {Profile: WorkflowProfile{Kind: WorkflowChain, Purpose: "Independently review an ADR.", Trigger: "Use when a proposed ADR needs decision-quality review.", UsuallyFollows: []string{"proposing-adr"}, CommonFollowUps: []string{"reviewing-plan-resync", "writing-plans"}}, RequiresAgent: "adr-reviewer", Sections: []string{
			"when-fires", "procedure", "artifact-path-detection", "dispatch-subagent",
			"classify-route-findings", "apply-fixes-commit", "re-review-loop", "status-flip",
			"hand-off-to-resync", "notes",
		}},
		"reviewing-impl": {Profile: WorkflowProfile{Kind: WorkflowChain, Purpose: "Independently assure an implementation.", Trigger: "Use when independent review has assurance value for the implementation.", UsuallyFollows: []string{"executing-direct", "executing-plans", "subagent-driven-development"}, CommonFollowUps: []string{"effort-workflow"}}, RequiresAgent: "code-reviewer", Sections: []string{
	...
	Vars: []VarDescriptor{
		...
		{Key: "checkCmd", Kind: "string", Description: "Command that checks rendered output for drift. Leave empty to run through the always-rendered `./awf` wrapper.", Default: "", Options: []string{"./awf check", "./x check"}},
