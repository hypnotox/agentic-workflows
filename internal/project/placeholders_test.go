...
func TestPlaceholderRegistry(t *testing.T) {
	reg, _ := placeholderRegistry(renderInputsForTest(projectWithScopes(t)))
	...
	bare, _ := placeholderRegistry(renderInputsForTest(projectAcceptAny(t)))
	if bare["prefix"] != "bare" {
...
func TestSectionDefaultPlaceholder(t *testing.T) {
	...
	reg, err := placeholderRegistry(renderInputsForTest(p))
	if err != nil {
	...
	out, err := substitutePlaceholders("x", "A {{=awf:sectionDefault}} B", reg)
	if err != nil {
...
func TestCommitScopeTableAndSentence(t *testing.T) {
	...
	table := commitScopeTable(renderInputsForTest(p))
	if !strings.Contains(table, "| `adr` | ADR docs |") || !strings.Contains(table, "| `rendering` | the render engine |") {
		t.Errorf("table missing rows:\n%s", table)
	}
	if s := commitScopeSentence(renderInputsForTest(p)); !strings.Contains(s, "`adr`, `rendering`") {
		t.Errorf("sentence = %q", s)
	}

	empty := projectAcceptAny(t)
	if commitScopeTable(renderInputsForTest(empty)) != "" {
		t.Error("accept-any commitScopeTable must be empty")
	}
	if commitScopeSentence(renderInputsForTest(empty)) != "" {
...
func TestSubstitutePlaceholders(t *testing.T) {
	p := projectWithScopes(t)
	reg, _ := placeholderRegistry(renderInputsForTest(p))

	// Fast path: no placeholder token.
	if out, err := substitutePlaceholders("x", "plain prose, no tokens", reg); err != nil || out != "plain prose, no tokens" {
		t.Errorf("fast path: out=%q err=%v", out, err)
	}

	// Known keys substitute.
	out, err := substitutePlaceholders("x", "Scopes: {{=awf:commitScopeList}}\n{{=awf:commitScopeTable}}", reg)
	if err != nil {
	...
	// Error cases.
	// invariant: rendering/inplace-and-placeholders:part-placeholder-sandboxed (TestSubstitutePlaceholders)
	for _, tc := range []struct{ name, body, want string }{
		...
		t.Run(tc.name, func(t *testing.T) {
			_, err := substitutePlaceholders("part", tc.body, reg)
	...
	bareReg, _ := placeholderRegistry(renderInputsForTest(projectAcceptAny(t)))
	if _, err := substitutePlaceholders("part", "{{=awf:commitScopeTable}}", bareReg); err == nil {
		t.Error("empty-value key: want error, got nil")
	}
}

// invariant: rendering/inplace-and-placeholders:escaped-placeholder-literal (TestPlaceholderEscape)
func TestPlaceholderEscape(t *testing.T) {
	...
	out, err := substitutePlaceholders("x", `\{{=awf:commitScopeTable}}`, reg)
	if err != nil {
	...
	out, err = substitutePlaceholders("x", `\{{= awf:x}}`, reg)
	...
	out, err = substitutePlaceholders("x", `\\{{=awf:commitScopeList}}`, reg)
	if err != nil {
...
func TestPlaceholderValueTokenFree(t *testing.T) {
	...
	if _, err := placeholderRegistry(renderInputsForTest(p)); err == nil || !strings.Contains(err.Error(), "commitScopeTable") {
