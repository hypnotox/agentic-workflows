...
func TestSchemaMinimumVersionAuthority(t *testing.T) {
	...
	if got := minVersionBySchema[24]; got != Version {
		t.Fatalf("generation-24 minimum version = %q, want %s", got, Version)
	}
	...
	if err := ValidateSchemaMinimumVersion(25, Version); err == nil || !strings.Contains(err.Error(), "no minimum") {
