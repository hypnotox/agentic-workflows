...
func TestSchemaMinimumVersionAuthority(t *testing.T) {
	...
	if got := minVersionBySchema[23]; got != Version {
		t.Fatalf("generation-23 minimum version = %q, want %s", got, Version)
	}
	...
	if err := ValidateSchemaMinimumVersion(24, Version); err == nil || !strings.Contains(err.Error(), "no minimum") {
