...
func TestPhaseTransactionOwnershipAcrossWorkflowSurfaces(t *testing.T) {
	...
	assertContract := func(variant string, surfaces map[string]string, gatePhrase string) {
		...
		assertAll("inline",
			...
			"scope, canonical checkout, start and end Git and worktree state", "changed paths", "exact focused commands and results",
			"completed and remaining work", "routed blockers", "generated-output or fixture evidence",
			"checkout identity, relevant authority, and overlapping paths remain unchanged", "later overlapping mutation invalidates the affected receipt",
		...
		assertAll("subagent",
			...
			"assigned scope, canonical checkout", "starting and ending HEAD", "changed paths", "exact focused command with cwd, argv, exit status, and actual result",
			"completed and remaining work", "separately routed blockers", "generated-output or fixture evidence",
			"parent independently inventories the checkout", "relevant authority, and overlapping paths remain unchanged", "later overlapping mutation invalidates the affected receipt",
