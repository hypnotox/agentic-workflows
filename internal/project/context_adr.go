...
func projectADRArtifact(filePath, decisionsDir string, adrs adr.Corpus, topics topic.Corpus, facets []ContextFacet) *ADRArtifactContext {
	...
	states := map[adr.Operation]state{}
	if record.IsProposed() {
		for _, op := range record.Operations {
			states[op] = state{name: "proposed"}
		}
	} else {
		for _, op := range progress.Remaining {
			states[op] = state{name: "remaining"}
		}
		for _, op := range progress.Canceled {
			states[op] = state{name: "canceled"}
		}
		for _, op := range progress.Applied {
			states[op.Operation] = state{name: "applied", sequence: op.Sequence}
		}
	}
	for _, op := range record.Operations {
		s := states[op]
		...
		entry := ADROperationContext{Operation: string(op.Verb), Claim: op.ID, Topic: topicOfClaim(op.ID), Progress: s.name, StateSequence: s.sequence, ClaimState: claimStateForOperation(string(op.Verb), op.ID, s.name, topics, history)}
