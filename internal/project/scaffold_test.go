...
func TestPitfallScaffoldCLIContract(t *testing.T) {
	...
	t.Run("load-and-directory-errors", func(t *testing.T) {
		...
		t.Run("nested source and injected read or walk failures", func(t *testing.T) {
			...
			if _, err := newPitfallWith("New", tree); err == nil || !strings.Contains(err.Error(), "direct child") {
			...
			if _, err := newPitfallWith("New", &faultPitfallFilesystem{pitfallScaffoldFilesystem: tree, readErr: readErr}); !errors.Is(err, readErr) {
				t.Fatalf("read error = %v", err)
			}
			walkErr := errors.New("walk failed")
			if _, err := newPitfallWith("New", &faultPitfallFilesystem{pitfallScaffoldFilesystem: tree, walkErr: walkErr}); !errors.Is(err, walkErr) {
		...
		t.Run("mkdir failure", func(t *testing.T) {
			...
			if _, err := newPitfallWith("New", faults); !errors.Is(err, mkdirErr) {
...
func TestNewPitfallPublicationFailureLeavesDestinationAbsent(t *testing.T) {
	...
	if _, err := newPitfallWith("Unpublished", faults); !errors.Is(err, publishErr) {
...
func TestNewPitfallCommittedCleanupOutcomeIsActionableAndDoesNotAdvance(t *testing.T) {
	...
	_, err = newPitfallWith("Committed", faults)
	...
	if _, retryErr := newPitfallWith("Committed", tree); retryErr == nil || !strings.Contains(retryErr.Error(), residuePath) {
		t.Fatalf("ordinary retry did not report cleanup residue: %v", retryErr)
	}
	if err := tree.Remove(residuePath); err != nil {
		t.Fatal(err)
	}
	if _, retryErr := newPitfallWith("Committed", tree); retryErr == nil || !strings.Contains(retryErr.Error(), "duplicates") {
...
func TestNewPitfallExclusiveRaceRefusesThenRetryReallocates(t *testing.T) {
	...
	if _, err := newPitfallWith("Race", faults); !errors.Is(err, os.ErrExist) {
	...
	if _, err := newPitfallWith("Race", faults); err != nil {
