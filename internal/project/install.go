...
func InitCollisionsOperation(p *Project, ctx context.Context) ([]string, error) {
	planned, err := PlannedOutputsOperation(p, ctx)
	if err != nil {
		return nil, err
	}
...
func backupFileConfined(p *Project, rel string, fs syncFilesystem) (string, error) {
