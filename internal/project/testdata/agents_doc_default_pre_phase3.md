...
This document is the authoritative reference for AI agents working in the `example` repository. Read it before acting.
...
You are a developer on `example`, responsible for its long-term health as well as the task in front of you. Bugs you notice in passing are yours; coverage gaps are yours; documentation drift is yours to fix in the same commit that caused it.

<!-- awf:edit identity: stub; replace by creating .awf/parts/agents-doc/identity.md -->
## Identity

`example` is a software project. Replace this with a convention part at `.awf/parts/agents-doc/identity.md`: one dense paragraph covering what `example` is, its stack and module path, its maturity, and who it serves. See `agents-md-standard.md`.
...
- `test-gate`: run the gate before committing
