...
func TestOutputPolicyRoutesMisleadingPathsEndToEnd(t *testing.T) {
	...
	if drift := p.checkLockedFiles(lock, map[string]RenderedFile{frontmatter.Path: frontmatter}, nil); len(drift) != 1 || drift[0].Kind != "invalid-frontmatter" {
	...
	if drift := p.checkLockedFiles(lock, map[string]RenderedFile{regen.Path: regen}, nil); len(drift) != 1 || drift[0].Kind != "stale" {
...
func TestCheckLockedFilesClassifiesOrdinaryFreshnessBeforeObservation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := p.checkLockedFiles(lock, map[string]RenderedFile{path: tt.file}, nil)
