...
func TestRenderAllAgentAssembleErrorOnMissingPart(t *testing.T) {
	root := scaffoldFiles(t, "prefix: example\nskills: []\nagents: [code-reviewer]\nhooks: []\n", map[string]string{
		"agents/code-reviewer.yaml": "sections:\n  doc-currency:\n    replaceWith: no-such-part.md\n",
	})
	p, err := Open(root)
	if err != nil {
		t.Fatal(err)
	...
	if _, err := p.RenderAll(); err == nil {
		t.Fatal("expected RenderAll to fail assembling an agent with a missing replaceWith part")
	}
}

func TestRenderAllDocAssembleErrorOnMissingPart(t *testing.T) {
	root := scaffoldFiles(t, "prefix: example\nskills: []\nagents: []\nhooks: []\ndocs: [architecture]\n", map[string]string{
		"docs/architecture.yaml": "sections:\n  overview:\n    replaceWith: no-such-part.md\n",
	})
	p, err := Open(root)
	if err != nil {
		t.Fatal(err)
	...
	if _, err := p.RenderAll(); err == nil {
		t.Fatal("expected RenderAll to fail assembling a doc with a missing replaceWith part")
	}
}

func TestRenderAllAgentsDocAssembleErrorOnMissingPart(t *testing.T) {
	root := scaffoldFiles(t, "prefix: example\nskills: []\nagents: []\nhooks: []\n", map[string]string{
		"agents-doc.yaml": "sections:\n  identity:\n    replaceWith: no-such-part.md\n",
	})
	p, err := Open(root)
	...
	if _, err := p.RenderAll(); err == nil {
		t.Fatal("expected RenderAll to fail assembling agents-doc with a missing replaceWith part")
	}
}
