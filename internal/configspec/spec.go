...
var keys = []Entry{
	...
	},
	{
		...
		Description:  "The branch effort work integrates into. A generated pre-push hook uses the freshly resolved destination tip of this branch as the policy base for new commit-bearing refs, and changing it invalidates that consumer without reflagging unrelated outputs. Must be non-empty and free of whitespace, must not start with `-`; slashes are legal, so `release/1.0` is accepted. There is no in-code default, and audit range resolution never reads it.",
		Availability: "Always.",
	},
	{
