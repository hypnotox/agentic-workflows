...
func TestOperationsFromMutations(t *testing.T) {
	root := t.TempDir()
	mutations := []Mutation{
		{Path: ".awf/config.yaml", BeforePresent: true, BeforeMode: 0o644, Before: []byte("old"), AfterPresent: true, AfterMode: 0o644, After: []byte("newcfg")},
		{Path: "docs/decisions/ACTIVE.md", BeforePresent: true, BeforeMode: 0o644, Before: []byte("idx"), AfterPresent: false},
	}
	ops, err := OperationsFromMutations(root, mutations, presentImg("oldlock"), presentImg("newlock"))
	if err != nil {
		t.Fatal(err)
	}
	if len(ops) != 3 || ops[len(ops)-1].Path != LockRel() {
		t.Fatalf("ops: %#v", ops)
	}
	if ops[0].Path != ".awf/config.yaml" || ops[1].Replacement.Present {
		t.Fatalf("deletion op not absent: %#v", ops[1])
	}
}

func TestJournalLoadRejections(t *testing.T) {
	// Raw malformed JSON, and DisallowUnknownFields rejection.
	root := t.TempDir()
	mustMkdir(t, filepath.Join(root, ".awf"))
	mustWrite(t, JournalPath(root), []byte("{not json"), 0o644)
	if _, err := LoadJournal(root); err == nil || !strings.Contains(err.Error(), "malformed") {
		t.Fatalf("malformed json: %v", err)
	}
	mustWrite(t, JournalPath(root), []byte(`{"version":1,"phase":"prepared","operations":[],"extra":1}`), 0o644)
	if _, err := LoadJournal(root); err == nil || !strings.Contains(err.Error(), "malformed") {
		t.Fatalf("unknown field: %v", err)
	}
	// Structured contract violations.
	base := lockJournal(phasePrepared)
	...
	// A missing journal file surfaces the read error.
...
func TestJournalCommitHappyPath(t *testing.T) {
	...
	for _, want := range []string{"operation: applied a.txt", "operation: applied .awf/awf.lock", "operation: attestation committed"} {
...
func TestJournalCommitRollsBackOnApplyFailure(t *testing.T) {
	...
	root := t.TempDir()
	mustMkdir(t, filepath.Join(root, ".awf"))
	// A pre-existing a.txt (prior) is restored; the ro/ directory is read-only so
	// writing ro/new.txt fails, triggering rollback.
	mustWrite(t, filepath.Join(root, "a.txt"), []byte("original"), 0o644)
...
func TestJournalRecoverTable(t *testing.T) {
	// Precommit with lock != final: roll back to priors, clear the journal.
	t.Run("precommit-rolls-back", func(t *testing.T) {
		root := t.TempDir()
		writeRawJournal(t, root, lockJournal(phaseApplying))
		mustWrite(t, filepath.Join(root, "a.txt"), []byte("new"), 0o644) // == replacement
		mustWrite(t, filepath.Join(root, LockRel()), []byte("old-lock"), 0o644)
		var log bytes.Buffer
		...
		// Idempotent: a second recovery with no journal simply errors (no journal).
		if err := Recover(root, io.Discard); err == nil {
			t.Fatal("second recovery with no journal accepted")
		}
	})
	// Precommit whose lock already carries the final hash: treat as committed.
	t.Run("precommit-lock-already-final", func(t *testing.T) {
		root := t.TempDir()
		writeRawJournal(t, root, lockJournal(phasePrepared))
		mustWrite(t, filepath.Join(root, LockRel()), []byte("FINAL"), 0o644)
		mustWrite(t, filepath.Join(root, "a.txt"), []byte("new"), 0o644)
		if err := Recover(root, io.Discard); err != nil {
			t.Fatalf("recover: %v", err)
		}
		if JournalPresent(root) {
	...
	// lock-committed with the final hash: clean up only.
	...
	// lock-committed with a different hash: refuse rather than roll authority back.
	t.Run("lock-committed-mismatch-refuses", func(t *testing.T) {
		root := t.TempDir()
		writeRawJournal(t, root, lockJournal(phaseLockCommitted))
		mustWrite(t, filepath.Join(root, LockRel()), []byte("DIFFERENT"), 0o644)
	...
	// A third-party edit halts rollback and preserves the journal, naming the path.
	t.Run("third-party-preserves-journal", func(t *testing.T) {
		root := t.TempDir()
		writeRawJournal(t, root, lockJournal(phaseApplying))
		mustWrite(t, filepath.Join(root, "a.txt"), []byte("TAMPERED"), 0o644)
		mustWrite(t, filepath.Join(root, LockRel()), []byte("old-lock"), 0o644)
...
func TestJournalHelpers(t *testing.T) {
	...
	// A directory is neither absent nor a readable regular file.
	mustMkdir(t, filepath.Join(root, "adir"))
	if _, err := imageOf(root, "adir"); err == nil {
		t.Fatal("directory imaged as a file")
	}
	// Removing a non-empty directory to reach an absent image fails.
	mustWrite(t, filepath.Join(root, "adir", "child"), []byte("x"), 0o644)
	...
	mustWrite(t, JournalPath(empty), []byte(`{"version":1,"phase":"prepared","finalLockSHA256":"","operations":[]}`), 0o644)
...
func TestJournalCommitLockFailureHaltsRollback(t *testing.T) {
	root := t.TempDir()
	mustMkdir(t, filepath.Join(root, ".awf"))
	// The lock path is a pre-existing directory, so writing the lock fails and its
	// prior image cannot be read during rollback: the journal is preserved.
