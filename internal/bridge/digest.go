...
// CutoffFacts returns the ADR-format-v1 cutoff (the highest ADR number plus one)
// and the sorted absent lower ADR numbers for the corpus under root. Migration
// normalization never changes an ADR number, so these facts are identical over
// the legacy HEAD and the prepared tree.
func CutoffFacts(root string) (int, []int, error) {
	...
	cutoff, gaps := cutoffFacts(adrIDs(corpus))
	return cutoff, gaps, nil
}

// Digest computes the stable current-state attestation tree digest over the
// post-normalization prepared inputs. The universe is exactly the config file,
// every domain sidecar, every topic input, every ADR, every file matched by the
// prepared config's marker source globs, and the required migration approval
// file; no other path enters it. For each universe path the image is the planned
// mutation's after-image when a normalization mutation targets it, otherwise the
// current on-disk bytes and permission mode. Terminal output writes and legacy
// deletions never touch a universe path, so the merged mutation set yields the
// post-normalization image for every member. Digest is byte-for-byte read-only.
func Digest(root string, mutations []Mutation) (string, error) {
	...
	preparedCfg, err := config.Parse("", converted)
	if err != nil { // coverage-ignore: readiness validated the converted config before attestation reaches the digest
		return "", err
	}
	after := map[string]Mutation{}
	...
	if preparedCfg.CurrentState != nil {
		if err := collectMarkerSources(root, preparedCfg.CurrentState.Sources, universe); err != nil { // coverage-ignore: only a permission or concurrent-removal fault under the scanned tree reaches here
			return "", err
		}
	}
	var records []digestRecord
	for path := range universe {
		if m, ok := after[path]; ok {
			if !m.AfterPresent {
				continue
			}
			records = append(records, digestRecord{path: path, mode: m.AfterMode, content: m.After})
			continue
		}
...
// universe, so generated indexes such as ACTIVE.md are excluded.
...
// collectMarkerSources adds every regular file matched by any prepared marker
// source glob to the universe, skipping nested projects and dependency trees the
// same way the marker rewrite scan does.
