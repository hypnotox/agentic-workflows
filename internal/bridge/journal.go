...
// OperationsFromMutations converts sorted mutations plus the sealed lock image
// into journal operations with the lock operation last. It reads each path's
// current image as the prior so recovery can restore it exactly.
func OperationsFromMutations(root string, mutations []Mutation, lockPrior, lockFinal Image) ([]Operation, error) {
	var ops []Operation
	for _, m := range mutations {
		if m.Path == LockRel() { // coverage-ignore: readiness mutations never target the lock; the lock op is supplied separately
			continue
		}
		prior := Image{Present: m.BeforePresent, Mode: m.BeforeMode, Content: m.Before}
		replacement := Image{Present: m.AfterPresent, Mode: m.AfterMode, Content: m.After}
		if !m.AfterPresent {
			replacement = Image{Present: false}
		}
		ops = append(ops, Operation{Path: m.Path, Prior: prior, Replacement: replacement})
	}
	ops = append(ops, Operation{Path: LockRel(), Prior: lockPrior, Replacement: lockFinal})
	if err := validateOperations(ops); err != nil { // coverage-ignore: readiness mutations are validated, unique, and safe; the lock op closes the set
		return nil, err
	}
	return ops, nil
}
...
// images and clears the journal; a failure after leaves the attested lock plus a
...
func CommitTransaction(root string, ops []Operation, log io.Writer) error {
	if err := validateOperations(ops); err != nil { // coverage-ignore: OperationsFromMutations validated the same set before this call
		return err
	}
	...
	fmt.Fprintln(log, "operation: attestation committed")
	return nil
}
