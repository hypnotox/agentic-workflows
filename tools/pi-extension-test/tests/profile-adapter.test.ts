...
import extension, { createLimiter, registerSubagentTools, type ExtensionDependencies } from "../../../.pi/extensions/awf-subagents/index.ts";
...
function checkoutGit(before="a",after="b",common=COMMON){let heads=0;return(args:string[],cwd:string)=>{if(args.includes("--show-toplevel"))return{code:0,stdout:`${cwd}\n`};if(args.includes("--git-common-dir"))return{code:0,stdout:`${cwd===ROOT?COMMON:common}\n`};if(args.includes("--absolute-git-dir"))return{code:0,stdout:`${cwd===ROOT?COMMON:ADMIN}\n`};if(args.includes("HEAD"))return{code:0,stdout:`${heads++?after:before}\n`};if(args[0]==="status")return{code:0,stdout:" M x\n"};return{code:1,stdout:""};};}
 ...
 test("limiter releases once, queues, and aborts queued work",async()=>{const l=createLimiter(1),r1=await l.acquire(undefined);let acquired=false;const p=l.acquire(undefined).then(r=>{acquired=true;return r;});await flush();assert.equal(acquired,false);r1();r1();const r2=await p;r2();const c=new AbortController();c.abort();await assert.rejects(l.acquire(c.signal),/aborted/);const hold=await l.acquire(undefined), normal=new AbortController(), normalWait=l.acquire(normal.signal);hold();(await normalWait)();const hold2=await l.acquire(undefined), queued=new AbortController(), rejected=l.acquire(queued.signal);queued.abort();await assert.rejects(rejected,/aborted/);hold2();});
...
test("negotiation correlations, receipts, results, deferred and once-only reporting",async()=>{
 ...
 for(const result of [
  {registrationId:"other",protocolVersion:2,state:"rejected"},{registrationId:"awf:subagent-profiles:v2",correlationId:"other",protocolVersion:2,state:"rejected"},
 ...
 ]){const x=harness();x.events.get("pi-tools:subagent-profiles:registration-result")(result);await x.start();await flush();if(result.registrationId==="awf:subagent-profiles:v2"&&!result.correlationId&&result.protocolVersion===2&&result.state==="registered")assert.equal(x.notices.length,0);}
...
test("live session, preference notices, routing hook precedence, fallback, overflow, and dedupe",async()=>{
 ...
 const maximum=harness({files:{[GLOBAL]:JSON.stringify(complete(`p/${"x".repeat(253)}`))},activeTools:["subagent_review"]});maximum.add(`p/${"x".repeat(253)}`);maximum.register();await maximum.start();const maxCard=await maximum.hooks.get("before_agent_start")({systemPrompt:"s"});assert.match(maxCard.systemPrompt,/awf subagent routing/);
});

test("all profiles select, prepare contracts, preserve state, and enforce commit policy",async()=>{
 const h=harness({files:{[GLOBAL]:JSON.stringify(complete())},exec:checkoutGit("a","b")});h.register();await h.start();const ps=h.profiles();assert.deepEqual(ps.map((p:any)=>[p.id,p.concurrency]),[["awf-grounding",10],["awf-explore",10],["awf-review",10],["awf-implement",1]]);
 const args=[{task:"g"},{task:"e",breadth:"broad",detail:"analysis"},{task:"r",kind:"adr"},{task:"i",allowCommits:true}];
 for(let i=0;i<4;i++){const c=context(args[i]), selected=await ps[i].selectModel(c);assert.equal(`${selected.provider}/${selected.id}`,"p/model");const state=await ps[i].beforeRun?.(c);const prepared=await ps[i].prepare(c);assert.equal(prepared.cwd,ROOT);assert.match(prepared.systemPrompt,/Contract body/);assert.doesNotMatch(prepared.systemPrompt,/name: role/);if(ps[i].afterRun){const out=await ps[i].afterRun({state:"completed"},state);assert.ok(out.profileData);}}
 const explicit=await ps[0].selectModel(context({task:"x",model:"p/model"}));assert.equal(explicit.thinkingLevels.length,7);assert.equal((await ps[0].selectModel({args:{task:"x"}})).provider,"p");
 const forbidden=harness({exec:checkoutGit("a","b")});forbidden.register();await forbidden.start();const ip=forbidden.profiles()[3],state=await ip.beforeRun(context({task:"x",allowCommits:false})),out=await ip.afterRun({},state);assert.match(out.failure,/committed despite/);assert.equal(out.profileData.commitVerification,"verified");
 const unchanged=harness({exec:checkoutGit("a","a")});unchanged.register();await unchanged.start();const up=unchanged.profiles()[3],us=await up.beforeRun(context({task:"x",allowCommits:true})),uo=await up.afterRun({},us);assert.match(uo.failure,/created no commit/);
 const unavailable=harness({exec:()=>({code:1,stdout:""})});unavailable.register();await unavailable.start();const vp=unavailable.profiles()[3],vs=await vp.beforeRun(context({task:"x",allowCommits:false})),vo=await vp.afterRun({},vs);assert.equal(vo.failure,undefined);assert.equal(vo.profileData.commitVerification,"unavailable");
});

test("contracts fail closed for missing and empty role artifacts",async()=>{for(const content of [err("no","ENOENT"),"---\nname: x\n---\n \n"]){const h=harness();h.deps.readFile=async()=>{if(content instanceof Error)throw content;return content;};h.register();await h.start();for(const [i,args] of [[0,{task:"x"}],[1,{task:"x",breadth:"targeted",detail:"paths"}],[2,{task:"x",kind:"code"}],[3,{task:"x",allowCommits:false}]] as any)await assert.rejects(h.profiles()[i].prepare(context(args)),/Missing Pi|no instruction body/);}});
...
test("verification checkout rejects every insecure or stale identity branch",async()=>{
 ...
 for(const [value,o,re] of cases){const h=harness(o);h.register();await h.start();await assert.rejects(h.profiles()[3].beforeRun(context({task:"x",allowCommits:true,verificationCheckout:value})),re,String(value));}
 const rootCanonical=harness({exec:checkoutGit(),realpath:async(p:string)=>{if(p===ROOT)throw err("root denied");return p;}});rootCanonical.register();await rootCanonical.start();await assert.rejects(rootCanonical.profiles()[3].beforeRun(context({task:"x",allowCommits:true,verificationCheckout:WORKTREE})),/project root/);
 const topCanonical=harness({exec:checkoutGit(),realpath:async(p:string)=>{if(p===WORKTREE)throw err("top missing");return p;}});topCanonical.register();await topCanonical.start();await assert.rejects(topCanonical.profiles()[3].beforeRun(context({task:"x",allowCommits:true,verificationCheckout:WORKTREE})),/does not exist/);
});
