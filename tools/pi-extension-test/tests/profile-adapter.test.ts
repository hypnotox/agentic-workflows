...
function harness(options:{files?:Record<string,string|Error>;answers?:Answer[];activeTools?:any;sessionManager?:any;exec?:(args:string[],cwd:string,index:number)=>any;realpath?:(p:string)=>Promise<string>;lstat?:(p:string)=>Promise<any>}={}) {
 ...
 void recorder.install((pi:any)=>registerSubagentTools(pi,deps)); return h;
}
...
test("default factory composes production dependencies",async()=>{const recorder:any=createExtensionRecorder({exec:async()=>({code:1,stdout:"",stderr:"",killed:false})});await recorder.install(extension as any);assert.ok(recorder.handlers.has("session_start"));assert.ok(recorder.apiCalls.some((call:any)=>call.name==="events.on"&&call.args[0]==="pi-tools:subagent-profiles:capability"));});
