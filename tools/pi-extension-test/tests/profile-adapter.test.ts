...
test("all profiles select, prepare contracts, preserve state, and enforce commit policy",async()=>{
 ...
 for(let i=0;i<4;i++){const c=context(args[i]), selected=await ps[i].selectModel(c);assert.equal(`${selected.provider}/${selected.id}`,"p/model");const state=await ps[i].beforeRun?.(c);const prepared=await ps[i].prepare(c);assert.equal(prepared.cwd,ROOT);assert.match(prepared.systemPrompt,/Contract body/);assert.doesNotMatch(prepared.systemPrompt,/name: role/);if(ps[i].afterRun){const out=await ps[i].afterRun({state:"completed"},state);assert.ok(out.profileData);assert.equal(Value.Check(ps[i].profileDataSchema,out.profileData),true);assert.deepEqual({requestedModel:out.profileData.requestedModel,resolvedProvider:out.profileData.resolvedProvider,resolvedModel:out.profileData.resolvedModel,resolutionSource:out.profileData.resolutionSource},{requestedModel:null,resolvedProvider:"p",resolvedModel:"model",resolutionSource:i===0?"global-role":i===1?"global-role":i===2?"global-role":"global-role"});}}
 ...
 const forbidden=harness({exec:checkoutGit("a","b")});forbidden.register();await forbidden.start();const ip=forbidden.profiles()[3],fc=context({task:"x",allowCommits:false});await ip.selectModel(fc);const state=await ip.beforeRun(fc),out=await ip.afterRun({},state);assert.match(out.failure,/committed despite/);assert.equal(out.profileData.commitVerification,"verified");assert.equal(Value.Check(ip.profileDataSchema,out.profileData),true);
 const unchanged=harness({exec:checkoutGit("a","a")});unchanged.register();await unchanged.start();const up=unchanged.profiles()[3],uc=context({task:"x",allowCommits:true});await up.selectModel(uc);const us=await up.beforeRun(uc),uo=await up.afterRun({},us);assert.match(uo.failure,/created no commit/);assert.equal(Value.Check(up.profileDataSchema,uo.profileData),true);
