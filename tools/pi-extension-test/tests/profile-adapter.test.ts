...
test("all profiles select, prepare contracts, preserve state, and enforce commit policy",async()=>{
 const h=await harness({files:{[GLOBAL]:JSON.stringify(complete())},exec:checkoutGit("a","b")});h.register();await h.start();const ps=h.profiles();assert.deepEqual(ps.map((p:any)=>[p.id,p.concurrency]),[["awf-grounding",10],["awf-explore",10],["awf-review",10],["awf-implement",1]]);
 const args=[{task:"g"},{task:"e",breadth:"broad",detail:"analysis"},{task:"review"},{task:"i"}];
 for(let i=0;i<4;i++){const c=context(args[i]), selected=await ps[i].selectModel(c);assert.equal(`${selected.provider}/${selected.id}`,"p/model");const prepared=await ps[i].prepare(c);assert.equal(prepared.cwd,ROOT);const state=await ps[i].beforeRun?.(c);assert.match(prepared.systemPrompt,/Contract body/);assert.doesNotMatch(prepared.systemPrompt,/name: role/);if(i===2)assert.match(prepared.systemPrompt,/reviewer\.md/);if(ps[i].afterRun){const out=await ps[i].afterRun({state:"completed"},state);assert.ok(out.profileData);assert.equal(Value.Check(ps[i].profileDataSchema,out.profileData),true);assert.deepEqual({requestedModel:out.profileData.requestedModel,resolvedProvider:out.profileData.resolvedProvider,resolvedModel:out.profileData.resolvedModel,resolutionSource:out.profileData.resolutionSource},{requestedModel:null,resolvedProvider:"p",resolvedModel:"model",resolutionSource:"global-role"});}}
 ...
 const changed=await harness({exec:checkoutGit("a","b")});changed.register();await changed.start();const ip=changed.profiles()[3],ic=context({task:"x"});await ip.selectModel(ic);await ip.prepare(ic);const state=await ip.beforeRun(ic),out=await ip.afterRun({},state);assert.match(out.failure,/changed HEAD.*restore the recorded before-run HEAD a before redispatch/);assert.doesNotMatch(out.failure,/recorded before-run HEAD b|Retry with verificationCheckout/);assert.equal(out.profileData.commitVerification,"verified");assert.equal(Value.Check(ip.profileDataSchema,out.profileData),true);
 for(const unavailableAt of ["before","after"]){let heads=0;const unavailable=await harness({exec:(args:string[])=>args.includes("HEAD")&&((unavailableAt==="before"&&heads++===0)||(unavailableAt==="after"&&heads++===1))?{code:1,stdout:""}:args.includes("HEAD")?{code:0,stdout:"a\n"}:args[0]==="status"?{code:0,stdout:""}:{code:1,stdout:""}});unavailable.register();await unavailable.start();const vp=unavailable.profiles()[3],vc=context({task:"x"});await vp.selectModel(vc);await vp.prepare(vc);const vs=await vp.beforeRun(vc),vo=await vp.afterRun({},vs);assert.match(vo.failure,/unavailable.*Repair snapshot availability.*retry with verificationCheckout set to that checkout root\./);assert.equal(vo.profileData.commitVerification,"unavailable");assert.equal(Value.Check(vp.profileDataSchema,vo.profileData),true);}
...
test("verification checkout rejects every insecure or stale identity branch",async()=>{
 ...
 for(const [value,o,re] of cases){const h=await harness(o);h.register();await h.start();await assert.rejects(h.profiles()[3].prepare(context({task:"x",verificationCheckout:value})),re,String(value));}
 const rootCanonical=await harness({exec:checkoutGit(),realpath:async(p:string)=>{if(p===ROOT)throw err("root denied");return p;}});rootCanonical.register();await rootCanonical.start();await assert.rejects(rootCanonical.profiles()[3].prepare(context({task:"x",verificationCheckout:WORKTREE})),/project root/);
 const topCanonical=await harness({exec:checkoutGit(),realpath:async(p:string)=>{if(p===WORKTREE)throw err("top missing");return p;}});topCanonical.register();await topCanonical.start();await assert.rejects(topCanonical.profiles()[3].prepare(context({task:"x",verificationCheckout:WORKTREE})),/does not exist/);
});
