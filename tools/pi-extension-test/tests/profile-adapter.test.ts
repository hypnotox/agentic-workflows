...
async function harness(options:{files?:Record<string,string|Error>;answers?:Answer[];activeTools?:any;sessionManager?:any;exec?:(args:string[],cwd:string,index:number)=>any;realpath?:(p:string)=>Promise<string>;lstat?:(p:string)=>Promise<any>}={}) {
...
test("verification checkout rejects every insecure or stale identity branch",async()=>{
 ...
 for(const [value,o,re] of cases){const h=await harness(o);h.register();await h.start();await assert.rejects(h.profiles()[5].beforeRun(context({task:"x",allowCommits:true,verificationCheckout:value})),re,String(value));}
 const rootCanonical=await harness({exec:checkoutGit(),realpath:async(p:string)=>{if(p===ROOT)throw err("root denied");return p;}});rootCanonical.register();await rootCanonical.start();await assert.rejects(rootCanonical.profiles()[5].beforeRun(context({task:"x",allowCommits:true,verificationCheckout:WORKTREE})),/project root/);
 const topCanonical=await harness({exec:checkoutGit(),realpath:async(p:string)=>{if(p===WORKTREE)throw err("top missing");return p;}});topCanonical.register();await topCanonical.start();await assert.rejects(topCanonical.profiles()[5].beforeRun(context({task:"x",allowCommits:true,verificationCheckout:WORKTREE})),/does not exist/);
});
