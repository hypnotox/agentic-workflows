...
test("all profiles select, prepare contracts, preserve state, and enforce commit policy",async()=>{
 ...
 const args=[{task:"g"},{task:"e",breadth:"broad",detail:"analysis"},{task:"adr"},{task:"plan"},{task:"code"},{task:"i"}];
...
test("verification checkout rejects every insecure or stale identity branch",async()=>{
 ...
 for(const [value,o,re] of cases){const h=await harness(o);h.register();await h.start();await assert.rejects(h.profiles()[5].prepare(context({task:"x",verificationCheckout:value})),re,String(value));}
 const rootCanonical=await harness({exec:checkoutGit(),realpath:async(p:string)=>{if(p===ROOT)throw err("root denied");return p;}});rootCanonical.register();await rootCanonical.start();await assert.rejects(rootCanonical.profiles()[5].prepare(context({task:"x",verificationCheckout:WORKTREE})),/project root/);
 const topCanonical=await harness({exec:checkoutGit(),realpath:async(p:string)=>{if(p===WORKTREE)throw err("top missing");return p;}});topCanonical.register();await topCanonical.start();await assert.rejects(topCanonical.profiles()[5].prepare(context({task:"x",verificationCheckout:WORKTREE})),/does not exist/);
});
