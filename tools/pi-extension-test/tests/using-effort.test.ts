...
function harness(replies: any[] = [], opts: { directory?: any; emitThrows?: boolean } = {}) {
  const tools = new Map<string, any>(), hooks = new Map<string, any>(), listeners = new Map<string, any>(); const events: any[] = [], calls: any[] = [];
  const pi: any = { registerTool: (x: any) => tools.set(x.name, x), on: (n: string, h: any) => hooks.set(n, h), events: { emit: (n: string, p: any) => { if (opts.emitThrows) throw new Error("emit"); events.push([n, p]); }, on: (n: string, h: any) => listeners.set(n, h) }, exec: async (_: string, args: string[]) => { calls.push(args); const r = replies.shift() ?? success(args[2] as any, args[5], args[3]); if (r instanceof Error) throw r; return { code: 0, stdout: typeof r === "string" ? r : line(r), stderr: "" }; } };
  ...
  return { pi, tools, hooks, listeners, events, calls, tool, ctx };
}
async function request(h: any, args: any) { return h.tool().execute("id", args, h.ctx); }
...
test("switching preserves prior association on detach refusal and otherwise remains detached on attach refusal", async () => {
  ...
  const repeat = harness([success(), success("attached", OTHER)]); await request(repeat, { effort: "demo" }); await request(repeat, { effort: "demo" }); assert.deepEqual(repeat.calls.map((x: any) => x[2]), ["attach", "attach"]);
});
...
test("direct association handles impossible success, detach refusal, and shutdown failure", async () => {
...
test("association lifecycle covers idle turns, malformed heartbeat facts, and serialized recovery", async () => {
  ...
  const serial = harness([success("attached", OTHER), success("attached", OTHER)]);
...
test("remote Pi capability, replay, publication failures, restart, and shutdown remain advisory", async () => {
  ...
  await defaultTool.execute("id", { effort: "demo" }, { cwd: "/tmp" });
  let missingDirectoryTool: any; effortExtension({ exec: standalone.exec, registerTool: (tool: any) => { missingDirectoryTool = tool } });
  await missingDirectoryTool.execute("id", { effort: "missing-dir" }, { cwd: "/definitely-missing" });
