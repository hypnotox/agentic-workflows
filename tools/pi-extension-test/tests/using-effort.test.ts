...
test("stdin child executor settles success, spawn failure, process error, and pre-abort without leaked handles", async () => {
  ...
  const errorHarness = childHarness(); const errorPromise = errorHarness.exec("./awf", [], { cwd: "/repo", timeout: 1 }); errorHarness.child.emit("error", new Error("child")); await assert.rejects(errorPromise, /child/); assert.equal(errorHarness.timers.every((timer) => timer.cleared), true);
...
test("child executor terminal guards tolerate close and output races", async () => {
  const settled = childHarness(); const promise = settled.exec("./awf", [], { cwd: "/repo", timeout: 5 }); const close = settled.child.listeners("close")[0] as (code: number | null) => void; const stdout = settled.child.stdout.listeners("data")[0] as (chunk: string) => void; settled.child.emit("close", 0); assert.equal((await promise).code, 0); close(0); stdout("x".repeat(MEMORY_STDOUT_MAX + 1)); assert.deepEqual(settled.kills, []);
