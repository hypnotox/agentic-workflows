...
test("index harness awaits recorder installation", async () => {
...
test("protocol-v2 registration is factory-time, atomic, correlated, and terminal notices are once-only", async()=>{
 const h=await harness(); assert.equal(h.request().name,"pi-tools:subagent-profiles:request"); assert.equal(h.request().v.protocolVersion,2);
 h.capability({protocolVersion:2,correlationId:"other",register:()=>assert.fail("foreign")}); assert.equal(h.batch(),undefined);
 const profiles=register(h); assert.equal(h.batch().registrationId,"awf:subagent-profiles:v2"); assert.equal(h.batch().suppressDefault,true); assert.equal(profiles.length,4); assert.deepEqual(profiles.map(p=>p.concurrency),[10,10,10,4]); assert.equal(profiles[3].exclusiveParentBatch,undefined);
 await h.hooks.session_start({},h.ctx); await new Promise<void>((resolve) => setTimeout(resolve,0)); assert.equal(h.notices.length,0);
 const rejected=await harness(); rejected.eventHandlers["pi-tools:subagent-profiles:registration-result"]({protocolVersion:2,registrationId:"awf:subagent-profiles:v2",state:"rejected",reason:"no"}); await rejected.hooks.session_start({},rejected.ctx); await new Promise<void>((resolve) => setTimeout(resolve,0)); assert.equal(rejected.notices.length,1); assert.match(rejected.notices[0][0],/no/);
 const missing=await harness(); await missing.hooks.session_start({},missing.ctx); await new Promise<void>((resolve) => setTimeout(resolve,0)); assert.equal(missing.notices.length,1); assert.match(missing.notices[0][0],/missing, late, or incompatible/);
});

test("profiles retain exact schemas, rendered contracts, routing selection, metadata, and commit policy", async()=>{
 const values=all(); const h=await harness({[global]:JSON.stringify(values)}); for(const ref of Object.values(values))h.add(ref); const ps=register(h); await h.hooks.session_start({},h.ctx);
 for(const p of ps) assert.equal(Value.Check(p.parameters, p.id==="awf-explore"?{task:"x",breadth:"targeted",detail:"paths"}:p.id==="awf-implement"?{task:"x"}:{task:"x"}),true);
 const contexts:any[]=[{args:{task:"x"},parent:{model:h.ctx.model}},{args:{task:"x",breadth:"broad",detail:"analysis"},parent:{model:h.ctx.model}},{args:{task:"x"},parent:{model:h.ctx.model}},{args:{task:"x"},parent:{model:h.ctx.model}}];
 for(let i=0;i<ps.length;i++){ const p=ps[i]; const c=contexts[i]; const selected=await p.selectModel(c); const preferenceIndex=i===0?1:i===1?2:i===2?3:4; assert.equal(`${selected.provider}/${selected.id}`, Object.values(values)[preferenceIndex]); const prepared=await p.prepare(c); const state=await p.beforeRun?.(c); assert.match(prepared.systemPrompt,/body/); assert.equal(prepared.cwd,root); if(p.afterRun) { const out=await p.afterRun({state:"completed"},state); assert.ok(out.profileData); } }
...
test("preference loading reloads global and local sources and invalid explicit overrides remain separate", async()=>{
 const h=await harness({[global]:JSON.stringify({default:"p/model"}),[local]:JSON.stringify({review:"p/model"})}); const state=await routing.loadPreferenceState(h.deps,h.ctx.modelRegistry); assert.equal(state.effective.review?.scope,"project"); assert.equal(routing.resolveChildModel(h.ctx.modelRegistry,h.ctx.model,"review","p/model",state).source,"requested"); h.models.delete("p/model"); assert.deepEqual(routing.registryFailures(h.ctx.modelRegistry,[state.global])[0],{kind:"field",scope:"global",field:"default",reason:"unregistered"});
});
