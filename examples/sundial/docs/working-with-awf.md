...
- `awf effort new --slug <slug> <outcome-title> [--json] [--no-worktree] [--base <ref>]`: require a caller-supplied canonical 1-through-32-byte new slug, publish unchanged schema-2 state with owned `.awf/efforts/<slug>/memory.md`, and create the managed `.awf/worktrees/<slug>/` checkout on `awf/<slug>` by default, reporting the path to continue in; flags are interspersed around the single independent title positional, existing residents through 63 bytes remain usable, `--no-worktree` keeps the invoking checkout, and `integrate`, `worktree remove`, and final `finish` stay separate operations.
...
CI (`"$(bash .awf/bootstrap.sh)" check`). `hooks` toggles five rendered git-hook payloads under
