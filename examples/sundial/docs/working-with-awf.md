...
- `awf read plan <plan> <P[.T]>`: resolve an exact filename or stem under the configured plans directory and print internal/plan's executable phase or task closure. Plan-v2 composes selected Applying Decisions, then Context Decisions, selected content, phase-owned Advances and Completes outcomes, Phase close, Definition of done, and Notes; a task scope notice makes these outcomes and close constraints rather than transferred authority. Plan-v1 remains compatibility behavior, while marker-absent historical plans retain legacy checking but have no executable projection.
...
Pi also renders the separate `handoff_session` extension for persisted interactive TUI sessions; it rejects cleanly when the runtime lacks the readonly persisted-session query. Workflow guidance calls it alone with exact `{memoryPath, kickoff}` arguments after persistence and a continuity notice or explicit approval. When supplied, memoryPath must normalize exactly to `.awf/efforts/<slug>/memory.md`; validation pins slug grammar, lexical and no-follow confinement, current ownership, one hard link, a 1 MiB valid-UTF-8 bound, stable file identity, either exact legacy `Effort: <slug>` or canonical frontmatter `effort: <slug>` identity, and repository identity. Handoff validates identity only: it never validates mutable Phase/Next/Updated metadata, parses `state.json` or `activity.json`, selects or assigns an effort, invokes an effort command, or mutates memory. A five-second Esc/Ctrl+C window precedes revalidation and parent-linked replacement. The old history and memory remain preserved. Kickoff failure leaves the exact wrapper in the editor; post-queue failure leaves recovery text, never retries automatically, and never starts a model turn.
Pi also renders the separate `handoff_session` extension for persisted interactive TUI sessions; it rejects cleanly when the runtime lacks the readonly persisted-session query. Workflow guidance calls it alone with exact `{kickoff}` prose after a required checkpoint and any needed reorientation instruction. The runtime preserves that bounded prose unchanged while it performs a five-second Esc/Ctrl+C countdown and parent-linked replacement. It revalidates only the pending request and persisted session, preserves old history, leaves failed automatic kickoff in the editor, exposes recovery text after a failure, never retries automatically, and owns no checkpoint, effort, repository, or filesystem policy.
