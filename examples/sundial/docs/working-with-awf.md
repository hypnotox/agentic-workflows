...
`direct `awf effort` commands new <title>`, `use <id>`, `clear`, `show`, and `rename <title>` manage one optional selected effort. `native skill discovery` only loads a fixed rendered workflow body and never changes lifecycle state. Handoff works without selection or memory; when both are present their effort identity must match, and child assignment occurs through the binary before kickoff.
