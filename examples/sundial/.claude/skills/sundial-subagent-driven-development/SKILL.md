...
4. For a `subagent-driven` phase, verify a known clean and green baseline. Call one implementation child alone in its parent tool batch with `allowCommits: true`. Choose the smallest model expected to complete reliably: `small` is for narrow, mechanical, low-ambiguity work; `standard` is for substantive but bounded work; and `large` is for broad, intricate, cross-cutting, or high-consequence work. Uncertainty, failed reasoning, or widened scope requires reconsideration and possible escalation. Select the smallest reliable target-native model explicitly; if this harness cannot select a model, use its default and note in the dispatch brief that explicit selection is unavailable, and the complete phase. The commit-capable owner performs all ordered work. Stage the complete transaction, run `awf check --staged`, then `./x gate`. Commit only after both commands pass; the hook repeats the staged check as defense in depth, then creates the declared phase-closing commit. Never dispatch concurrent implementers or helpers as phase owners.
...
5. Validate the reported commit and clean checkout, then dispatch one report-only phase review. Choose the smallest model expected to complete reliably: `small` is for narrow, mechanical, low-ambiguity work; `standard` is for substantive but bounded work; and `large` is for broad, intricate, cross-cutting, or high-consequence work. Uncertainty, failed reasoning, or widened scope requires reconsideration and possible escalation. Select the smallest reliable target-native model explicitly; if this harness cannot select a model, use its default and note in the dispatch brief that explicit selection is unavailable. The parent owns focused settlement commits and checkpoints only after findings resolve.
