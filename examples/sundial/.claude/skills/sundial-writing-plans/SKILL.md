...
description: Use to write an implementation plan for a complex sundial task under docs/plans. Enforces project plan conventions (bite-sized tasks, exact file paths, exact content, exact commands). Terminal step invokes sundial-reviewing-plan or routes to an execution skill.
---
...
Writes a plan to `docs/plans/YYYY-MM-DD-<topic>.md` per the sundial plan convention. The plan is the execution record; the design lives in the linked ADR(s) (when any exist) — a plan may link zero or more. Do not duplicate rationale — link.
