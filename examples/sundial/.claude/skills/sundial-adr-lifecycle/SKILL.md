...
- `Implementing → Implementing` appends exactly one Applied batch with its mutations, including the final batch, or one Reapplied batch correcting an already-applied add or update.
- `Implementing → Implemented` appends only the terminal status event after implementation has applied every declaration.
...
1. **Edit the ADR history and status.** For a first incremental batch, append Implementing status then an Applied event; each later batch, including the final one, appends one Applied event without changing status; settled review later appends only Implemented; abandonment appends only Abandoned with a rationale. Applied grammar is `- YYYY-MM-DD: Applied; operations: <operation-list>`, whose members may appear in any order. If an already-applied add or update needs another material correction, append `- YYYY-MM-DD: Reapplied; operations: <operation-list>` without changing status. Reapplied operations must have an earlier Applied occurrence, may repeat, and do not change progress; removes and events after terminal status are refused. Repeat the latest stamp on status events; an amendment instead appends its own `- YYYY-MM-DD: Amended; content-sha256: <digest of the amended content>` event in its own commit.
