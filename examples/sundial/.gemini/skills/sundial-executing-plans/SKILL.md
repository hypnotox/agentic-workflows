...
1. Resolve the mutable plan and read its complete phase structure. Before mutation, validate the one immutable effort slug and exact `.awf/efforts/<slug>/memory.md` path, confirm either legacy `Effort: <slug>` or canonical `effort: <slug>` identity, and preserve one user-managed writer. Repository sources and current-state documentation outrank checkpoint prose, and standalone memory is forbidden.
