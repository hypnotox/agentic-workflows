...
A minimal simple fix uses no effort. Once work is a concrete non-minimal outcome, create or resume exactly one immutable slugged effort with `awf effort new "<outcome>"`; it always owns `.awf/efforts/<slug>/memory.md`. The effort's managed worktree is the default execution location (`--no-worktree` is the explicit exception), while the owned memory stays under the primary checkout. Carry the slug and exact path through mandatory checkpoints, but verify checkpoint claims against repository sources and current-state documentation. A routine checkpoint persists the effort and then permits a discretionary eligible session-replacement choice where the target supports one; a safe resumable point has an immediate successor that can start independently. The new session logs an actual replacement boundary before substantive work, while in-session continuation or a replacement that leaves the old session active logs none. Mandatory approval remains an explicit stop until its approval and next action are persisted. One effort has one user-managed writer; report-only children and helpers never edit shared memory. Never create standalone memory, commit the owned file, or cite its concrete path in an ADR, plan, or commit message. Worktree-backed efforts integrate and remove topology after terminal review and before retrospective; retrospective finishes the effort last. The detailed protocol lives in the workflow doc's working-memory section.
