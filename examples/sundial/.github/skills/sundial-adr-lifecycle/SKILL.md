...
5. **Validate the staged transaction.** Stage the complete transaction; the commit requires `awf check --staged` and `./x gate` to pass. A wired pre-commit hook enforces both at commit time; run them manually first only in a clone without wired hooks. If either command fails, fix the cause and re-stage before retrying.
