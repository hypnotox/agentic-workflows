...
5. **Hand off after resync settles.** Once the resync review converges (no user-decision findings, or all user decisions resolved), the next chain node is implementation. Call `awf_workflow` alone with `skill: "executing-plans"` for inline execution or `skill: "subagent-driven-development"` for subagent-per-task execution, depending on the plan's task structure.
