...
Any time behaviour is wrong and the cause is not yet known. Skip only for a one-line fix whose failing test already exists and whose cause is obvious (call `awf_workflow` alone with `skill: "bugfix"` directly in that case). If the bug reveals a load-bearing design gap rather than a straightforward defect, escalate to `awf_workflow` with `skill: "brainstorming"` in a singleton call after confirming the root cause.
...
6. **Hand off.** When the root cause is confirmed and the failing test is in place, call `awf_workflow` alone with `skill: "bugfix"` for the fix + commit + review discipline. If investigation reveals a design gap rather than a defect, call `awf_workflow` alone with `skill: "brainstorming"` instead.
