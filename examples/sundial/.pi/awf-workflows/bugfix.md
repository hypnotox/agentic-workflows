...
If the root cause is not yet known, call `awf_workflow` alone with `skill: "debugging"` first.

## Procedure

1. **Ensure a regression test exists that fails for the right reason.** Call `awf_workflow` alone with `skill: "tdd"` for the project's test-first discipline: it picks the right surface, writes the failing test, and verifies it fails for the right reason before the fix.
...
5. **Call `awf_workflow` alone with `skill: "executing-direct"`, then continue through its routed implementation-review successor.**
