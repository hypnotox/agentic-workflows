...
3. **Call `subagent_review` with `kind: "code"`, independently from the implementer.** Put the complete reviewer brief in `task`; optionally set exact `model` as `provider/model-id`; omission inherits the parent. The brief must include:
   ...
   - The affected context instruction: with the concrete SHAs substituted, direct the reviewer to run `awf context --show all-rules --show evidence --show pending $(git diff --name-only ${baseSha}..${headSha})` itself so the doc-currency and convention lenses receive the additional topic rules, backing evidence, and pending changes they need without unrelated facets. Pass the resolved SHA range, not pasted packet output.
