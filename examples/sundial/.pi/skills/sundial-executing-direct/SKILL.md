...
1. Before mutation, classify and validate continuity. A minimal simple fix uses no effort. A concrete non-minimal outcome must already have exactly one immutable slugged effort that always owns `.awf/efforts/<slug>/memory.md`; confirm either legacy `Effort: <slug>` or canonical `effort: <slug>` identity, carry the exact path, and preserve one user-managed writer. Repository sources and current-state documentation outrank the checkpoint, helpers never edit shared memory, and standalone memory is forbidden. Re-read the settled design brief and the applicable current-state authority. Per `docs/maintainable-code-design.md`, assess bounded enabling refactoring before editing and preserve settled boundaries. Do not introduce design choices that brainstorming did not settle.
