...
- **Phase-closing subject:** each phase declares one closing subject in a fenced `commit` block. The phase owner stages the complete transaction and creates that one closing commit, which requires `awf check --staged` and `./x gate` to pass; a wired pre-commit hook enforces both, and a manual run first is needed only in a clone without wired hooks.
