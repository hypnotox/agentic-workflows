...
3. **Call `subagent_review` with `kind: "code"`, independently from the implementer.** Put the complete reviewer brief in `task`; optionally set exact `model` as `provider/model-id`; omission inherits the parent. The brief must include:
   ...
   - The affected context (paste the output of `awf context $(git diff --name-only ${baseSha}..${headSha})`) so the doc-currency and convention lenses know the owning domains, the applicable current-state claims (rules and invariants), and any Accepted pending changes without re-deriving them.
