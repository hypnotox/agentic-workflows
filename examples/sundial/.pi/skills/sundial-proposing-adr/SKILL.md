---
name: sundial-proposing-adr
description: >
  Use to propose a new ADR under docs/decisions for sundial.
  Writes the ADR file with required frontmatter and sections, regenerates
  INDEX.md, commits, and hands off to sundial-reviewing-adr.
---
...
8. **Terminal step:** invoke `sundial-reviewing-adr` via the project's skill-invocation mechanism, passing the ADR path. The reviewer applies its lenses and reports findings; route them per the reviewing skill's procedure.
...
- The ADR stays `status: Proposed` through review. A later implementation step owns any nonterminal `Accepted` or `Implementing` transition and every explicit incremental batch; the terminal-review flow owns only the status-only `Implemented` transition. Direct terminal implementation remains one implicit batch. Those transitions use `sundial-adr-lifecycle`, not this skill. V2 claim operations may apply in individually checked batches with unordered operation membership; each Applied event travels with exactly its matching claim mutations.
