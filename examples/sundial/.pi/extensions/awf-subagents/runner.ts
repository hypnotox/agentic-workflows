...
export interface RunUpdate { events: readonly DisplayEvent[]; omittedEvents: number; }
    ...
    async run(request: RunRequest): Promise<RunResult> {
      ...
      try {
        ...
        const result = await new Promise<RunResult>((resolve, reject) => {
          ...
          const finish = (error?: Error, failed = false, failureMessage?: string): void => {
            if (settled) return;
            settled = true;
            ...
            else resolve(buildResult(failed, failureMessage));
          };
          const appendDisplayEvent = (event: DisplayEventInput): void => {
            ...
            request.onUpdate?.({ events: [...events], omittedEvents });
          };
          ...
          child.once("close", (code) => {
            ...
            if (forcedFailure) {
              ...
              finish(undefined, true, forcedFailure.message);
            } else if (code !== 0) {
              stopReason = "error";
              finish(undefined, true, `Subagent exited ${code}: ${truncateStderr(stderr) || "no diagnostics"}`);
            } else if (stopReason === "error" || stopReason === "aborted") {
              finish(undefined, true, `Subagent ${stopReason}: ${truncateOutput(output)}`);
            } else {
              finish();
            }
          });
