...
An ADR number, once assigned, never changes: a checked transition refuses a record that takes a different number or loses its number under the same slug. A stale numbering is unmade rather than corrected, so on a corpus carrying duplicate numbers and no pending record `awf adr number` refuses and offers the reset-remake recipe verbatim as a hint, deciding that from the corpus alone and reading no git provenance or merge base.
Origin: ADR-0194
Backing: test
