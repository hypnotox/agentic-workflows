...
Canonical workflow Markdown under `internal/docs/` owns the instructions: `effort.md` covers continuity, notes, worktrees, and handoffs; `changes.md` covers optional document roles, authored relationships, and ADR authority; `completion.md` covers verification, coherent commits, retrospective findings, and applicable integration and cleanup. Each also supplies its generated skill body for both harnesses. Completion entrypoints apply during implementation so verified intermediate units receive commit guidance before final completion. The workflow applies with or without an effort and routes to the document lifecycle only when those artifacts are relevant; small changes do not need the full ADR workflow.
