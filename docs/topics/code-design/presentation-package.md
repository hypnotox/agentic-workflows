...
**Applicability:** Owning domain selectors: `internal/checkresult/**`, `internal/presentation/**`. Topic selectors: `internal/presentation/**`. Both domain and topic selectors must match. Run `awf read topic code-design/presentation-package --coverage` for current applicable and owned paths and marker sites.
