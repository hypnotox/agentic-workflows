...
**Applicability:** Owning domain selectors: `internal/checkresult/**`, `internal/execution/**`, `internal/presentation/**`. Topic selectors: `internal/execution/**`. Both domain and topic selectors must match. Run `awf topic code-design/execution-planning --coverage` for current applicable and owned paths and marker sites.
