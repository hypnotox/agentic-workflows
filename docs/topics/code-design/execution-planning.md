...
**Applicability:** Owning domain selectors: `internal/execution/**`. Topic selectors: `internal/execution/**`. Both domain and topic selectors must match. Run `awf topic code-design/execution-planning --coverage` for current matched paths and marker sites.
