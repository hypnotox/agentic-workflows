...
Checkpoints are durable; check-ins are deliberate. A boundary runs three steps in order: persist working memory first (an existing or deliberately created file updates in its own tool batch), decide whether user attention is required (authority drift, materially changed choices, scope expansion, unresolved correctness or safety concerns, blockers, failed required verification), then either raise a check-in and stop or state a continuity notice and continue. A routine checkpoint summary is a continuity notice, not a stop. The end of brainstorming and the settled ADR review are the two mandatory approval check-ins: present the summary, explicitly request approval, and continue only after approval is granted and persisted. Target-native continuation follows without an unsupported session-replacement claim. Long implementation phases repeat the routine protocol after independently resumable committed and reviewed tasks.
