...
The ADR-0151 plan directed a doctor-command-contract proof marker onto
...
Two ADR-0151 proposal-phase commits staged a regenerated docs/decisions/INDEX.md without
the .awf/awf.lock outputHash update that the same `./x sync` had written to the working
...
pollutes the next unrelated commit or surfaces as confusing drift. After any `./x sync`,
...
and at merge time run `./x sync && ./x check` immediately after the rename to let the
...
therefore structurally unexecutable - every `./x sync`, `./x check`, and gate run fails to
...
`./x sync` regenerates `docs/decisions/INDEX.md` from the ADR corpus and regenerates topic
