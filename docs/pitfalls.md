...
**This entry was wrong twice over, and the corrections are the lesson.** It recorded two
instances (0079 → 0065, 0093 → 0024) and inferred a promotion trigger from them. A corpus
sweep on 2026-07-15 found **16 partial amendments, 10 of them missing the back-pointer**;
the two recorded here were merely the ones review happened to catch, and both had since
been fixed. Counting only the failures someone noticed understated the problem by five
times and pointed the diagnosis at the wrong place: prose *had* been tried (this note
...
`amends:` field with a derived back-edge). The corpus is knowingly left at 6 of 16
conformant (ADR-0116 Decision 6), so a fresh sweep will find residue: do not mistake it for
recurrence.
