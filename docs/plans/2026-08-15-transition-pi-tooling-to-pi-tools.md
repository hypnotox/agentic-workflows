...
- Phase deviation inventory (verbatim): `tools/pi-extension-test/tests/model-routing.test.ts` and `tools/pi-extension-test/tests/profile-adapter.test.ts` replaced the listed monolithic adapter and runner test ownership; `internal/project/render_tree_test.go`, `internal/project/repository_awf_invocation_test.go`, `internal/project/residue_scan_test.go`, and `internal/evals/chain_test.go` received necessary stale-output and generated-set corrections; `templates/docs/working-with-awf.md.tmpl` received the prerequisite text because that document's rendered section is template-owned. These additions preserved the approved atomic cutover and changed no durable boundary.
- Phase review settlement made routing metadata invocation-scoped and schema-bounded for all four profiles, removed the dead awf limiter because scheduling belongs exclusively to `pi-tools`, aligned the strict test-only protocol projection with the public protocol-v2 API, and made commit verification unavailable rather than falsely failing when either Git snapshot cannot be read.
- Phase review settlement moved test-backed invariant proof markers to the focused Go tests that exercise their complete claims, strengthened target, wiring, and runtime-smoke assertions for the handshake, runtime floor, Git audit, exclusivity, routing, role loading, editor quiet, and render boundary, and removed nominal context-classification proofs.
- Phase review settlement now delays the runtime double's final registration result until tool installation is complete, executes a captured profile lifecycle with schema validation, proves the generated routing card reaches a real parent model request, publishes executable unpinned installation guidance and the adopter-facing changelog entry, and keeps native skill discovery and effort registration covered without testing external `pi-tools` execution mechanics.
- Residual review settlement expanded the grounding ownership proof across Pi adapter schema and guidance, grounding and exploration skills, coupling-audit routing, and exhaustive non-Pi skill and agent leakage checks while retaining Claude-native dispatch evidence.
- Residual review settlement made the Go profile proof exact over all four closed public schemas, required and optional fields, tool names, concurrency and exclusivity, and made the shared role-loader proof cover every rendered role path, authority prepend, actionable repair, and absence of duplicated role prose. The strict TypeScript suite remains complementary runtime evidence.
