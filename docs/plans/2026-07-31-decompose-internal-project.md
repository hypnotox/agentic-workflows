...
Execute ADR-0195: carve `internal/contextq` and `internal/resident` out of `internal/project`,
...
- [ ] **Task 1.6: Land the updated claim prose and the Implementing flip.** In
  ...
  `Revised-by: ADR-0195`, keep `Backing: test`. Append to the ADR's Status history an
...
- [ ] **Task 2.4: Route the render kind comparison and prove single home.** First, route
  ...
  seam's own spelling, decided untouched (ADR-0195 item 7), and the ADR Consequences records
...
- [ ] **Task 2.5: Ownership, claim prose, Applied event.** Add `internal/resident/**` to
  ...
  `Origin: ADR-0195`, `Backing: test`. Update the architecture source part sentence describing the resident-root
...
- [ ] **Task 3.6: Ownership, claim prose, Applied event.** Add `internal/contextq/**` to
  ...
  value and its two core-side constructors." `Origin: ADR-0195`, `Backing: test`. Update the
...
- [ ] **Task 4.3: Claim prose, Applied event.** Land in
  ...
  `Origin: ADR-0195`, `Backing: unbacked`,
...
- [ ] **Task 5.4: Claim prose, Applied event.** Land in
  ...
  and content from its caller rather than re-reading the embedded tree." `Origin: ADR-0195`,
...
- [ ] **Task 6.2: Update the claim prose.** In
  ...
  `Revised-by: ADR-0195`, keep `Backing: test`.
- [ ] **Task 6.3: Close the record.** Rewrite the roadmap's deferred-decomposition section
  (`.awf/docs/parts/roadmap/deferred.md`) to point at ADR-0195, recording the sequencing
...
- The claim prose in `.awf/topics/parts/tooling/context-and-topic/current-state.md`, slug
  ...
  fails the structural test rather than degrading silently to unowned." `Origin: ADR-0195`,
  `Backing: test`.
...
- `./x check` and `./x gate` clean at every phase close; the whole-effort acceptance is the
  ...
  covered; `awf check` reports ADR-0195 operations 1-6 Applied with operation 7 remaining
...
- Deferred by decision: `internal/git` untouched (ADR-0195 item 7); slash/OS-space discipline;
...
- Deferred by Phase 1 review ruling (2026-07-31): the in-core kind-literal dispatch that
  ...
  to the proven scope instead (ADR-0195 item 6 records the reasoning). The scaffold base-TID
...
- Phase 6 deviations (2026-07-31): (1) Task 6.2's claim body shipped with its final clause
  ...
  future-effort acceptance instead of attributing it to ADR-0195, which records no such
  statement.
