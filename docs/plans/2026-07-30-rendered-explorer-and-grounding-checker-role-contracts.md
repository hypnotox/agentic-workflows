...
Operation-to-transaction assignment (ADR-0179 declares six; each Applied event travels in the commit
that makes its claim true, and declaration order is preserved across the batches):

| Operation | Transaction |
|---|---|
| add `rendering/workflow-skill-templates:explorer-and-grounding-role-contracts` | Phase 1 (first batch) |
| add `rendering/pi-workflows:pi-role-contract-loader` | Phase 1 |
| update `rendering/pi-workflows:pi-implement-role-artifact` | Phase 1 |
| update `rendering/workflow-skill-templates:bounded-exploration-reporting` | Phase 1 |
| update `rendering/workflow-skill-templates:cross-runtime-exploration-dispatch` | Phase 1 (last of batch) |
| update `rendering/workflow-skill-templates:implementer-role-contract` | Phase 3 (final batch) |

Phase 2 applies no operation. Phase 1 appends the `Implementing` status event before its five `Applied`
events; Phase 3 appends its one `Applied` event and then `Implemented`.
...
- **Modified:** `internal/catalog/standard.go`, `internal/catalog/catalog.go`,
  ...
  plus every file `./x render` regenerates (the rendered agents, skills, Pi extension, `AGENTS.md`,
  `docs/decisions/INDEX.md`, `docs/config-reference.md`, and both `awf.lock` files).
- **Deleted:** none.
...
- [ ] **Task 1.1: Create `templates/agents/explorer.md.tmpl`.** The body reuses the current
  ...
  limiter sentence, so the existing pinned literals move rather than change. Do not add backticks
  around `targeted`, `bounded`, `broad`, `paths`, `summary`, or `analysis`: the current literals carry
  none, and `rendering/workflow-skill-templates:skill-prose-tool-agnostic` scans for backticked tool
  words. Carry no decision-record citation in any comment (`TestTemplateSourceResidue` forbids it).
  Exact content:
...
- [ ] **Task 1.3: Register both `AgentSpec` entries in `internal/catalog/standard.go`.** Add to the
  `Agents: map[string]AgentSpec{` literal at `:118`, keeping the map's existing alphabetical-by-key
  reading order. Each `Sections` list must match its template's `awf:section` marker names exactly, in
  order. Both omit `RequiresSkills` and `Data`, per Tasks 1.1 and 1.2. Exact content:
...
- [ ] **Task 1.5: Add a golden test per agent in `internal/project/spine_test.go`.**
  `TestEveryCatalogArtifactHasGoldenTest` (`internal/project/catalog_sweep_test.go:139-152`) fails
  without them. Model both on `TestImplementerAgent` (`spine_test.go:277`): render via
  `renderAgentGolden(t, "<name>", data)` with `data` of `{"prefix": "example", "vars": map[string]any{},
  "data": map[string]any{}}`, assert `name: <agent>` appears, then assert one representative phrase per
  section so a dropped section fails loudly. For `explorer` those phrases are `"Handle exactly one
  information need"`, `"refinement of an earlier result stays sequential"`, `"Breadth is ordered
  targeted < bounded < broad"`, `"paths < summary < analysis"`, `"Ground every material claim with
  file:line evidence"`, and `"Retain no search session or state"`. For `grounding-checker` they are
  `"do not edit files or commit"`, `"Work only from the brief you were given"`, `"Surface unstated
  assumptions"`, `"advisory and single-pass"`, and `"open-question | possible-issue"`.
...
- [ ] **Task 1.6: Rewire both Pi call sites and delete `rolePrompt`.** In
  ...
  2. Add two loaders beside `loadImplementer`, each supplying all three `ContractSource` fields
     (`relative`, `noun`, `repair`, `prepend`); `prepend` is required and interpolated
     unconditionally at `:224`, so neither may omit it:
  ...
  5. Delete the entire `rolePrompt` function (`:174-201`) and its now-unused `role` parameter union.
     Both callers are gone after steps 3 and 4, so nothing references it. Keep the
     `ExplorationBreadth` and `ExplorationDetail` aliases: step 4's template literals still use them
     through `params`.

  Verify with `grep -n rolePrompt templates/pi/awf-subagents/index.ts.tmpl`, which must return no
  output.
...
- [ ] **Task 1.7: Name both agents in both dispatching skills.** In
  ...
  exploration subagent` must survive verbatim, because it is pinned at `target_test.go:429` and `:491`
...
- [ ] **Task 1.9: Revise `TestBoundedExplorationReporting` for the moved prose.** In
  ...
  The fixture at `target_test.go:455` renders with `agents: []`; add `explorer` to it so step 3's
  render produces a body.
...
- [ ] **Task 1.11: Author the two new claims and update the three existing ones.** In
  ...
  Place a proof marker for each of the two new claims on the tests that establish them: `// invariant:
  rendering/workflow-skill-templates:explorer-and-grounding-role-contracts` on Task 1.5's two golden
  tests, and `// invariant: rendering/pi-workflows:pi-role-contract-loader` on Task 1.10's two
  TypeScript-seam counterpart Go test, or on a new Go test asserting the rendered extension contains
  both loader functions and neither inline role body if no existing Go test covers it. Confirm with
  `./x check`, which fails on a claim with no matching proof and on a proof naming an unknown claim.

- [ ] **Task 1.12: Append the ADR's `Implementing` and five `Applied` status events.** Add to the
  `## Status history` of
  `docs/decisions/0179-rendered-explorer-and-grounding-checker-role-contracts.md`, in declaration
  order, one `Implementing` line then one `Applied` line per operation. Neither the content digest nor
  the state sequences may be precomputed here, because the sequence moves whenever any sibling ADR
  applies a batch. Obtain both with the documented method (`docs/pitfalls.md`, "An ADR's frozen digest
  and next state-sequence cannot be read directly"): write the digest as 64 zeros, run `./x check` and
  read the computed digest from the failure, correct it, then let the duplicate-sequence error name the
  next free sequence. The digest excludes `Status history`, so the same value repeats on the
  `Implementing` event here and the `Implemented` event in Phase 3. Line shapes:

  ```
  - 2026-07-30: Implementing; content-sha256: <computed>
  - 2026-07-30: Applied; state-sequence: <next>; operations: add `rendering/workflow-skill-templates:explorer-and-grounding-role-contracts`
  ```

  followed by one line per remaining Phase 1 operation, each with its own sequence, in the ADR's
  declaration order.

- [ ] **Task 1.13: Render, then verify the whole phase.** Run `./x render` (expect it to rewrite the
  rendered agents, both skills, the Pi extension, `AGENTS.md`, `docs/config-reference.md`, and both
  `awf.lock` files) and then `./x check`, which must print `awf check: clean`. Run `./x gate` early
  rather than only at the phase close: the `AgentSpec` and template guards fail fast and each costs a
  round trip.
...
- [ ] **Task 2.1: Pair the two direct dispatchers.** In `internal/catalog/standard.go`, add
  ...
  "grounding-checker"` to the `"brainstorming"` spec (`:10`), in each case immediately before
  `Sections:` to match the field order the paired specs already use at `:23` and `:29`. Do not touch
  `debugging` (`:45`) or `refactor-coupling-audit` (`:107`): they consume exploring transitively and
  ADR-0179 item 6 accepts that gap.
...
- [ ] **Task 2.4: Add the paired agent to every affected fixture.** This is a batch task. Each site
  ...
  Exhaustive affected-site set: `internal/project/render_tree_test.go:115` and `:141` (both
  `[explorer]`), `internal/project/skillrefs_test.go:88` (`[explorer]`) and `:102`
  (`[grounding-checker]`, since it enables `brainstorming` only),
  `internal/project/context_artifacts_test.go:260` (`[explorer]`),
  `internal/project/target_test.go:367` (both, per the edge above),
  `internal/project/target_test.go:455` (already given `[explorer]` by Task 1.9, so no further edit),
  and `internal/migrate/closeenabledset_test.go:82` (`[grounding-checker]`, since it enables
  `brainstorming` only).

  `internal/project/project_test.go:1458` is deliberately excluded: it pairs `brainstorming` with a
  `local: true` sidecar precisely to prove a local artifact skips the closure check, and it must keep
  passing untouched. Do not add an agent there.
...
- [ ] **Task 2.8: Upgrade both config trees to generation 24 and add the changelog entry.** Run `awf
  upgrade` against the repository root and against `examples/sundial`, so both trees carry the new
  schema stamp; `applyCloseEnabledSet` is a no-op for their `agents:` lists because Task 1.4 already
  enabled both agents. Add a `changelog/CHANGELOG.md` `[Unreleased]` entry (the section at `:9`)
  covering generation 24 and the two Pi tools that now fail closed without their rendered agent, as
  `docs/releasing.md:66` requires for adopter-facing change.

- [ ] **Task 2.9: Render and verify.** Run `./x render` then `./x check`, which must print `awf check:
  clean`. Confirm the generation gate accepts the tree: `./awf list` must succeed rather than refuse on
  schema generation.
...
- [ ] **Task 3.2: Correct the three README enumerations.** `README.md:46` currently reads "Three review
  agents (`adr-reviewer`, `plan-reviewer`, ..." and enumerates a catalog that now has six agents;
  reword it so the enumeration covers the reviewers, the implementer, the explorer, and the
  grounding-checker without asserting a count that the next agent falsifies. `README.md:249` reads "and
  four agents" and becomes six. `README.md:12` frames the agent set as "independent review agents";
  reword so it covers dispatched non-review roles too. A bare count bump would reintroduce the error
  ADR-0177 made, so reword rather than increment. Two surfaces need no edit and must not be touched
  reflexively: `internal/configspec`'s agents-key description carries no count, and `cmd/awf/main.go`'s
  package comment names artifact kinds without counting them.

- [ ] **Task 3.3: Append the final `Applied` and `Implemented` status events.** In the ADR's `## Status
  history`, append the sixth operation's `Applied` line then the `Implemented` line. Reuse the digest
  from Phase 1 unchanged (it excludes `Status history`); obtain the next free sequence by the same
  documented method, since sibling ADRs may have consumed sequences since Phase 1:
...
- [ ] **Task 3.5: Render and verify.** Run `./x render` (regenerating `docs/decisions/INDEX.md` and both
  locks for the status change) then `./x check`, which must print `awf check: clean`.
