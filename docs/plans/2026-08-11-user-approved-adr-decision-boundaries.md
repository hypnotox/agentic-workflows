...
Post-check: Run `go test ./internal/project -run 'Test(ADRReadmeDecisionRouting|ProposingAdrTemplate|V3ADRTemplateEmptyDataFallback|BrainstormingTemplate)$'`, inspect the generated ADR guide, scaffold, brainstorming skill, and proposing skill for coherent consent and altitude semantics, then require `./x render && ./x check` to leave no drift.
...
- 2026-08-11 Phase 1 reasoned path correction: added `internal/project/golden_test.go` because it owns the existing ADR guide and scaffold semantic rendering assertions that Task 1.1 must extend; keeping those checks in their single existing home is required by repository test structure.
- 2026-08-11 Phase 1 review settlement: limited the applied templates claim to behavior shipped in Phase 1 and recorded a Reapplied correction; completed plan-or-direct-execution routing, strengthened consent-evidence assertions, and moved the adopter-facing changelog entry into Phase 1. These corrections close the report-only review findings without changing approved semantics.
- 2026-08-11 Phase 2 review settlement: narrowed the memory-log claim to the actual effort-free plan and implementation review contract, recorded a Reapplied correction, and strengthened the named proofs for both ADR consent-evidence branches and reasoned surplus removal.
- 2026-08-11 Phase 2 renewed-review settlement: pinned the implementation-review template's exact effort-free field-omission clause and mutation-confirmed that deleting it fails `TestMemoryLogConsumerCoverage`.
- 2026-08-11 Terminal review settlement: restored ADR-0224's existing durable-rationale requirement as a conjunction with prior user acceptance for retained mechanisms in the guide, scaffold, proposing workflow, and ADR-scope lens; strengthened exact rendering assertions for both halves.
- 2026-08-11 Divergence-review settlement: restored the same durable-rationale conjunct in the `decision-artifact-routing` claim's manual Verify procedure and recorded a matching ADR-0264 Reapplied event.
