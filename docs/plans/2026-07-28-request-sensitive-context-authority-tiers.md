...
Implement [ADR-0174](../decisions/0174-request-sensitive-context-authority-tiers.md) so bare
...
the existing concise/full/path-attribution claims false. The transaction applies all six ADR-0174
...
- **Modified (lifecycle and generated outputs):**
  `docs/decisions/0174-request-sensitive-context-authority-tiers.md`, this plan,
...
- [ ] **Task 1.1: Pin the old observable projection around the refactor.** In
  ...
  `cmd/awf/context_test.go`, add table cases that capture the pre-ADR-0174 behavior which Phase 1
  must preserve:
...
- [ ] **Task 2.3: Render tiers, relationships, counts, and sources.** In `cmd/awf/context.go`:
  ...
  - existing selectors, claim summaries, evidence, references, pending, ADR navigation, artifact
    detail, and spill delivery retain their grammar unless ADR-0174 explicitly changes visibility.
...
- [ ] **Task 2.6: Update adopter-facing and current documentation.** Make the following authored
  ...
  - `.awf/docs/parts/testing/gate.md`: state that repository regression tests hold the two bare
    ADR-0174 request sets within direct delivery, while explicit detail remains spill-capable; do
...
- [ ] **Task 2.7: Apply all State changes and freeze the artifacts in the same transaction.** In
  ...
  the existing `Revised-by` prefix and append `ADR-0174`, and preserve backing mode and proof marker
  ownership:
  ...
  In ADR-0174, use the direct `Proposed` to `Implemented` transition: set frontmatter
...
- [ ] **Task 2.8: Verify the atomic contract transaction and commit.** Run these commands in order:
  ...
  allowed=(
    ...
    README.md
    changelog/CHANGELOG.md
    docs/decisions/0174-request-sensitive-context-authority-tiers.md
...
- [ ] **Task 3.1: Run governed implementation review over the concrete range.** From a clean tree,
  ...
  range, ADR-0174, and this plan. In addition to its ordinary lenses, require review of request-kind
  ...
  single verify pass. Any finding that changes ADR-0174's settled design requires user approval and
  a successor correction because ADR-0174 is Implemented.
...
The tree is drift-free and clean; all tests and the full gate pass; both claims show ADR-0174 in
`Revised-by` with their original backing modes; ADR-0174 and this plan are Implemented; and the
...
requires amending Proposed ADR-0174 plus fresh review before execution continues.
...
Integration found that current main had independently assigned ADR-0173 after this effort branched and had advanced the global state sequence through 78. The merged record was therefore renumbered mechanically to ADR-0174, its references and plan metadata moved with it, and its direct implementation batch received state-sequence 79.
