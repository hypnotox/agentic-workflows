...
- Terminal assurance found that schema-38 retirement did not resolve YAML aliases safely, ADR review omitted the references facet needed for typed linked-plan discovery, and the no-depth-control census remained too narrow. The assurance fix removes every semantic aliased selection while materializing external aliases when their anchor is removed, requests `--show references` with end-to-end facet-omission proof, and enumerates configuration fields, live config, catalog/runtime production code, and template runtime inputs with spaced and camel-case falsifying mutations.
