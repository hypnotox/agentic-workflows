...
Add schema generation 40 after the integrated generation 39 in `internal/migrate/migrate.go` and add the exact generation-40 minimum-version entry in `internal/project/project.go`, pinned by `internal/project/version_test.go`. Implement an idempotent config-tree migration that removes exactly `reviewing-plan-resync` from the top-level `skills` sequence of a generation-39-stamped tree before catalog validation, reports the removal once, preserves sequence order and every unrelated byte/value allowed by the config editor, and leaves configs without the selection unchanged. Use the existing config editing owner rather than parsing YAML in the migration. Make `ConfigForCurrentSchema` remove the retired selection from historical config bytes before current catalog consumption, accounting for generation 39 removing the entire historical selection surface on older trees. Cover block and flow forms supported by the editor, sole-item/empty result, absence, repeated application, malformed input, change reporting from a generation-39 fixture, schema stamping, ordered registry behavior, minimum-version mapping, and unconditional forward-port behavior. Do not remove historical structural-heading or generation-39 migration entries needed by projects crossing older generations before generation 40.
...
- Final integration with configuration-surface collapse preserved main's assigned schema generations 38 and 39 and moved the independently allocated targeted resync retirement to generation 40. Generation 39 already removes the entire selection surface for older trees; generation 40 remains load-bearing for a generation-39-stamped tree that still carries the independently retired item. The integration also replaces selection-era CLI and fixture expectations with main's unconditional full-catalog authority while retaining the linked-plan freshness and no-resync contracts.
