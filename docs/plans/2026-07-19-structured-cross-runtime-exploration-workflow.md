...
- [ ] In the existing isolation test, call exploration with `{ task: "inspect", breadth: "bounded", detail: "summary" }` and assert the prompt contains each exact policy fragment: `adaptive maximum`; definitions for targeted, bounded, and broad; `tracked files plus non-ignored untracked working-tree files`; ignored files, `.git`, nested repositories, and external dependency exclusions; `Not found within <breadth> boundary:`; not-found as successful execution; broad absence naming the project search universe and searched surfaces; `inconclusive`; `unverified`; `insufficient`; evidence grounding; one information need; `paths`, `summary`, and `analysis`; no edits, commits, or recursive delegation; and parent-driven sequential refinement through `a new fresh-context call` that may correct the task, change report detail, or widen breadth. Preserve assertions for root cwd, inherited model/thinking level, `EXPLORE_TOOLS`, partial-details isolation, final-only model content, failures, and absence of any `subagent_` child tool.
...
- [ ] Create `/home/hypno/Projects/agentic-workflows/templates/skills/exploring/SKILL.md.tmpl` with the catalog's seven section markers and these exact semantic paragraphs (headings/frontmatter may wrap, but meaning and quoted protocol strings must not change):
  ...
  - `results`: require material claims to have file/line evidence; require exact prefix `Not found within <breadth> boundary: <what was searched>` and define not-found as successful execution; require a broad absence report to name the project search universe and searched surfaces; distinguish inconclusive and unverified from absence; consume only relevant final findings; after not-found, inconclusive, unverified, or insufficient output, permit a new parent-driven fresh-context call that corrects the task, changes report detail, or widens breadth.
...
- [ ] Apply this authored-doc batch. The exhaustive affected-site set is the thirteen authored paths below; the representative and edge wording are exact, and the other sites use the identical conjunctive trigger/schema vocabulary where applicable:
  ...
  - `/home/hypno/Projects/agentic-workflows/templates/docs/working-with-awf.md.tmpl`: replace every `{task}`-only exploration call with required `{task, breadth, detail}`; state adaptive maximum, independent detail, not-found as successful execution, broad absence naming the project universe and searched surfaces, and a parent-driven new call after not-found, inconclusive, unverified, or insufficient output that may correct task, change detail, or widen breadth.
