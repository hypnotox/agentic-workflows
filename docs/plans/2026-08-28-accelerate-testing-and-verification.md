...
Representative: ["tooling/quality-gates:exact-revision-repository-acceptance", "tooling/changelog-and-release:release-gate-on-tag"]
...
Phase 5 distinguishes production imports from test-only imports so a test edge selects its importing package without falsely affecting that package's production consumers. Initial implementation allowed the `cmd/awf` exact-name composition suite to replace a full indirect reverse-dependent run after measurement showed that package exceeded the common budget. Review found no actual caller oracle proving that substitution complete, so repository assurance authority requires every production reverse dependent in full and treats declared meta-suites as additional repository-wide boundaries. Direct owner changes still run the complete owner package, suites run only when their owner package is not already selected, and shared or uncertain evidence still widens to complete Go behavior. The qualified common workload remains the representative `internal/testselection` owner change and does not weaken caller behavior to meet its budget. Settlement verification also exposed that long per-worker temporary roots broke existing Unix-socket fixtures and that the performance reporter test had not incorporated the already-landed common-feedback observations. The executor now retains isolation under a short Unix root with a red-then-green socket regression, and the reporter asserts all three qualified workload aggregates. The bounded verify pass then found that merging a declared suite into an already-selected full owner package discarded the suite's required-unit evidence. Selection now carries those exact required names onto the full package target, which runs once under JSON evidence and refuses if any declared unit is absent; a red-then-green direct-owner regression proves the correction.

The first hosted qualification run for candidate `61aa6f56cebfdb79a1ae73b4375248f13f5cff80` was intentionally triggered through temporary draft PR 17 after the corrected pre-push policy allowed publication. Linux coverage and policy, Pi, analysis, mutation, and release-config passed, but every macOS native shard failed before execution because the runner used the Bash 4-only `mapfile` builtin while GitHub's macOS image supplies Bash 3.2. A red-then-green command-runner regression disables `mapfile` through `BASH_ENV`, and the shared shard setup now populates its cache array with the Bash 3-compatible read loop. The failed run is diagnostic rather than qualification evidence; a new immutable candidate must pass exact-SHA hosted assurance before terminal closure.

The second exact-SHA run, `33213652746`, exercised the repaired cache setup at candidate `61574c83e3a1bd4f2cecf6f74107eeb04a27f2e1`, then every macOS shard failed because Bash 3.2 under nounset rejects expansion of the intentionally empty optional test and coverage argument arrays. The retained runner regression now requires nounset-safe optional array expansion while preserving the qualified shard commands. Linux coverage shard `0-2` separately encountered a proxy.golang.org HTTP/2 module-download error while its peer Linux shards and all non-native lanes passed; treat that isolated network failure as diagnostic unless it recurs. This run is not qualification evidence, so terminal closure still requires a green immutable candidate.

The third exact-SHA run, `33215287605`, reached native test execution at candidate `2ac0a04e363b9e15d4458cb4d4ad2d2459671a3d`; every Linux coverage shard, coverage policy, Pi, analysis, mutation, and release configuration passed, while every macOS shard failed when isolated test roots inherited macOS's symlinked `/tmp`. Repository-boundary and hard-safety checks correctly rejected either the lexical `/tmp` ancestor or disagreement with Go's canonical `/private/tmp` package directories. A red-then-green runner fixture now supplies a symlinked temporary parent and requires each shard to canonicalize its private root before exporting `HOME`, `TMPDIR`, and `GOTMPDIR`. This run is diagnostic rather than qualification evidence.

The fourth exact-SHA run, `33216575625`, exercised candidate `03303d2371e423865371cd7ac14d75c551fa4649`. Every Linux coverage shard, coverage policy, Pi, analysis, mutation, and release configuration passed. Five macOS native workloads passed and the remaining workloads exposed independent fixture portability defects: rendered hooks still used Bash 4-only collection and associative arrays, coverage aggregation used the same constructs, the covercheck mutation census combined `mapfile` with GNU `find -printf`, two Unix-socket fixtures exceeded Darwin's path budget, one Git path fixture used a byte illegal in macOS filenames, and one fake Node worker used GNU `sed -i` semantics. Red-then-green or directly reproducing regressions now exercise each portable replacement under the system-shell constraints. This run is diagnostic rather than qualification evidence.

The fifth exact-SHA run, `33219469288`, qualified candidate
`f6f3eceea6c99d383fe3ddce7221c26ef68f05dd`. Every Linux coverage shard, native macOS shard,
coverage-policy, Pi, analysis, mutation, release-config, and stable gate job passed against that
revision. The run started at `2026-08-28T23:09:55Z` and the gate completed at
`2026-08-28T23:14:29Z`, a 274-second hosted critical path. The 60-second stronger hosted target was
not achieved; the slowest native shards remained the critical path. The 56-second release-config
lane and its exact snapshot validation passed. The SHA-keyed run and coverage artifacts remain at
`https://github.com/hypnotox/agentic-workflows/actions/runs/33219469288`.

### Terminal reconciliation
Implementation range: d53a6c172ce177c8461bb82be7032101c217657f..f6f3eceea6c99d383fe3ddce7221c26ef68f05dd
Touched paths:
- ".awf/awf.lock"
...
- ".awf/hooks/reference-transaction.sh"
...
- ".awf/topics/parts/tooling/test-infrastructure/current-state.md"
...
- "cmd/awf/commitgate_test.go"
- "cmd/awf/config_test.go"
- "cmd/awf/context_test.go"
- "cmd/awf/dispatch.go"
- "cmd/awf/dispatch_test.go"
- "cmd/awf/effort_test.go"
- "cmd/awf/effort_worktree_test.go"
- "cmd/awf/failure_paths_test.go"
- "cmd/awf/fixture_seed_test.go"
- "cmd/awf/gate_test.go"
- "cmd/awf/global_help_test.go"
- "cmd/awf/init.go"
- "cmd/awf/init_test.go"
- "cmd/awf/initrender_test.go"
- "cmd/awf/lease_diagnostic_lifetime_test.go"
- "cmd/awf/list_add_test.go"
- "cmd/awf/main.go"
- "cmd/awf/main_test.go"
- "cmd/awf/memorygate_test.go"
- "cmd/awf/new_test.go"
- "cmd/awf/prosegate_test.go"
- "cmd/awf/publishing_test.go"
- "cmd/awf/test_helpers_test.go"
- "cmd/awf/topic_test.go"
- "cmd/awf/uninstall_test.go"
- "cmd/awf/version_test.go"
- "cmd/covercheck/main.go"
- "cmd/covercheck/main_test.go"
- "cmd/releasecheck/main.go"
- "cmd/releasecheck/main_test.go"
- "cmd/testperformance/main.go"
- "cmd/testperformance/main_test.go"
- "cmd/testselection/main.go"
- "cmd/testselection/main_test.go"
- "coverage-baseline.json"
- "coverage-review.json"
- "docs/architecture.md"
- "docs/config-reference.md"
- "docs/decisions/0314-performance-budgeted-parallel-verification.md"
- "docs/decisions/0315-validate-only-pushed-ref-commit-deltas.md"
- "docs/decisions/INDEX.md"
- "docs/development.md"
- "docs/plans/2026-08-28-accelerate-testing-and-verification.md"
...
- "internal/evals/adr_scope_test.go"
- "internal/evals/chain_test.go"
- "internal/evals/fixture_test.go"
...
- "internal/git/entrypoints_test.go"
- "internal/git/terminal_paths_test.go"
- "internal/git/working_changes.go"
- "internal/git/working_changes_test.go"
- "internal/project/effort_workflow_template_test.go"
- "internal/project/fixture_seed_test.go"
- "internal/project/gate_runner_test.go"
- "internal/project/hooks_test.go"
- "internal/project/project_test_helpers_test.go"
- "internal/project/remote_policy_docs_test.go"
- "internal/project/repository_awf_invocation_test.go"
- "internal/project/repository_wiring_test.go"
- "internal/project/tree_reader_unix_test.go"
- "internal/publisher/confighash.go"
- "internal/publisher/confighash_test.go"
- "internal/publisher/fixture_seed_test.go"
- "internal/publisher/output_plan_unix_test.go"
- "internal/publisher/render.go"
- "internal/publisher/render_test.go"
- "internal/publisher/sync_fault_test.go"
- "internal/publisher/test_helpers_test.go"
- "internal/render/vars.go"
- "internal/render/vars_test.go"
- "internal/testperformance/testperformance.go"
- "internal/testperformance/testperformance_test.go"
- "internal/testselection/selection.go"
- "internal/testselection/selection_test.go"
- "internal/testsupport/exported_test_calls_test.go"
- "internal/testsupport/shorttemp.go"
- "internal/testsupport/testsupport.go"
- "internal/testsupport/testtemp.go"
- "internal/testsupport/thin_command_composition_test.go"
- "internal/testsupport/treeseed.go"
- "internal/testsupport/treeseed_test.go"
- "templates/hooks/pre-push.sh.tmpl"
- "templates/hooks/reference-transaction.sh.tmpl"
- "templates/partials/gate-cadence.md"
- "test-performance.json"
- "test-selection.json"
- "x"
Material deviations:
- The compact coverage candidates did not reproduce every required identity, so the implementation retained deterministic whole-module text shards and canonical merging.
- The common-feedback and mandatory 4x ordinary-full targets passed, while the 55-second local-full and 60-second hosted stronger targets remained unmet and are reported without threshold changes.
- The governed first push exposed target-wide commit-policy rejection of already-hosted history, so ADR-0315 and its compact plan repaired pushed-ref delta selection before hosted qualification could continue.
- Four diagnostic exact-SHA runs exposed macOS Bash, temporary-root, hook, and fixture portability defects; each repair landed with focused regression evidence before the final exact-SHA run qualified.
