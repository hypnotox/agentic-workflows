...
```bash
unexpected="$(git diff --cached --name-only HEAD -- 'docs/decisions/*.md' 'docs/plans/*.md' | grep -vE '^(docs/decisions/INDEX\.md|docs/decisions/introduce-core-and-full-workflow-profiles\.md|docs/plans/2026-08-14-core-and-full-workflow-profiles\.md)$' || true)"
test -z "$unexpected"
...
- Phase 2 renewed-review settlement: residual global singleton classification, narrow Data cloning, self-referential alias expectations, and whole-file bypass exemptions remained. The follow-up parameterizes singleton classification by the selected catalog, deep-clones arbitrary map/slice/array/pointer Data shapes, asserts independent mutation outcomes and complete-catalog equality, and uses an AST census with exact function-level composition exemptions and falsification probes.
- Phase 3 execution deviation: the historical-leaf snapshot excludes generated `docs/decisions/INDEX.md` in addition to the linked ADR and plan because the required Implementing transition necessarily regenerates that structural index; authored historical ADR and plan leaves remain the preservation population.
- Phase 2 final-review settlement: `View.Catalog` now returns defensive snapshots and each opened Project retains one private snapshot, with cross-project and returned-snapshot mutation regressions. The AST census resolves import aliases, rejects dot imports, and scopes exemptions to free-function declaration identity, every kind uses one catalog-aware template identity resolver without a dummy coverage exclusion, singleton documentation now matches Path-based membership, and injected template-read errors are covered through config-reference rows. Repo-local audit still advises that adopter-facing production paths lack a changelog entry; this phase intentionally changes no rendered output, CLI behavior, or config/lock schema, so the changelog's actual-effect contract forbids a false notable-change entry.
- Phase 3 coverage settlement deviation: the activation added focused profile validation, initialization, migration, capability-refusal, and scaffold tests, including the previously omitted `internal/project/gatedcommands_test.go`; it restored specific coverage-ignore rationales on defensive branches whose inputs are already validated or whose generated paths are unique, removed unused profile accessors, and eliminated a wasted check-report initialization. These changes preserve the approved behavior and restore the 100% coverage and dead-code gates.
- Phase 3 review settlement: review found retained Core artifacts still exposed Full authority, profile hashing relied indirectly on layout, direct config validation accepted an absent profile, the exported staged-drift seam could reproject an already filtered catalog, several proof markers were nominal, and transition coverage was incomplete. The settlement makes every retained expanded Core artifact operationally governance-free while preserving Full prose, hashes the explicit profile, makes current config validation strict while historical pre-profile audit snapshots project to Full, derives staged views from the complete catalog, binds markers to focused profile tests, and adds Core/Full command, staged, pruning, empty-ancestor, malformed-history, and restoration regressions for both targets. Added paths include `internal/project/profile_semantics_test.go` plus focused config, audit, catalog, topic, command, hash, and staged-drift tests and the additional retained templates surfaced by exhaustive rendering.
- Phase 3 verify settlement: Core's Pi review schema now exposes only code review; checkpoint, reviewer, and workflow prose renders Core-neutral authority while Full retains governance wording; closure scans bind executable references and exact leaked instructions; staged profile selection retains the injected complete catalog; historical Full projection is pinned to pre-profile schemas; every declared Full-only command family is dispatch-tested under both profiles; and Full -> Core -> Full lock membership is compared across the complete transition.
- Phase 3 renewed-review settlement: the reported closure finding was authority-preserving rather than a user decision because the approved Core boundary already forbids Full concepts. The residual settlement removes stale-ADR behavior and prose from Core's commit gate and retained hooks/docs, renders Core-specific reviewer focus and read instructions instead of Full catalog data, mutation-proves the closure scanner across retained artifact classes, pins the command matrix exactly to the FullOnly specification with handler-specific outcomes, and compares transitioned Core lock membership to an independently rendered clean-Core set.
- Terminal implementation-review settlement: fresh Core rendering exposed remaining Full vocabulary in shared docs, retained operational skills, Pi preference prose, static command help, and unprojected reviewer data; historical audit also synthesized Full for invalid schema-46 snapshots, while the closure oracle was too narrow. The settlement projects Core reviewer metadata at the catalog boundary, gates or neutralizes every cited shared surface, limits historical migration to schemas before the profile generation, makes command help profile-neutral, and scans the broader governance vocabulary over every actual Core output with representative real-output mutations.
- Terminal verify settlement: Core bootstrap and retrospective prose are profile-aware; the Core config reference omits Full-only rows and derives reviewer focus defaults from the selected catalog. The closure proof scans the complete fresh-init managed output set, including generated and bootstrap outputs, and independently mutates every representative output class. The complete-range audits remain clean except for the existing coverage-ignore advisories.
