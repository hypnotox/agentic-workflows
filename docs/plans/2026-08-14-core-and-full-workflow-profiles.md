...
- Phase 2 final-review settlement: `View.Catalog` now returns defensive snapshots and each opened Project retains one private snapshot, with cross-project and returned-snapshot mutation regressions. The AST census resolves import aliases and rejects dot imports, every kind uses one catalog-aware template identity resolver without a dummy coverage exclusion, and singleton documentation now matches Path-based membership.
