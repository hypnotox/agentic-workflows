...
Phase 5 application: a red-first plan-v2 fixture proved that canonical `0001:#1` still parsed and that malformed `0001:#01` lacked stable-slug guidance. Plan-v2 now accepts only stable lowercase-kebab Decision selectors and gives both ordinal forms explicit stable-slug guidance. Project validation no longer treats hand-built ordinal references as plan-v2 input, while ADR-owned pre-V4 enumeration and navigation tests remain unchanged. Applied the three declared current-state updates and successor-ADR event; no implementation deviation was required. Focused and full tests, render meaning review, drift generation, and dead-code analysis pass; canonical coverage remains 96.9% raw and 99.8% filtered with only the expected production-universe hash update.

Phase 5 review found two mechanical proof gaps. Settlement exercises ordinal rejection through both Applying and Context and backs the stable-V4-only vocabulary across the writing skill, reviewing skill, plans README, and plan template in source and rendered outputs. Temporary mutations that restored ordinal parsing or ordinal guidance independently made the owning tests fail for the parser and each guidance surface class.

Task 6.1 candidate verification used exact binary SHA-256 `42dba8f71b4a49a20398d99b44973d83d49bae5e7cb1a35d98ae39cb500e0f85`. Disposable copies of the recorded primary tips passed candidate upgrade, staged and repository checks, and each repository-required gate: aeonseed `3cc849280cd9`, agentic-workflows `eaafe14d0b79`, fleet `afaef79d5d30`, go-php `7e7649dd66d2`, jugend-im-zentrum `d41b0544fa30`, nouris `2468d0de5996`, pi-science `ab434da61ad4`, pi-tools `489e6e335f86`, remote_pi `141e782396cc`, and sudoku-solver `17435bd144f2`. Agentic-workflows ran its implementation-aware gate at candidate tip `432419c349a3`. Pre-existing build-tree scale, timeout, wrapper, tmpfs, and unpinned-container-linter constraints were confined to disposable infrastructure and did not change source or weaken a repository gate. All managed sources ended clean at their unchanged tips; every disposable clone and external build tree was removed.

Task 6.2 and precommit Task 6.3 assurance passed `go test ./cmd/awf ./cmd/releasecheck ./cmd/versioncheck ./cmd/pincheck ./internal/audit ./internal/migrate ./internal/upgrade ./internal/effort ./internal/plan ./internal/plancheck`, `go test ./...`, `./x render`, `./x check`, `./x deadcode`, `go run ./cmd/versioncheck`, `go run ./cmd/pincheck`, `./awf check staged`, and `./x gate`. Rendered release, upgrade, audit, effort, and plan boundaries meaning-review as one live schema-46 floor, one audit-only schemas-3-through-46 horizon, canonical current formats, and retained generic journal recovery. The gate's documentation-only profile skipped redundant Go and Pi suites after the explicit full test run and retained only pre-existing advisory lint findings.

Postcommit Task 6.3 assurance ran `./awf audit eaafe14d0..ca30a89f1` and `./x audit-local eaafe14d0..ca30a89f1`. Workflow audit was clean across all 26 commits. Local audit returned no errors; its 81 warnings enumerate two re-evaluated journal coverage directives and exact independently reviewed raw-coverage identities already accepted by the phase gates and reviews. Range and path inspection found no RF-014B, RF-010, `project.Open`, punctuation normalization, initialization-provenance normalization, represented-parser deletion, generic-journal deletion, or historical-record cleanup outside the protected RF-008B boundary.
