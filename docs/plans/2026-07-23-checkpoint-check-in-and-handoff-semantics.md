...
- Task 2.1 execution (2026-07-23): the recovery notify and editor write are wrapped in a swallowing try/catch, deviating from the task's unguarded pseudocode: the real-Pi container lane found an unguarded `ctx.ui.notify` throwing on the disposed old-session UI after a post-teardown replacement failure, masking the original error the extension must report. Review then reordered the guarded block to write the editor before the notice so the notice is only shown once the recovery text exists.
