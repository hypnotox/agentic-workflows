...
- [ ] **Task 0.1: Verify ADR-0149's shared-claim updates are Applied.** Run `grep -n 'Applied\|^status:' docs/decisions/0149-deterministic-effort-lifecycle-accounting-and-pi-workflow-routing.md`. Proceed only when, for each of `rendering/workflow-skill-templates:memory-checkpoint-chain-coverage`, `rendering/pi-workflows:pi-session-handoff-workflow`, and `rendering/pi-workflows:pi-session-handoff-lifecycle`, either ADR-0149 declares no operation on that claim or its Applied events cover the operation (a `status: Implemented` line satisfies all three at once; an Abandoned ADR-0149 cancels its Remaining operations and also unblocks). If any relevant operation is still Remaining, stop: ADR-0151 Decision item 9 forbids proceeding.
- [ ] **Task 0.2: Rebase and confirm a clean baseline.** Rebase this branch onto current `main` (local `main` when it is ahead of `origin/main`). Never resolve a conflict by hand-editing a rendered file; change the config-tree source and regenerate with `./x sync`. Acceptance: `./x check` reports clean and `./x gate` exits 0 on the rebased branch before any Phase 1 edit.
...
- [ ] **Task 1.1: Create `templates/partials/checkpoint-routine.md`** with exactly this content (`targetSessionHandoff` is the existing var already used by `templates/partials/memory-checkpoint.md`; preserve its spelling):
...
- [ ] **Task 1.2: Create `templates/partials/checkpoint-approval.md`** with exactly this content:
...
- [ ] **Task 1.8: Author the claim mutations for batch 1.**
  ...
  - Same file: replace the `memory-checkpoint-chain-coverage` claim body with:
    ...
    ```
    Every checkpoint-carrying skill renders the routine protocol: memory persistence first, then the attention classification, then either a raised check-in that stops or a continuity notice that continues without ending the turn. The implementation skills embed the complete routine protocol at their per-task sections, retrospective alone carries the memory-file deletion step, and a non-trivial brainstormed effort becomes memory-backed when its first settled decision is persisted while small checkpoint-less efforts stay memory-free.
    ```
...
- [ ] **Task 2.1: Add the post-queue recovery path to `templates/pi/awf-handoff/index.ts.tmpl`** in the `awf-handoff-continue` handler (pseudocode; the user-facing strings are exact contracts):
  - Move `const wrapper = buildKickoffWrapper(request.memoryPath, request.kickoff);` above the post-countdown `await validateMemoryPath(...)` call (`buildKickoffWrapper` is pure), so recovery content exists on every post-queue failure branch.
