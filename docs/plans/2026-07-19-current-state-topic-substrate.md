...
- **Modified:** `internal/config/{config,config_test}.go`, `internal/configspec/{spec,spec_test}.go`,
  ...
  `templates/docs/working-with-awf.md.tmpl`, `README.md`, and `changelog/CHANGELOG.md`.
...
- [ ] **Task 1.3: Add the bridge schema generation.** In `internal/migrate/migrate.go`, append
  generation 14 named `current-state-topic-substrate` with a no-op apply function: this generation
  recognizes prepared topic inputs but performs no semantic conversion. Update the current-generation
  pin and registry coverage in `internal/migrate/migrate_test.go`. In `internal/project/project.go`,
  set `Version` to `0.18.0` and add `14: "0.18.0"` to `minVersionBySchema`; update the exact pins in
...
- [ ] **Task 1.5: Upgrade both adopted fixtures, regenerate, and commit.** Build the source binary
  once, use it to run `upgrade` at the repository root and in `examples/sundial`, then sync so config
  reference and version-pinned outputs settle:
  ...
  Expected: both locks report schema 14 and awf 0.18.0, `./x check` is drift-free, and the gate
  reports 100% coverage, no production dead code, and clean prose. Stage only the Phase 1 Go sources,
  tests, generated config references/version outputs, and both locks reported by `git diff --name-only`;
  commit:
...
- [ ] **Task 2.2: Build one validated corpus and marker index.** Create
  `internal/topic/corpus.go` with `LoadCorpus`, `All`, `ByTopicID`, `ByClaimID`, and `ForDomain` plus
  deterministic incoming/outgoing direct-reference indexes. Discover metadata and parts from the two
  fixed trees, require a one-to-one pair, require the owning domain in `Config.Domains`, allow the
  same local slug in different topics, and reject duplicate full IDs, dangling/self references, and
  invalid Origin/Revised-by ADRs. Resolve provenance from the injected `adr.Corpus`; require every
  cited ADR to exist and be Implemented, and preserve Revised-by order without duplicates. Reference
  cycles remain legal and are never expanded.

  Create `internal/topic/markers.go` to scan only configured `currentState.sources` and parse a marker
...
- [ ] **Task 2.3: Add focused parser/corpus tests.** Create
...
- [ ] **Task 2.4: Add topic templates and deterministic render models.** Create
...
- [ ] **Task 2.5: Integrate the producer with project loading and output planning.** Create
...
- [ ] **Task 2.6: Prove the complete render lifecycle.** Create
...
- [ ] **Task 2.7: Document the bridge producer, sync, verify, and commit.** Update the authored
  ...
  preparation artifacts, that legacy ADR and invariant authority still governs ordinary commands,
  and that topic outputs participate in the output plan/lock/prune/drift model. Update
  `templates/docs/working-with-awf.md.tmpl` with the two input paths, strict metadata and claim grammar,
  output paths, and the bridge limitation. Run `./x sync`, `./x check`, and `./x gate`; expected:
...
- [ ] **Task 3.3: Test scaffold and command behavior.** Create
  `internal/topic/scaffold_test.go`; extend `internal/clispec/clispec_test.go` and
  `cmd/awf/new_test.go` for success, exact two-file content, no invented claims, no sync/lock/config
  mutation, missing arguments, unknown/noncanonical domain, slug collision suffixing, reserved index,
  either-side orphan refusal, rollback after the second write fails, version gating, help/dispatch
  parity, and the unknown-kind diagnostic. Run:
...
- [ ] **Task 3.4: Document, sync, verify, and commit.** Update the tooling current-state authored
  part and `templates/docs/working-with-awf.md.tmpl` with the exact command, two outputs, no-sync
  behavior, and manual-authoring requirement. Run `./x sync`, `./x check`, and `./x gate`; expected:
...
- [ ] **Task 4.2: Add the version-gated static-fallback CLI.** In
  ...
  must have no writer dependency and must not change the worktree, index, config, or lock.
...
- [ ] **Task 4.3: Test every projection and read-only boundary.** Create
  ...
  digests proving read-only behavior. Extend clispec/dispatch/gated-command pinned tests rather than
  creating a second command list. Run:
...
- [ ] **Task 4.4: Document, sync, verify, and commit.** Update the tooling current-state authored
  part and working-with-awf template with selector grammar, default fields, independent detail flags,
  JSON behavior, and the active-only bridge limitation. Run `./x sync`, `./x check`, and `./x gate`;
  expected: no drift, 100% coverage, no production dead code, and clean prose. Stage only Phase 4
  paths and generated fan-out; commit:

  ```commit
  feat(tooling): query current-state topics
  ```

## Phase 5: Publish the substrate boundary and freeze this plan

- [ ] **Task 5.1: Update adopter-facing release surfaces.** In `README.md`, add the topic input/output
  tree, `awf new topic`, and `awf topic` to the feature/command sections, with a bridge warning that
  normal context and invariant authority have not switched. In `changelog/CHANGELOG.md` under
  Unreleased, record schema 14, optional `currentState`, topic rendering/indexes, and both CLI
  additions; explicitly defer migration and authority cutover. Do not claim that ACTIVE.md,
  Superseded, legacy invariants, or ADR-derived context has been removed.

- [ ] **Task 5.2: Run whole-effort acceptance checks.** Run:
  ...
  diff check emits no output. In a temporary adopted fixture, scaffold a topic, manually complete one
  rule and one invariant, sync it, query it in human and JSON forms, remove it, sync again, and confirm
  its document/index/lock entries are pruned while legacy `awf context` and `awf invariants` output is
  unchanged.

- [ ] **Task 5.3: Freeze only this plan and commit.** Record actual implementation findings under
  ...
  commitments. Run `./x sync`, `./x check`, and `./x gate`; stage exactly this plan, README,
  changelog, and any sync-generated files changed by those authored inputs; commit:
