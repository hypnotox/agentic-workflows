...
`internal/effort` remains the safe-resident and activity-policy owner; `cmd/awf` exposes only the three JSON protocol operations. The generated client is the strict protocol-v2 transport boundary, while the generated Pi index owns one serialized process-local association, fixed-root directory inspection, transient context injection, heartbeat and shutdown cleanup, and optional Remote Pi translation. Authoring inputs and their rendered outputs land with the behavior they describe. One coherent implementation-with-docs transaction changes the binary and every generated consumer together before terminal implementation review; only after that review settles does one inline direct transaction apply all eight current-state updates and co-freeze the ADR and this plan.

## Phase 1: Replace checkout-aware activity and runtime

**Execution mode: subagent-driven.**

Advances: ["repository-green"]
Completes: ["protocol-v2", "direct-pi-association", "rendered-doc-currency"]
...
Edge: "Retain ordinary Git-authoritative managed-worktree operations, memory metadata behavior, and the statement that activity is advisory rather than authority or a lock; describe the direct Pi runtime only in the same transaction that lands its generated implementation and tests."
...
### Task 1.4: Strictly decode and invoke protocol v2 in the generated client
Latitude: exact
...
### Task 1.5: Replace runtime transfer with one direct serialized association
Latitude: exact
...
Post-check: "Run ./x render && ./x check && go test ./internal/project ./internal/evals && ./x pi-test run && git diff --check; require success, compare git diff --name-only with the union of this phase's Paths fields, and stop for plan correction on any unexplained generated path."
...
Run `gofmt` on every changed Go file, then run exactly `go test ./internal/effort ./cmd/awf ./internal/clispec ./internal/worktree ./internal/project ./internal/evals`, `./x render`, `./x pi-test run`, `./x check`, and `./x gate`; each command must exit zero and strict TypeScript coverage must remain at the existing floor. Run `if rg -n 'EffortTransferCoordinator|changeCwd|queueCommand|changedCwd|receivingCheckout|CheckoutRole|ctx\\.ui|activity (resolve|checkout)' templates/pi/awf-effort .pi/extensions/awf-effort internal/effort cmd/awf/effort.go internal/clispec/clispec.go; then exit 1; fi`; it must exit zero with no match. Stage the complete protocol, generated client/runtime, tests, authoring inputs, changelog, locks, and rendered outputs explicitly. Require `awf check staged` to report clean and create one commit.

```commit
refactor(awf): make Pi effort association CWD-neutral
```

## Phase 2: Apply all claims and freeze the reviewed change

**Execution mode: inline.**

Completes: ["deferred-implemented-application", "repository-green"]

### Task 2.1: Directly apply the eight declared claim updates
Kind: batch
Latitude: exact
...
Post-check: "Before Task 2.2, inspect the five claim-source diffs against the ADR and run ./awf context --show invariants --show evidence --show pending on the ADR plus all five claim-source paths, consuming any AWF_CONTEXT_SPILL_V1 packet; require exactly the eight intended claim mutations, every proof marker resolved to its named live test, no unrelated claim mutation, and all eight operation rows in observable pre-event state [proposed, active-current] with none applied or canceled."

Start this phase only after Phase 1 is committed, independent terminal implementation review has settled with no unresolved finding, every review fix is committed and re-reviewed as required, `git status --short` is empty, and `./x check` plus `./x gate` pass. This phase contains no production behavior, tests, adopter behavior docs, or changelog changes.
...
### Task 2.2: Record the direct Applied and Implemented transition and freeze the plan
Latitude: exact
Applying: ["simplify-pi-effort-association-around-fixed-repository-paths:compatibility-and-verification"]
