...
- Independent implementation assurance found three mechanical proof and prose gaps. The settlement narrows README's banner-free boundary to `local: true`, proves retained `awf:edit` and banner-free authored ADR/plan/local artifacts in the family matrix, and pins bridge machine inputs to config plus template with no `AGENTS.md` dependency.
