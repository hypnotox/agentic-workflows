...
Implement ADR-0181 (single-home ownership) and ADR-0193 (git access through one semantic
...
- **Modified:** `internal/git/controlroot.go`, `internal/git/git.go`,
  ...
  `docs/decisions/0193-*.md`, this plan, and every rendered output of the above.
...
- [x] **Task 1.1b: Renumber the git-seam ADR (unconditional).** Main has consumed the
  ...
  frontmatter, and every `ADR-0193` literal in this plan's Goal, Architecture summary,
...
- [x] **Task 7.1: Repo-walking proofs and the entrypoint table.**
  ...
  tables together are the proof carrier for `fixture-isolation-parity` (ADR-0193 item 11),
...
- [x] **Task 7.3: Apply ADR-0193's eleven operations in one batch.** In
  `.awf/topics/parts/tooling/git-access/current-state.md`: author the seven new claims per
  ADR-0193 item 12 (five `Backing: test` with proof markers placed on the seam walker,
  ...
  (`Origin: ADR-0193`, prose preserving ADR-0127 by reference, existing test backing).
...
- Implementation-surfaced (Phase 5), six deviations: `effort.Open` and `worktree.Open`
  ...
  `Dependencies` carries one optional member, the durability fault hook (see the ADR-0193
...
- User decisions (2026-07-31, after the Phase 6 review): the fixture lane's duplicate of
  ...
  noticing it, so ADR-0193 is amended with a `fixture-isolation-parity` operation (its
...
- Phase 7 SPLIT, and the plan's Task 7.5 is superseded on ordering. The plan places the
  ...
  back (the same corner ADR-0187 was forced into). Because ADR-0193's operations are
...
- Terminal review settlement (2026-07-31). The review returned "do not freeze yet" with three
  ...
  table, mutation-verified pin by pin, and ADR-0193 item 11 now states what two tables prove
...
- Final transaction (2026-07-31): both ADRs applied and flipped in one commit, ADR-0181 at
  state-sequence 108 and ADR-0193 at 109. The claims could not land before the flip and the
