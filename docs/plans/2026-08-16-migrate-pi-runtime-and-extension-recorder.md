...
Post-review settlement: nine findings corrected the discarded recorder fixtures: the profile adapter and effort harnesses now compose recorder-owned events, hooks, commands, UI, model registry, active tools, execution, tools, and contexts; only malformed active-tool capability coverage and awf filesystem, preference, effort, UUID, queue, and transport policy seams remain local. The index suite now installs the adapter through the recorder and uses its registry fixture. A deterministic structural assertion requires all three converted suites to import, create, and install the recorder, so removing or bypassing it fails the strict lane. Remaining generic effort micro-fixtures were replaced with recorder direct invocation and recorded tool state. README and authored development authority now name the exact `fork-v0.84.2.2` asset, and the Unreleased breaking entry records the raised effort floor. Semantic reinspection after rendering covered the root effort entrypoint, README prerequisite, development and testing documents, Pi runtime topic, and rendering domain; the floor, six-profile boundary, source-only recorder, unpinned adopter runtime, and retained real-SDK smoke were coherent with no contradictory fragment or meaning drift. The existing `runtime.test.ts` deviation remains unchanged.
