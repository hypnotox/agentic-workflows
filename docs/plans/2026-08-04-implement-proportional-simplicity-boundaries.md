...
Update `templates/agents/implementer.md.tmpl` to preserve the same material boundary in a no-user child. When an invalidating source fact requires user approval, the implementer stops before any further mutation and reports the changed fact, why the approved approach no longer fits, affected approved categories, and simplest viable options. Adjust the closed stopped outcome so it accepts either a named failing check with actual output or that complete approval-requiring invalidating-source report; retain status, completed work, remaining work, and attempts in both cases. Do not create a third outcome or require a fabricated failing check.
...
- Add the `rendering/workflow-skill-templates:mandatory-approval-boundaries (TestMaintainableCodeStageCoverage)` proof marker to `TestMaintainableCodeStageCoverage`; extend that test's per-skill expectations for the approved contract, plan-selection rule, all four implementation paths, TDD restraint, and the material-deviation categories. Assert the complete escalation payload - changed fact, why the approved approach no longer fits, affected categories, and simplest viable options - plus stopping before further mutation and equivalent mechanical autonomy wherever those implementation paths render it.
- Extend `TestImplementerAgent` for the approval-requiring stopped shape, the complete four-part escalation payload, and the stop-before-further-mutation boundary; confirm no failing check is required for that alternative.
...
- `mandatory-approval-boundaries`: add the proportionate brainstorming contract and renewed user approval for deviations in behavior, scope, structure, dependencies, patterns, checks, or testing strategy. Before further mutation, escalation identifies the changed fact, explains why the approved approach no longer fits, names affected approved categories, and presents the simplest viable options; equivalent mechanical choices remain autonomous.
