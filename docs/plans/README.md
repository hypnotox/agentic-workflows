...
- Phases each declare **Execution mode: `inline`** or **`subagent-driven`** independently. A phase is
  ...
  steps, not default dispatch, review, checkpoint, or commit boundaries. Each names exact paths,
  relevant symbols, commands with expected terminal states, and exact content/diffs or
...
- Exact form remains mandatory for machine-consumed configuration and manifests, contract-bearing
  ...
  and batch-task representative and edge transformations. `TBD`, `implement later`, outcome-only
  summaries, and hidden design choices are placeholders, never pseudocode.
- A batch task retains an exact representative, exact edge unless identical everywhere, exhaustive
  affected-site set, and deterministic post-check. Optional helpers are sequential and commit-disabled:
  each site belongs to the parent or exactly one helper, helper subsets are path-disjoint, shared files
  remain parent-owned, and focused mutating commands stay confined to the assigned subset.
- Each phase declares one phase-closing commit, stages the complete transaction, runs `awf check --staged`
  and the gate, then commits. Definitions land with first production use; no dead-code exception exists.
