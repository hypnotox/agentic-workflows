...
Assert the two clauses the settled ADR requires but that no other assertion reaches. In all four skill outputs, assert that the shared partial names plan resync's governed return edge as the exception to the same-artifact no-loop rule. In the three reviewer-agent outputs, assert the ADR carve-out literal `is not an unauthorized deviation merely because its proposed future state differs from current state` and the reviewer-side new-authority trigger literal `would make a new load-bearing choice material outside approved durable boundaries`.
