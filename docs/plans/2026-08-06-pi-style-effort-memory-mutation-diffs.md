...
- Phase 2 deviations, all preserving the approved outcome. The shared `memoryDiff` validator now also rejects an empty diff carrying a line number and a nonempty diff carrying null, which tightens `edited` and `updated` too, under Task 2.1's single-validation-home directive. The `edited` branch reuses that validator instead of an inline duplicate, and `MemoryReply.diff` is typed `MemoryDiff`. The preview option is a trailing closed `MemoryPreviewOption` parameter on both mutation functions rather than a field inside `memoryUpdate`'s arguments, keeping the two operations symmetric and the tool's closed argument schema separate from client invocation. `renderCall` does not start a preview once execution has started, because Pi redraws a settled row. The mutation row omits a placeholder background function that `buildMutationRow` always replaces before any render. `docs/decisions/INDEX.md` is a declared Task 2.3 path but is unchanged, since the ADR stays `Implementing`.
- Phase 2 review confirmed all seven reported deviations sound, but contested the verification behind one and found four further defects. The settlement gives the started-execution guard a distinct tool-call id so its assertion discriminates, strips the binary's trailing row terminator before `renderDiff` so no rendered diff carries a stray context line, re-cuts the renderer fixtures to the terminated shape the binary actually emits, replaces the empty-diff assertion that could never fail, bounds retained preview state a rendered-but-unexecuted call would otherwise strand, includes the working directory in preview identity, and keeps a failed asynchronous redraw inside its callback instead of escaping as an unhandled rejection that would end the session. The inert fourth `MutationStatus` value and its now-constant argument are gone, the test file initializes the theme it needs instead of depending on file ordering, and a duplicate empty-variable leak assertion is removed. Every one of these fixes was confirmed by mutation: removing any of them turns the suite red. The claim text is unchanged because none of these refinements contradict it and the transition carries no remaining declared operation; the proof pins were extended to cover them, and the changelog gained the adopter-facing rendering entry.
