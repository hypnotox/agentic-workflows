...
Post-check: "After `./x render`, `rg -n 'nonempty strict subset|at least one applied and one remaining|while another operation remains|corrections after the final Applied|final Applied batch.*Implemented|final batch and the Implemented flip|declaration-ordered operation-list|batches preserve declaration order' .awf/parts .awf/agents .awf/docs templates internal/catalog/standard.go .pi .claude AGENTS.md docs/decisions/README.md docs/decisions/template.md docs/domains docs/topics docs/working-with-awf.md docs/pitfalls.md docs/glossary.md examples/sundial/.pi examples/sundial/.claude examples/sundial/AGENTS.md examples/sundial/docs/decisions/README.md examples/sundial/docs/decisions/template.md examples/sundial/docs/domains examples/sundial/docs/topics examples/sundial/docs/working-with-awf.md examples/sundial/docs/pitfalls.md examples/sundial/docs/glossary.md` returns no output. Numbered historical ADRs and implementation plans are deliberately outside this live-guidance search and remain unchanged. `./x check` exits zero, and publication-safety tests emit no unresolved-value or no-value token."
...
ordered history. Update the root ADR-template convention part and its rendered skeleton with the same
grammar, and revise the application-batch glossary entry so it defines unordered operation membership
inside an ordered history occurrence. Correct all three stale lifecycle pitfalls: terminal adjacency,
the all-Applied fixture workaround, and the lack of a nonterminal all-Applied state. Strengthen catalog and project
...
outputs; run `gofmt -w internal/catalog/standard.go internal/catalog/catalog_test.go internal/project/golden_test.go internal/project/target_test.go internal/project/example_wiring_test.go internal/project/guide_scopes_test.go internal/project/spine_test.go`; run `go test ./internal/catalog ./internal/project`; then run `./x check` and `git diff --check`. Every command must exit zero, the index must list the ADR as Implementing, and `./awf context --show pending docs/decisions/decouple-adr-application-completion-from-terminal-review.md` must report exactly the three named operations Applied and only `merge-transition-ordered-aggregate` Remaining.
...
`./awf context --show pending docs/decisions/decouple-adr-application-completion-from-terminal-review.md` must show all operations Applied with none Remaining while the ADR remains in flight. The generated index must
