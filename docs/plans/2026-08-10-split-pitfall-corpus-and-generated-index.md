...
- Renewed Phase 1 review found six residual proof-driven gaps: YAML scalar coercion, multiline and fence-edge CommonMark masking, legacy list canonicalization, non-regular migration destinations, semantically empty section remainders, and an incomplete global single-home census. The second settlement corrected all six within the existing parser, migration, and structural-proof boundaries, kept the original claim prose and Applied event unchanged, and introduced no design deviation.
- Phase 2 review found four authority-determined gaps: scaffold publication duplicated the exclusive publisher, project paths could escape through parent symlinks, and both CLI claims had incomplete cross-layer backing. The settlement routed directory and complete-file publication through cohesive root-confined `internal/filesystem` and `internal/filepublication` capabilities, removed the duplicate writer, and distributed mutation-confirmed proof markers across spec, gate, dispatch, census, publication, confinement, race, and presentation tests. The added filesystem and publication paths are a reasoned deviation necessary to satisfy the approved exclusive-creation outcome under the existing root-confinement and single-publication-home claims; no durable design changed.
- Renewed Phase 2 review found reachable publication branches hidden by coverage annotations, duplicated confined and unconfined publication algorithms, incomplete production dispatch backing for the pre-existing creation inventory, and a tooling-scoped settlement subject that contradicted code-design classification authority. The corrected settlement uses one adapter-backed publication state machine, directly proves collision exhaustion and cleanup errors, exercises every authored-kind dispatch, and is classified `refactor(code-design)`; no claim or Applied event changed.
- Terminal cross-phase assurance found six authority-determined gaps: generation-43 operations were not root-confined, sidecar removal failure was falsely coverage-excluded, parser identity accepted noncanonical injected paths, generated metadata lacked context-specific escaping, committed publication cleanup failure was ambiguous, and new migration errors lacked consistent context and identity checks. Although four findings were initially labeled user decisions, D2-D5/D7 and the active confinement, publication, and pitfall claims already determine their correction. The terminal settlement root-confines migration, proves removal failure and retry, rejects noncanonical identity, escapes metadata through the shared pitfall model, models committed cleanup residue as an actionable outcome, and preserves wrapped filesystem identities without changing durable authority.
- The single terminal verify pass confirmed all six corrections and found one mechanical proof gap: the root-confined handle invariant did not directly exercise new Replace and Remove operations through an escaping parent or assert their wrapped error identities. The final focused test correction covers both operations, preserves an external sentinel, and asserts `errors.Is` for underlying absence failures.
