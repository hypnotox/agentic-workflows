...
- capabilities are requested with `remote-pi:capabilities:request` and returned as one complete authoritative `remote-pi:capabilities` singleton-provider snapshot whose optional `displaySuffix` member is exactly `{version:1,namespaces:["awf"]}` for support;
- suffix state is set with `remote-pi:display-suffix:set` and exactly `{namespace:"awf",value:<canonical effort slug>}`; explicit clear uses the same event with exactly `{namespace:"awf",value:null}`;
- producer replay is requested with `remote-pi:display-suffix:request` and exactly `{namespace:"awf"}`;
- the `awf` value is 1 through 63 printable ASCII bytes matching `[a-z0-9]+(?:-[a-z0-9]+)*`; Remote Pi composes it only for human-facing presentation as `<assigned base> - <suffix>` and never feeds the composite into configured name, broker/mesh address, cwd lock, peer self-filtering, relay room identity, or routing;
- a capabilities snapshot with absent, malformed, unsupported-version, or namespace-missing `displaySuffix` withdraws support and clears presentation; no response leaves the consumer initially unsupported; missing listeners, malformed payloads, and thrown event handlers remain advisory;
- factory load and awf `session_start` both request capabilities, Remote Pi `session_start` synchronously requests suffix replay after its listener is installed, and both sides clear process-local suffix state unconditionally at shutdown/replacement boundaries before replay.
...
Extend the pinned in-memory runtime coverage in `runtime.test.ts` through the real replacement-bound custom-message API. Prove the persisted child transcript entry has role `custom`, `customType: "agent-handoff"`, `display: true`, and the exact envelope, while the provider request sees the ownership prefix and exact kickoff. Prove the default renderer supplies the single visible `[agent-handoff]` label without registering an awf renderer. The new assertions must fail against `sendUserMessage`; run `./x pi-test run` and record that expected red state before production edits.
...
In `templates/pi/awf-handoff/index.ts.tmpl`, define one pure exported envelope helper or one equivalently testable single-home translation that returns the exact prefix, two line feeds, and the unmodified accepted kickoff. Keep pending state as the original kickoff so the tool result remains unchanged; construct the envelope only after the queued command has matched its request and use that same value for every automatic and recovery path.
...
Post-check: "Run ./x render, inspect every changed generated path, and require ./x check plus ./awf context --show pending on the linked ADR to report exactly the first two operations Applied and the effort-association operations Remaining."
...
Before dispatching this phase, the parent uses the approved Remote Pi peer boundary to send explicit implementation authorization for only the normative provider half stated in Architecture summary. The Remote Pi owner works under that repository's own guide and plan discipline, changes no awf file, runs its TypeScript typecheck/tests/build, independently reviews its change, and returns: the exact changed paths, terminal commands and results, the final 40-hex owner commit, and the matching v1 contract fixture. The parent verifies that receipt against all six contract bullets above and stops on any semantic mismatch; naming or formatting corrections determined by those bullets are mechanical, while a material mismatch returns to ADR amendment rather than being hidden in this plan.
...
Create `tools/pi-extension-test/fixtures/remote-pi-display-suffix-v1.json` as data-only structural evidence. Record the Remote Pi repository identity, returned 40-hex owner commit, exact capability/request/set/replay event names, complete payload examples for supported snapshot, canonical `demo-effort` set, explicit-null clear, namespace and value bounds, authoritative-snapshot withdrawal, synchronous session-start replay, exact `assigned base - suffix` composition, and the explicit list of operational identities the suffix cannot affect. Do not record Remote Pi helper symbols, release versions, installation paths, reconnect internals, or package metadata. Load and assert this fixture in the TypeScript suite; do not import Remote Pi code.

Rewrite the Remote Pi portion of `using-effort.test.ts` before production changes. Require metadata replacement to remain independent. Require factory-time and every `session_start` capability request; a valid `displaySuffix` v1 `awf` snapshot publishes the current slug through `remote-pi:display-suffix:set`; late support publishes immediately; a complete snapshot with the field absent, malformed, wrong-version, or without `awf` withdraws support and emits explicit null; repeated clears are idempotently accepted; `remote-pi:display-suffix:request` replays the current slug or explicit null; detach, ownership loss, restart, and shutdown emit unconditional explicit-null clears; thrown/missing event integration never changes tool, context, heartbeat, or metadata outcomes. Assert that no event name or type contains `name-override`, no fallback reconstructs or sends a base/composite name, and no publication changes the current effort snapshot. Run `./x pi-test run`; the new assertions must fail against the name-override implementation.

### Task 2.2: Replace name override with authoritative display-suffix replay
Latitude: exact
Applying: ["separate-pi-continuation-context-from-user-and-routing-identity:presentation-not-routing-identity", "separate-pi-continuation-context-from-user-and-routing-identity:display-suffix-capability", "separate-pi-continuation-context-from-user-and-routing-identity:authoritative-capability-replay", "separate-pi-continuation-context-from-user-and-routing-identity:independently-owned-delivery"]
Paths: ["templates/pi/awf-effort/index.ts.tmpl", ".pi/extensions/awf-effort/index.ts", "examples/sundial/.pi/extensions/awf-effort/index.ts", "tools/pi-extension-test/tests/using-effort.test.ts"]

In `templates/pi/awf-effort/index.ts.tmpl`, replace the `nameOverride` capability shape, `nameOK`, `namePublished`, name set/request listeners, and all `remote-pi:name-override:*` emissions with named awf-owned structural types matching the pinned fixture. Treat every received capabilities event as a complete snapshot: support is true only for exact display-suffix version 1 with an array containing `awf`; any other received value sets support false and emits `{namespace:"awf",value:null}`. No response leaves initial support false.

Keep metadata publication generated from the current immutable effort snapshot and independent of suffix support. When support is true and an effort is current, publish only its canonical slug as suffix. A late valid snapshot republishes current state. The display-suffix replay listener always publishes current slug when supported and current, otherwise explicit null. Clear suffix unconditionally and idempotently on explicit detach, successful switch detachment, owner loss, `session_start`, and `session_shutdown`, even if local publication flags believe nothing was set. At `session_start`, clear local association/publication as today, emit null metadata and suffix, then request capabilities again. Retain the factory-time request. Catch every optional event emission failure, never emit a name override, never inspect an assigned name, never compose presentation locally, and never make suffix support a condition for metadata, association, context, heartbeat, or detach.

Run `./x render` before tests. Require `./x pi-test run` to pass at 100 percent configured TypeScript coverage. Require `if rg -n 'name-override|nameOverride|NameOverride|_displayName|assigned.*name' templates/pi/awf-effort .pi/extensions/awf-effort examples/sundial/.pi/extensions/awf-effort tools/pi-extension-test/tests/using-effort.test.ts; then exit 1; fi` to exit zero with no match.

### Task 2.3: Apply effort claims and align all presentation documentation
Kind: batch
Latitude: exact
Applying: ["separate-pi-continuation-context-from-user-and-routing-identity:presentation-not-routing-identity", "separate-pi-continuation-context-from-user-and-routing-identity:display-suffix-capability", "separate-pi-continuation-context-from-user-and-routing-identity:authoritative-capability-replay", "separate-pi-continuation-context-from-user-and-routing-identity:independently-owned-delivery"]
Paths: ["templates/skills/using-effort/SKILL.md.tmpl", ".awf/parts/workflow/chain.md", ".awf/parts/working-with-awf/config-and-overrides.md", ".awf/topics/parts/rendering/pi-workflows/current-state.md", ".awf/domains/parts/rendering/current-state.md", ".awf/docs/parts/architecture/overview.md", ".awf/docs/parts/architecture/components.md", ".awf/docs/parts/architecture/data-flow.md", ".awf/docs/parts/architecture/dependencies.md", ".awf/docs/parts/testing/gate.md", ".awf/docs/parts/testing/layout.md", "README.md", "changelog/CHANGELOG.md", "internal/project/target_test.go", "internal/project/output_plan_test.go", "docs/decisions/separate-pi-continuation-context-from-user-and-routing-identity.md", ".pi/skills/awf-using-effort/SKILL.md", "AGENTS.md", "docs/architecture.md", "docs/testing.md", "docs/workflow.md", "docs/working-with-awf.md", "docs/topics/rendering/pi-workflows.md", "docs/domains/rendering.md", "docs/decisions/INDEX.md", ".awf/awf.lock", "examples/sundial/.awf/awf.lock"]
...
Post-check: "Run ./x render, inspect every changed generated path, and require ./x check plus ./awf context --show pending on the linked ADR to report all four operations Applied with none Remaining or Canceled while the ADR remains Implementing."
...
Update authored skill, guide, architecture, testing, domain, README, and changelog prose in the same transaction. State that Remote Pi owns `<assigned base> - <effort slug>` composition and user-facing projection, while the assigned base, mesh address, cwd lock, relay room, and routing remain unchanged. State that old or missing Remote Pi support degrades to metadata-only without restoring the retired name override. Name the independent owner-commit fixture as provenance, not a dependency or release floor. Run `./x render`; include all deterministic root and adopter consequences and never edit generated files directly.
...
This tail is not an implementation phase or transaction. After both phase commits and any phase-review fixes are clean and green, merge the integration branch into the managed worktree. Resolve or abort any conflict and require the completed merge, `./x check`, and `./x gate` to be clean. Run `./awf adr number separate-pi-continuation-context-from-user-and-routing-identity`, then `./x render`; stage only numbering and deterministic render output, require `./awf check staged && ./x gate`, and commit `docs(adr): number Pi continuation identity` with the printed slug-to-number mapping in the body.

Invoke the native `awf-reviewing-impl` workflow over every awf implementation and review-fix commit after the committed plan-review baseline. Resolve findings in new checked and gated commits until terminal review reports zero unresolved findings. The review verifies the pinned Remote Pi receipt and fixture structurally but does not claim authority to review or mutate the Remote Pi owner commit; require the Remote Pi peer's own terminal review/test receipt to remain clean.

In the clean intended target checkout, run `./awf effort integrate pi-handoff-identity`. Accept only an already-integrated or fast-forward result, or a divergent result explicitly reported as staged without a commit. For a divergent result, run `./awf check staged && ./x gate`, commit the merge, and invoke `awf-reviewing-impl` again over the combined target history until zero unresolved findings. Stop on conflicts, red verification, a Remote Pi contract mismatch, or a user-decision finding. The ADR remains Implementing and this plan remains Proposed throughout this tail. Proceed to Phase 3 only in the integrated primary checkout after both repositories' terminal reviews are settled.
...
Latitude: exact
Paths: ["docs/decisions/separate-pi-continuation-context-from-user-and-routing-identity.md", "docs/plans/2026-08-04-separate-pi-continuation-identity.md", "docs/decisions/INDEX.md", ".awf/awf.lock"]
...
Stage only the lifecycle transaction. Require `./awf check staged`, `./x check`, and `./x gate` to pass, and require `./awf context --show pending` on the numbered ADR to report every operation Applied with no Remaining or Canceled operation. Commit:
...
- `dod: cross-repository-contract` Remote Pi owns and proves stable identity plus live `<assigned base> - <suffix>` presentation; awf pins its reviewed owner commit and exact version-1 structural fixture without package/runtime coupling, and missing or malformed integration degrades to metadata-only behavior.
...
- Phase 2 dispatch must not grant the Remote Pi peer awf ownership, and the awf phase owner must not edit Remote Pi. The peer may refine internal helper placement and test partitioning, but the six exact contract bullets and repository boundary are not implementation latitude.
