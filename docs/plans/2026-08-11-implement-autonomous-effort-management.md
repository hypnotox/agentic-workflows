...
Settlement-review refinement: renewed review found that the first scanner revision was line-bound and polarity-blind and that exact-phrase switching-owner bans could reject references while missing paraphrased ownership. A second attempted prose scanner remained vulnerable to clause association and synonym problems. The parent therefore removed both universal-language approximations and used the repository's structural single-home model instead: the rendered autonomous-creation contract marker and designated deliberate-switching section each occur exactly once in `effort-workflow`, no other rendered skill owns either marker, and ordered assertions prove the complete keep, discontinue, and discard semantics in both targets. Existing active explicit-slug signature checks remain unchanged. This reasoned correction permits non-owning references without pretending to validate arbitrary natural-language paraphrases. Final renewed review found one mechanical omission in the positive no-authorization oracle; the parent pinned the complete `or repeated authorization precedes creation` relation rather than four disconnected nouns.

Terminal-assurance settlement: terminal review over `c8ab23d84..46db00003` found three mechanical relation gaps. The correction now asserts the full normalized no-authorization sentence and falsifies its clause boundary, requires the explicit old-effort disposition decision and falsifies its removal, and states plus proves the complete dirty-topology path from context transfer through inspection, explicit native-Git discard, ordinary topology removal, and archival finish. This settlement clarifies the already approved switching safety contract and adds no lifecycle or CLI behavior.
