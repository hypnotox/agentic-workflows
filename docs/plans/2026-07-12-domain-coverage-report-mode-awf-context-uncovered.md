...
- **Created:** none (all additions land in existing files).
...
- [ ] **Task 1.2 — Add `project.Uncovered` + `UncoveredResult`.** Append to
  ...
  func (p *Project) Uncovered(tracked, scanRoots []string) (UncoveredResult, error) {
  	...
  	// covered tracked path. A directory absent here has no covered descendant.
...
- [ ] **Task 1.3 — Wire the `--uncovered` mode into `runContext` + add `printUncovered`.**
  ...
  func runUncovered(cwd string, scanRoots []string, staged bool, rng string, asJSON bool, stdout io.Writer) error {
  	...
  	if _, err := os.Stat(config.ConfigPath(cwd)); err != nil {
  		...
  		return printUncovered(stdout, project.UncoveredResult{ScanRoots: scanRoots}, asJSON,
...
- [ ] **Task 1.6 — Update `docs/working-with-awf.md`.** Find the `awf context` entry
  (~line 26) and add a sentence documenting the mode: "Run `awf context --uncovered`
  (optionally scoped to scan-root directories) to list git-tracked paths owned by no
  domain — the signal for where to configure a new domain." Keep the surrounding prose
  intact. If `awf context` has no dedicated line, add the sentence to the command's
...
- [ ] **Task 1.10 — Verify and commit.** Run `./x sync` (regenerates nothing yet —
  ...
  clispec, and `docs/working-with-awf.md` files (and `.awf/awf.lock` + any regenerated
  rendered doc if `./x sync` changed one). Commit:
