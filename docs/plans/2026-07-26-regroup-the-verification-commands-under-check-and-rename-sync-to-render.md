...
- **The `help-lists-group-children` marker landed on a new test, not the named one.** ADR-0159 Decision 6 and Task 2.2 both site it on "an extension of `cmd/awf/help_test.go`'s `TestCliCommandSpecSingleSource`"; it went on a sibling `TestHelpListsGroupChildren` instead. The assertion has to bound each child block by the next top-level line, which needs its own body rather than an extension of a single-source test. Benign, but recorded here because the ADR names a site a reader will otherwise look in and not find.
- **`tooling/cli:group-child-gating-honored` needed two proof sites.** The clispec resolver test proves resolution; it cannot prove the driver *honours* the result, and reverting the driver to `top.Gating` left it green. The claim now also carries a marker on `cmd/awf/checkgroup_test.go`'s schema-ahead-child test, which is the half that goes red.
