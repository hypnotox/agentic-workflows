...
Update the project-output-plan explanatory introduction to describe Publisher semantic reuse by coordinator operations without transferring Publisher ownership or collapsing capability universes. Reconcile authored architecture components, data flow, and the tooling-domain narrative so CurrentStateCoordinator is current rather than future and commit authorization retains its policy owner. Add narrow domain and topic selectors for new production and owner-test paths, and move or extend backing proof markers without changing active claim bodies. Preserve the supported project context aliases and direct delegation adapters needed to keep the existing context-query, state-ownership, and single-plan claims true, then render generated topics, domains, architecture, selectors, and lock.

No successor ADR is required because the implementation applies ADR-0296 and preserves ADR-0299, ADR-0300, and every active claim. If the clean implementation would require a material mutation to an active claim body, make the coordinator the primary producer of Publisher corpora, collapse distinct universes, transfer domain or commit policy, weaken current/history separation, or create a durable compatibility choice, stop before mutation and return to the ADR chain.

### Task 4.2: Complete RF-005 behavioral and structural assurance evidence
Applying: ["0296:dependency-direction", "0296:boundary-values", "0299:publisher-constructs-operation-plan", "0299:neutral-plan-values-below-coordination", "0300:owners-classify-results", "0300:repository-checker-aggregates-results"]
Paths: ["glob:internal/currentstatecoord/**", "glob:internal/contextinput/**", "internal/project/stateownership_test.go", "internal/testsupport/deps_test.go", "internal/testsupport/check_result_ownership_test.go", "cmd/awf/presentation_boundary_test.go"]
...
Record any authority-determined route deviation and residual debt in Notes before phase review. Leave ADR-0296, ADR-0299, and ADR-0300 Implemented and this plan Proposed for the orchestrator-owned deferred terminal transaction. Do not edit the audit program, add changelog text when observable behavior is unchanged, number or integrate anything, remove topology, run retrospective, or finish the effort.
...
- `dod: documented-boundary` Authored selectors, architecture, project-output-plan, state-ownership, and context-query authority describe and cover the implemented coordinator and neutral values; generated outputs and lock are clean without a new ADR or audit-program edit.
...
Initial plan review found two reasoned boundary issues. Stale-merge authority loading and qualification are now reserved for Phase 3 so Phase 1 cannot create overlapping coordinator paths. Active claim bodies remain unchanged under their existing Implemented ADR authority; Phase 4 updates explanatory introductions, architecture, selectors, domain narrative, and proof locations while retaining bounded project adapters that keep the exact claims true. If implementation makes any active claim materially false, execution stops for the ordinary ADR chain instead of hiding a claim mutation in documentation work.
