...
Independent terminal assurance found that working context had lost the established non-Git filesystem fallback and that its preparation combined snapshot configuration with caller-selected catalog facts. The settlement restores `snapshot.FilesystemTree` only for `ErrNotARepository`, adds a command-level red-then-green non-Git oracle, and derives a fresh operation-local `ProjectState` from the selected snapshot configuration plus the caller's stable roots, nested topology, complete catalog, and target declarations. A mismatched-caller test proves the selected snapshot controls profile facts and catalog membership. This reasoned correction preserves the approved neutral input, Publisher ownership, independent universes, and no-cache boundary without adding a service locator or changing behavior elsewhere.
