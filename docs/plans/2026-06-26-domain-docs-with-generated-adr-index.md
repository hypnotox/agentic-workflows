...
- [ ] Create `internal/project/domains_test.go`:
  ...
  - `TestDomainPartOrphan`: a part at `domains/parts/rendering/decisions.md` (the forced-index section, deliberately not declared) → orphan drift — proving the generated index cannot be silently shadowed; likewise `domains/parts/rendering/bogus.md` (undeclared section) → orphan; and a part dir for a non-declared domain → orphan.
