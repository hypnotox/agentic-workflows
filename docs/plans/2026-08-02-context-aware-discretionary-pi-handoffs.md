...
The original Phase 1 transaction applies State change 1, and its post-review settlement applies State change 2 after the approved ADR amendment moved the workflow claim beside the behavior it governs. Phase 2 applies State changes 3 and 4, Phase 3 applies State changes 5 through 7, and each transaction stages its matching current-state claim mutation and proof marker. Phase 4 lands and reviews the final runtime behavior while operation 8 remains pending; deferred Phase 5 applies that final claim update immediately before the terminal status event. Every behavior phase and review settlement updates the owning `.awf/` documentation source and runs `./x render` so root and Sundial generated outputs travel with their causes.
...
- **Phase 1 review settlement:** `.awf/topics/parts/rendering/pi-workflows/current-state.md`, `internal/evals/chain_test.go`, `templates/partials/{checkpoint-routine,checkpoint-approval}.md`, `templates/agents-doc/AGENTS.md.tmpl`, the linked ADR, and generated `docs/topics/rendering/pi-workflows.md`, `docs/decisions/INDEX.md`, `.awf/awf.lock`, and any other declared generated family that `./x render` actually changes.
...
- **Phase 3:** `.awf/topics/parts/rendering/pi-runtime/current-state.md`, `.awf/docs/parts/architecture/{overview,components,data-flow,dependencies}.md`, `.awf/docs/parts/testing/{gate,layout}.md`, `.awf/topics/parts/rendering/adapter-outputs/current-state.md`, `.awf/topics/parts/rendering/project-output-plan/current-state.md`, `.awf/topics/parts/tooling/quality-gates/current-state.md`, `.awf/docs/glossary.yaml`, `.awf/domains/parts/rendering/current-state.md`, `.awf/parts/agents-doc/{identity,working-memory}.md`, `templates/partials/{checkpoint-routine,checkpoint-approval}.md`, `templates/agents-doc/AGENTS.md.tmpl`, `templates/docs/{workflow,working-with-awf}.md.tmpl`, and `changelog/CHANGELOG.md`.
- **Phase 4:** `.awf/docs/parts/testing/{layout,tiers}.md` and `.awf/docs/parts/releasing/content.md`.
...
**Phase 1 review settlement update `pi-session-handoff-workflow`:**
...
**Execution mode: subagent-driven.** Start only from the committed and reviewed plan with `git status --short` producing no output, `./x check` finishing clean, and `./x gate` passing. The original independently green workflow-policy transaction applies State change 1; the post-review settlement below applies State change 2 without rewriting that closing commit.
...
- [ ] Land the Phase 1 review-settlement `pi-session-handoff-workflow` literal block in `.awf/topics/parts/rendering/pi-workflows/current-state.md`, preserving its existing proof markers, and append an Applied event naming only `update rendering/pi-workflows:pi-session-handoff-workflow` to the linked ADR.
- [ ] Run `./x render`; inspect all changed root and Sundial workflow/guide output, `docs/topics/rendering/pi-workflows.md`, `docs/decisions/INDEX.md`, locks, and every other changed generated family. Require `go test ./internal/evals ./internal/project ./internal/contextq`, `git diff --check`, `./x check`, `./awf check --staged`, and `./x gate` to pass, then commit:

```commit
fix(rendering): settle discretionary handoff review
```

## Phase 2: Reduce handoff to bounded prose

**Execution mode: subagent-driven.** Start with `git status --short` empty, `git merge-base --is-ancestor d6978dc8 HEAD` and `git merge-base --is-ancestor d7e6c370 HEAD` succeeding, `./x check` clean, and `./x gate` passing. These ancestor checks preserve the Phase 1 settlement and its final proof correction without making later plan-currency commits invalidate the baseline. This phase is one independently green runtime-contract transaction and applies State changes 3 and 4 in declaration order.
...
- [ ] **Task 2.4: Apply the two runtime claim updates and documentation.** Append one Applied event naming, in order, `update rendering/pi-workflows:pi-session-handoff-lifecycle` and `update rendering/pi-workflows:pi-session-handoff-public-contract`; land those two Phase 2 literal blocks with their updated proof markers in the same staged transaction. The workflow operation is already Applied by the Phase 1 review settlement and must not be repeated. Update every Phase 2 authored documentation path to remove runtime memory validation and describe the division of ownership: workflow instructions require and reorient from checkpoints, bounded kickoff prose carries that instruction, and the runtime owns replacement mechanics only. Remove or rewrite the `handoff_session memory paths are one effort-owned spelling` pitfall rather than preserving a false warning. Add an Unreleased breaking-change entry that names removal of `memoryPath` and the exact `{kickoff}` replacement schema.
...
- [ ] **Task 3.5: Apply the context, target, and runtime-floor claim batch.** Append one Applied event naming, in order, `add rendering/pi-runtime:pi-context-usage-injection`, `update rendering/pi-runtime:pi-extension-target-render`, and `update rendering/pi-runtime:pi-minimum-runtime`. Land the three Phase 3 literal blocks and the named Go proof marker together. Update every Phase 3 authored documentation path in the same transaction as the behavior: identify separate observation/replacement/subagent ownership; enumerate five governed Pi TypeScript outputs; describe Pi-only render/prune behavior, strict container coverage, neutral model-window terminology, active-branch compaction counting, publication-safe unavailable output, and the absence of pressure action or supported-operation warnings. Update both checkpoint partials and `internal/evals/chain_test.go` so the discretionary choice now names the exact `[session context]` line as its model-window/active-branch-compaction evidence while preserving the generic relevance/upcoming-work judgment and all Phase 1 ordering assertions. Add the exact glossary entry `session context facts: The transient model-facing line reporting current tokens against the active model window and the active-branch compaction count.` Add an Unreleased feature entry for the standalone context-usage extension.
...
**Execution mode: subagent-driven.** Start with `git status --short` empty, `git merge-base --is-ancestor c9330faa HEAD` and `git merge-base --is-ancestor d8185821 HEAD` succeeding, `./x check` clean, and `./x gate` passing. These ancestor checks preserve the Phase 3 implementation and its review settlement without making this plan-currency commit invalidate the baseline. This phase is one independently green real-runtime/documentation transaction. It leaves State change 8 Remaining so the ADR retains a legal `Implementing` state through terminal review.
...
- [ ] **Task 4.2: Document the pinned smoke while leaving the final operation pending.** Do not append an Applied event and do not mutate `rendering/pi-runtime:pi-real-runtime-smoke` in this phase. Update only the Phase 4 authored paths: testing layout/tiers describe the pinned real-request compaction refresh without mislabeling it as a second gate tier, and release instructions exercise the context line, compaction refresh, discretionary cancellation/editor recovery, and no persistence or automatic action. All architecture, agent-guide, rendering, workflow, glossary, root/Sundial adopter, and general container-test documentation already traveled with Phase 3 and must not be deferred into this transaction. Ensure no release smoke asks for optional confined memory.
