...
Implement ADR-0188: current-state-v2 ADR content stays amendable until a terminal status via
...
prescribed by ADR-0188 item 7: it runs only after the applicable terminal review settles, applies
both claim operations as one direct implicit batch, and flips ADR-0188 and this plan to
Implemented. Design rationale lives in ADR-0188; do not re-derive it here.
...
- [ ] **Task 2.3: flip-ownership surfaces.** Exact replacements:
  - `templates/skills/reviewing-adr/SKILL.md.tmpl`, `status-flip` section: replace the sentence
    ...
    Accepted flip keeps a named owner (ADR-0188 item 7 moves only the Implemented flip).
...
- [ ] **Task 2.4: guide, template, and agent-guide prose.**
  ...
  - Three plan-freeze surfaces (added to ADR-0188 item 9 by amendment): in
...
- [ ] **Task 3.2: glossary, pitfalls, roadmap.**
  ...
  - `.awf/docs/pitfalls.yaml`, the ADR-0154 amendment-freeze lesson: insert one sentence BEFORE
    ...
    attribution terminal): "Since ADR-0188 the amendment window extends to any non-terminal
    ...
    attribution to "(ADR-0154, ADR-0188; 2026-07-23, 2026-07-30)." and align the entry's
  ...
  - `.awf/docs/parts/roadmap/deferred.md`, section "A frozen-state ADR flip can smuggle unreviewed
    section content": rewrite its premise to the residual case: since ADR-0188 the stamp chain
...
**Execution mode: inline.** Precondition, per ADR-0188 item 7: execute only after the applicable
...
- [ ] **Task 4.1: apply the claim operations.** In
  ...
  - Update `adr-status-enum-and-matrix`: prose becomes "Every governed ADR is routed by the two
    ...
    `ADR-0143, ADR-0188`; `Origin:` and `Backing:` unchanged.
...
- [ ] **Task 4.2: flip ADR-0188 and this plan.** In
...
- The full corpus parses under the new binary with zero record edits outside ADR-0188 itself
...
- ADR-0188 itself takes the direct Proposed-to-Implemented path, exercising first-stamp
