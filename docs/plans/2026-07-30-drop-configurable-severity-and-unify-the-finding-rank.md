...
Execute ADR-0183: remove the two `currentState` severity configuration keys and the `off` value, collapse
...
and rationale live in ADR-0183; this plan is the execution record.
...
ADR-0183 applies incrementally: Phase 1 applies no operation, Phase 2 flips the ADR to Implementing and
...
ADR-0183 stays `Proposed` through this phase. Start from a clean working tree on
...
`./x gate` successful. This is ADR-0183's first incremental application transaction: it applies
...
- [ ] **Task 2.5: Add the migration test.** Create `internal/migrate/dropseveritysettings_test.go`
  asserting, against a fixture tree at generation 24: both keys are removed; every other `currentState`
...
- [ ] **Task 2.6: Set the schema version floor and re-point the generation-pinned assertions.** Bump
  ...
  unreleased minor, and reuse would leave `internal/project/version_test.go:15` pinned at generation 24,
...
- [ ] **Task 2.7: Upgrade both in-repo trees before any render or check.** Registering `{To: 25}` makes
...
- [ ] **Task 2.10: Update the documentation this falsifies.** Reality and its documentation land in the
  ...
  - `changelog/CHANGELOG.md`, under `## [Unreleased]` `### Breaking changes`: add an entry naming both
    removed keys, the fixed ranks, the removal of `off`, and schema generation 25 with `awf upgrade` as the
...
- [ ] **Task 2.11: Author the two claims and record the application.** Claim order in a part file renders as
  ...
  Schema generation 25 removes currentState.topicCoverage and currentState.topicFanout from a config tree, announcing each removal it performs, and leaves every other configured key byte-identical.
  Origin: ADR-0183
  Backing: test
  ```

  In ADR-0183, change `status: Proposed` to `status: Implementing`, append the Implementing status event
...
`./x gate` successful. This is ADR-0183's second incremental application transaction: it applies
...
- [ ] **Task 3.7: Author the two claims and record the application.** In
  ...
  `%d warning(s)` count (`cmd/awf/audit.go:58` and `:61`, `cmd/repoaudit/main.go:115`), which ADR-0183 item 3
  ...
  In ADR-0183, append one Applied event using the next `state-sequence` reported at execution time and
...
`./x gate` successful. This is ADR-0183's final application transaction: it applies
`invariants/current-state-authority:currentstate-handshake-findings-unranked`, then flips ADR-0183 and this
...
- [ ] **Task 4.6: Flip both records.** In ADR-0183, append the final Applied event using the next
...
- Applied against a fixture tree at generation 24, `awf upgrade` announces each key it removes, leaves every
  other `currentState` key byte-identical, and lands the tree at generation 25.
- `./awf check` is clean in both the root tree and `examples/sundial`.
- ADR-0183's Status history carries one Implementing event and Applied events covering all five declared
...
Close the integration step with `grep -rn 'ADR-0183' --include='*.go' internal cmd` returning no output,
...
per-migration byte-level branch; only generation 20 had one. Without a generation-25 branch every
...
and a test covering the forward-port, its idempotence, and the skip at generation 25. Anyone removing a
...
- The outcome-modeling pattern topic that ADR-0183 unblocks. Its shape is settled but it is a separate
...
- The `%d warning(s)` verdict summaries at `cmd/awf/audit.go:58`, `:61`, and `cmd/repoaudit/main.go:115`
  survive deliberately, per ADR-0183 item 3. They are verdict prose, not a finding rank, so
