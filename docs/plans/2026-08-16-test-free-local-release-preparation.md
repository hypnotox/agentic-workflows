...
- 2026-08-16 terminal-review settlement: replaced three stale `project.Version` constant references in `cmd/awf` comments and a test-case name with embedded-authority terminology. The mechanical prose-only correction changes no behavior and is verified by the focused CLI tests and full gate.
