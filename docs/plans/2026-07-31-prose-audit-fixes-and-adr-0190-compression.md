...
section D and by the 2026-07-31 post-0187 re-verification), and apply ADR-0190's
...
Task 2.2 (a scope correction, claim quoted there) and the Phase 7 claim revision ADR-0190
...
catalog-touching ones, then the shared-partial extractions ADR-0190 decision 2 bounds,
then the deferred ADR-0190 application transaction (template compression + claim update +
...
- **Modified:**
  ...
  - `.awf/topics/parts/rendering/workflow-skill-templates/current-state.md`,
    `docs/decisions/0190-compress-governed-dispatch-guidance-with-reference-and-shared-partials.md`,
...
**Execution mode: inline.** One transaction for the B4 extractions ADR-0190 decision 2
...
## Phase 7: ADR-0190 application - compression, claim update, flips

**Execution mode: inline.** This is the deferred final transaction of ADR-0190's single
...
- [ ] **Task 7.1: Create the model-selection partial and its guide consumer.** Create
  ...
  guide is the partial's only consumer; no skill site includes it (ADR-0190 decision
...
- [ ] **Task 7.4: Trim the working-with-awf convention part (ADR-0190 decision 4).** In
...
- [ ] **Task 7.5: Apply the claim update.** In
  ...
  `Revised-by: ADR-0190` under `Origin: ADR-0173`. `Backing: test` and the proof marker
  are unchanged.
- [ ] **Task 7.6: Flip the statuses.** Append to ADR-0190's Status history the direct
...
- [ ] **Phase-close: stage, check, gate, and commit.** Stage the complete transaction
  ...
  against ADR-0190's operation); `./x gate` green; commit:

```commit
feat(invariants): compress dispatch guidance (implements 0190)
```
...
- The sundial example re-renders clean (`./x check` covers it) and
  `grep -rn 'sundial new plan' examples/sundial/` returns no output.
- ADR-0190 and this plan both read `status: Implemented`; `docs/decisions/INDEX.md`
  reflects it and was generated, not hand-edited.

## Notes

- Deferred, needs its own decision per ADR-0190 decision 2's bound: merging the
