...
use the same boundary. Assert that an existing effort resumes under fixed identity without
reconfirmation only while work remains within its confirmed outcome; a newly discovered outcome
cannot silently reuse, rename, replace, or create beside that active effort. Require both creation-
failure branches: retained pair-and-response evidence reports the concrete
...
- In `templates/docs/working-with-awf.md.tmpl` and `.awf/parts/working-with-awf/commands.md`, prefix the `awf effort new` mechanics with `Workflow authorization precedes this command: discovery creates no effort, and first creation follows a clear later user response confirming the labeled outcome and proposed title. Existing efforts resume without title reconfirmation only for work inside the confirmed outcome; a new outcome cannot silently reuse or replace an active effort.` Do not imply the CLI verifies confirmation.
- In `.awf/parts/working-with-awf/config-and-overrides.md`, replace its first creation summary with `Discovery creates no effort. A concrete non-minimal outcome uses exactly one immutable slugged effort only after the labeled outcome and proposed title receive clear later user confirmation; an existing effort resumes under its fixed identity without title reconfirmation only for work inside the confirmed outcome, while a newly discovered outcome cannot silently reuse, rename, replace, or create beside the active effort.` Retain the writer, authority, integration, removal, retrospective, and finish clauses. Add `Full-replacement workflow, guide, checkpoint, or affected skill parts must re-derive this confirmation boundary; default-template projection tests cannot inspect replacement prose.` to the override responsibility paragraph.
...
exception. `confirmed-report-support` must require an existing confirmed effort, route missing ownership back to
first-creation confirmation, never create an effort, and remain report-only toward parent memory.
`never-create-support` must say it never creates an effort; `exploring` remains report-only toward
parent memory, while `orienting` retains its one-writer stale-checkpoint correction. Across all roles
retain the within-confirmed-outcome restriction and fixed-identity resume wording where applicable,
...
- `refactor-coupling-audit`: state `This report-only audit requires an existing confirmed effort before non-minimal pre-ADR analysis and never creates a missing effort. If ownership is absent, stop and return to mandatory first-creation outcome/title confirmation. Receive the existing slug and owned memory path as read-only context; never create a second effort, become a second writer, or edit shared memory.` Retain its authority and coupling-report contract.
- `exploring`: state `This support path never creates an effort. When a parent effort exists, its slug and owned memory path are read-only context; never create a second effort, become a second writer, or edit shared memory.` Retain its report-only and authority contracts.
...
Edge: "Name the upgrade obligation for adopter-owned full replacements of workflow, guide, checkpoint, or affected skill parts; state that existing efforts resume unchanged only inside their confirmed outcome and that no CLI or schema migration occurs."
