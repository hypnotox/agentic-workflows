...
- 2026-08-10 terminal review settlement: wrapped the two kernel rollback parent-sync failures with their reservation-versus-deletion boundary while preserving error identity. Reapplied `tooling/cli:effort-command-contract` to state the already-proven primary-root-qualified archive path: repository-relative from the primary checkout and absolute from a linked checkout.
- 2026-08-10 terminal verify-pass settlement: routed injected rollback parent-sync faults through the same reservation-versus-deletion context as kernel failures and proved both context and wrapped error identity. Expanded the existing Unreleased entry for the adopter-visible diagnostic precision. These mechanical residuals close terminal review without another review dispatch.
- 2026-08-10 integration deviation: merging `main` introduced ADR-0258's `internal/filepublication` single home for released-platform atomic no-replace namespace publication. Moved the archive directory rename mechanism behind that existing leaf, kept `internal/effort` as the owner of archive identity and recovery policy, and added a direct directory collision/move proof. This preserves ADR-0259's no-copy/no-traversal boundary while satisfying the integrated single-home invariant.
