...
status: Proposed
---
# Plan: Remove the global state sequence (ADR-0189)

## Goal

Implement ADR-0189: remove the repository-global contiguous `state-sequence` namespace, order
...
migration (generation 26). Non-goals: the merge-time ADR-numbering design (a separate effort), any
...
transaction. Phase 1 flips ADR-0189 to Accepted. Phase 2 is the flag day: one transaction carrying
the grammar and validation rewrite, the presentation-field removals, migration 26 plus its run over
this repository, the documentation-source sweep with re-render, the test sweep with relocated proof
markers, the claim mutations for eight of the nine declared operations, and ADR-0189's Implementing
...
- **Created:**
  - `internal/migrate/adrnumberprovenance.go` (migration 26)
...
## Phase 1: Accept ADR-0189

**Execution mode: inline.**

- [ ] **Task 1.1: Flip ADR-0189 to Accepted.** In
  `docs/decisions/0189-replace-the-global-state-sequence-with-adr-number-provenance-order.md`,
...
- [ ] **Task 2.8: Migration 26 (`internal/migrate/adrnumberprovenance.go`).** Follow the
  ...
  `{To: 26, Name: "adr-number-provenance", Apply: applyADRNumberProvenance}`. Two passes:
...
- [ ] **Task 2.13: Claim mutations and ADR-0189 events.** In
  `.awf/topics/parts/invariants/current-state-authority/current-state.md`:
  - Delete the whole `### \`invariant: application-batch-sequence-order\`` block.
  - Add, with `Origin: ADR-0189` and `Backing: test`:
  ...
  - `merge-transition-ordered-aggregate`: replace `several application batches are legal when the
    ...
    any number of dominated updates`; append `ADR-0189` to its `Revised-by`.
  - `implemented-impact-bidirectional`: replace `has its required current or removed result` with
    ...
    no authority`; `Revised-by` gains `ADR-0189`.
  - `state-impact-transition-atomic`: after `exactly its matching claim mutations occur in one
    ...
    empty`; `Revised-by` gains `ADR-0189`.
  ...
  - `corpus-raw-access-enumerated`: `The two ordered schema migrations` becomes `The three
    ordered schema migrations`; `Revised-by` gains `ADR-0189`.
...
```commit
feat(adr-system): remove the global state sequence (ADR-0189)
```
...
- [ ] **Task 3.1: Apply `update-requires-substance` and flip Implemented.** In
  ...
  its canonical ascending position`; `Revised-by` gains `ADR-0189`. In the ADR file, append
...
```commit
docs(adr): implement 0189 ADR-number provenance order
```
...
- `./awf topic invariants/current-state-authority` shows the two new claims with
  `Origin: ADR-0189` and no `[state-sequence:` suffix anywhere in its output.
