...
Four implementation phases keep every commit green while respecting both ADRs' lifecycle order. Phase 1 accepts the prerequisite ADR. Phase 2 applies its three operations atomically, adds the one-way production-import proof, and implements it. Phase 3 accepts the filesystem ADR only after the prerequisite is terminal. Phase 4 first leaves its vertical slice dirty for report-only pre-commit review, then lands production handle, shared fault source, outside-package test consumer, upgrade-local structural contract and policy helpers, domain ownership, all five claims and proof markers, filesystem ADR implementation, and plan freeze in one combined green transaction. The production-package domain-ownership and scoped-topic coverage gates mutually require the domain assignment and active filesystem claims in that same transaction, so separate implementation and deferred-authority commits cannot be green.
...
- [ ] **Task 2.1: Add a repository production-import proof.** In `internal/testsupport/deps_test.go`, retain `dependencyViolations` and `TestZeroInternalDeps` unchanged in purpose. Add a `productionTestsupportImportViolations(path string, source any) ([]string, error)` parser helper and scan the canonical repository boundary with `testsupport.WalkRepoSources`, anchored with `testsupport.RepoRoot(t)` from the external `testsupport_test` package so nested checkout gitfiles are pruned. The walker covers every production `.go` source in this repository, including the root package, `cmd/**`, production packages under `internal/**`, `changelog/**`, `templates/**`, `tools/**`, and the separate `examples/sundial` module. It excludes the entire `internal/testsupport/**` source boundary, which is test-only and remains governed by `TestZeroInternalDeps`; `_test.go`; every directory named `testdata`, `vendor`, or `node_modules`; `.git/**`; and resident `.awf/efforts/**` and `.awf/worktrees/**` trees because none is outside-testsupport production source. Do not exclude another nested module merely because it has its own `go.mod`. Parsing failures are test failures, not ignored files.
...
- [ ] **Task 4.1: Implement the production handle and its contract tests.** Create `internal/filesystem/handle.go` with a one-sentence package comment and concrete `Handle` owning an `*os.Root`. Export and document `Open(root string) (*Handle, error)`, `Close() error`, `Walk(subtree string, visit func(path string, info fs.FileInfo) (bool, error)) error`, `Read(path string) ([]byte, error)`, `Info(path string) (fs.FileInfo, error)`, and `LinkInfo(path string) (fs.FileInfo, error)`. `Open` calls `os.OpenRoot`. Every operation accepts only `.` or `fs.ValidPath(name)`, rejects empty/absolute/parent paths before delegation, uses slash-relative names, and wraps operation plus path with `%w`. `Walk` uses the root's `fs.FS`, resolves `DirEntry.Info` before the consumer callback, supplies metadata for the entry itself without following a final symlink, treats callback `true` as descend and `false` as skip only for directories, ignores the boolean for nondirectories, and propagates callback identity. It never returns absolute paths or exposes `filepath.SkipDir`. It does not follow directory symlinks. Keep any genuinely race-only adapter branch behind a specific `coverage-ignore`; do not copy digest exclusions.
  ...
  Build an AST scanner over every repository non-test Go source. `TestRootConfinedFilesystemSingleHome` proves every future `single-production-handle` clause: calls to `os.OpenRoot` and stored `*os.Root` production values occur only under `internal/filesystem/**`; the explicitly distinct `internal/testsupport/fsfixture/**` path is classified as test source rather than production; `internal/filesystem` declares exactly one exported concrete handle type, `Handle`, and no interface; and every outside-package production use imports the concrete package. Synthetic refuting sources must prove detection of an extra `os.OpenRoot` call, a stored `*os.Root`, a second exported concrete handle type, and a provider-owned exported interface. Do not add invariant proof-marker comments before review settles; the combined authority transaction owns them.
...
- [ ] **Task 4.5: Document architecture and prepare the combined authority transaction.** In `.awf/docs/parts/architecture/components.md`, add one bullet saying `internal/filesystem` owns the one production `os.Root`-backed handle, consumers own local structural contracts and policy helpers, and `internal/testsupport/fsfixture` is the distinct standard-library-only kernel-backed fault source. Before report-only pre-commit review, do not add `internal/filesystem/**` to `.awf/domains/tooling.yaml`, author claims or markers, or transition ADR/plan status. The production-package domain-ownership gate rejects the package without the domain assignment, while scoped-topic coverage rejects that assignment without active filesystem claims; after the user-approved pre-commit review, add both with all authority inputs and Phase 4 implementation in one combined green transaction. Run `./x render` for the architecture output only.

- [ ] **Task 4.6: Verify the reviewable vertical slice.** Run `go test ./internal/filesystem ./internal/testsupport/fsfixture ./internal/upgrade`; all pass. Run `rg -n 'var lstat|SwapVar\(t, &lstat|os\.(ReadFile|Stat|Lstat)|filepath\.WalkDir' internal/upgrade/digest.go internal/upgrade/*_test.go`; production `digest.go` and the old mutable seam produce no matches, while direct OS assertions in tests are acceptable only outside the injected policy path. Run `go test ./internal/testsupport` to prove production does not import the fixture. Run `./x render && ./x check`; report the expected mutually-enforcing ownership failure separately: the new production package lacks a domain assignment, while assigning it without its active scoped claims is also rejected. This review checkpoint does not claim a green gate before authority.

- [ ] **Phase-close: settle report-only pre-commit review.** Leave the provider, fixture, outside-package consumer, tests, architecture source, and rendered output dirty for independent review. Do not stage or commit this checkpoint. The user approved replacing separate implementation and deferred-authority commits with this pre-commit review followed by one combined green transaction: apply domain ownership, all five claims and markers, ADR implementation, and plan freeze with the implementation. Splitting it cannot satisfy the mutually enforcing production-package ownership and scoped-topic coverage gates.
...
Before report-only pre-commit review, verify the three prerequisite phase commits are present in order and the worktree contains only the reviewed Phase 4 implementation. Both ADR lifecycle states remain prerequisite `Implemented` and filesystem `Accepted`, and the plan remains `Proposed`. Run `go test ./internal/filesystem ./internal/testsupport/fsfixture ./internal/upgrade ./internal/testsupport` and confirm every package passes. Re-run the scratch production import violation from Phase 2 and one scratch `..` or escaping-symlink handle access from Phase 4, confirm the intended test/refusal, restore the files, and report the expected ownership failure before authority.

After report-only pre-commit review settles, the user-approved one combined green transaction owned by `awf-reviewing-impl` adds `internal/filesystem/**` to `.awf/domains/tooling.yaml` adjacent to `internal/git/**` before `./x render`. The production-package ownership gate requires that assignment, and scoped-topic coverage requires its active filesystem claims; these mutually enforcing gates prohibit separate implementation and deferred-authority commits. The transaction then does all of the following atomically:
...
7. Run `./x render`; stage all authored claims, markers, ADR, plan, `docs/decisions/INDEX.md`, generated topic/domain/architecture outputs, and `.awf/awf.lock`. `./awf check staged` and `./x gate` must both pass before the combined commit.
8. Commit the combined transaction exactly with `git commit -m "refactor(code-design): compose root-confined upgrade filesystem"`. The subject is the one closing subject for implementation, claim application, ADR terminal state, and plan freeze.
...
- The required old-signature test-first command could not run after a prior dirty owner had already converted production, so it did not run and no pre-conversion focused test is claimed. Instead, `go run ./cmd/baselinedigest` ran a disposable faithful copy of `b00d1274`'s pre-conversion digest program to capture `sha256:ec2b28599282f9ae5e0a92d28527d228bac7870303375518e718b126f5231940`. The frozen converted `TestTreeDigestCharacterization` asserts that same literal digest and its literal seven-path universe; it is a converted characterization, not evidence that the unavailable old-signature focused test ran.
- Phase 4 coverage exclusions: `internal/filesystem/handle.go` and `internal/testsupport/fsfixture/fsfixture.go` each retain only the underlying `WalkDir` callback error after `LinkInfo` resolved the root and the `DirEntry.Info` error after `WalkDir` delivered an entry. Permission controls are execution-identity-dependent; otherwise both require a concurrent filesystem change. All injected digest-policy failure branches are exercised by the kernel-backed fixture.
