...
- [ ] In `.awf/domains/parts/tooling/current-state.md`, reword the ADR-0039 sentence (line 5) that today slash-joins `sync/check/invariants/audit/list/enable/disable/new` (omitting `config`/`context`) to consume the placeholder: `A binary-version gate (ADR-0039) precedes those reads: the gated commands ({{=awf:gatedCommands}}) refuse to run against a project rendered by a *newer* awf …` — a sentence rewrite, not token substitution, so the generated list carries the correct membership. **While in this file, also fix the stale CLI-symbol references** the refactor invalidates: `addRemoveSingleton` (line 5) → `enableDisableSingleton`, and the line-19 phrase "parsed by a positionals filter over the former len(args) dispatch" (the `positionals` helper is deleted in Phase 2) → describe the new parse-once driver over the `internal/clispec` table.
