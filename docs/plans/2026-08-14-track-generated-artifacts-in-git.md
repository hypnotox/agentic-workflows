...
- Phase 2 review found that the outer current-state guard and staged version gate still refused an absent staged lock before drift collection. The settlement adds `cmd/awf/main.go` so only the staged aggregate and direct staged drift pass that absence through to the planned `untracked` result; staged state and invalid authority remain operational refusals. This added path is required by the approved absent-lock outcome and retains the existing guard and gate ownership boundaries.
- Renewed Phase 2 review required proof attribution to cover both verification universes and the staged absent-lock and invalid-lock clauses. The settlement links the existing repository proofs, adds an exact staged whole-OutputPlan-set proof, and marks the full-dispatch and operational-error tests that exercise those clauses.
- Terminal assurance found that staged membership reused the content-scannability projection and therefore misclassified an indexed symlink as untracked. The settlement separates snapshot metadata membership from staged bytes and proves the symlink receives content drift instead.
