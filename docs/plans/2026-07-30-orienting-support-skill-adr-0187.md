...
- Both destination topics reach the twenty-claim advisory ceiling with this ADR's adds: `rendering/workflow-skill-templates` via `orienting-single-home` and `config/migrations-and-locks` via `orienting-skill-backfill` (ADR-0187 Consequences names the first). The next claim added to either topic needs a split or a new topic.
