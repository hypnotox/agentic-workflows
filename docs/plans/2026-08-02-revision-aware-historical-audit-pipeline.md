...
Implement `TreeEntry`, `CommitEntries`, and `CommitBlobsAt` inside `internal/git` using go-git tree metadata and exact blob lookup. No backend representation crosses the exported signatures. Reuse `BlobMode`, rerooting, context checks, and opaque error translation. Enumeration must not call a blob reader. Selected loading must read each requested blob once and return owned bytes. In `internal/snapshot/index.go`, factor the existing `git.IndexBlob` to `snapshot.File` mode/byte translation into one unexported helper used by complete `treeFromBlobs` and a documented `NewSelectionFromBlobs([]git.IndexBlob) (*Selection, error)`. `internal/audit` consumes that constructor as the concrete first caller; it must not duplicate the mode switch or construct translated `snapshot.File` values itself. Extend `internal/snapshot/selection_test.go` to prove every Git blob mode, byte cloning, sorting, and invalid translated selection through this exact constructor.
...
- Phase 4 benchmarks on an AMD Ryzen 5 7600X measured `BenchmarkAuditHistoryCodeOnly50` at 67.9 ms/op and 14.8 MB/op, `BenchmarkAuditHistoryAuthorityHeavy50` at 113.9 ms/op and 49.3 MB/op, and `BenchmarkAuditHistoryMergeHeavy50` at 43.1 ms/op and 16.4 MB/op. A prebuilt sparse-loader binary audited a clean representative 84-commit range in 6.197 seconds, down from the earlier roughly one-second-per-commit behavior and below the diagnostic substantial-improvement threshold without making time a gate.
- The authority-heavy profile still attributed 36.95 percent of allocation space cumulatively to ADR file parsing, with YAML parser allocation and filesystem/stat work also prominent; CPU samples were led by syscall time. A follow-up should first measure operation-local incremental ADR/topic corpus reuse across adjacent authority revisions before considering broader structural sharing; this transaction adds neither.
- Implementation deviation: the obsolete reduced complete-tree entrypoints were retired instead of delegating to the shared file-set core because no production consumer remained and the dead-code gate forbids test-only production APIs. Complete `snapshot.Tree` APIs and full current/staged loaders remain unchanged; historical policy uses only `snapshot.Selection`.
