...
At a workflow-safe resumable checkpoint, follow the ordinary `effort-workflow` guidance when an effort owns continuity. Resolve its memory path with `./awf effort show <slug>`, update the Markdown through ordinary file tools under the one-writer rule, and reorient the successor from repository authority before substantive work. Managed worktree use ends before integration, deferred lifecycle closure, worktree removal, and retrospective. A replacement session records the actual boundary; continuation, cancellation, or a failure that leaves the current session active records none. `pi-tools` owns the executable handoff mechanism; awf owns the host-neutral workflow projection.
