...
category. Sixteen express a fact that genuinely differs between repositories: the skill-name
prefix, the integration branch, the eight command vars, the domain keys, the tag vocabulary, the
commit-scope taxonomy, the context-ignore globs, the current-state marker sources, the commit
policy, and the two gate exemption lists. Every one of them holds a non-default value in this
checkout, because a repository that did not set them would be describing itself wrongly.
...
1. `decision: repo-facts-only` A configuration key exists only to express a fact that genuinely
   differs between the repositories awf serves. A key that expresses a preference about awf's own
   behaviour is not a key, because awf has one behaviour. The admission test for a proposed key is
   whether two served repositories would legitimately hold different values; if they would not, the
   value is fixed in the binary. This subsumes ADR-0084's var-descriptor policy and extends it to
   the whole config tree, to sidecar fields, and to init answers. Reintroducing a behaviour
   preference as a key requires a successor record.
...
2. `decision: full-catalog-render` Every catalog skill, agent and doc renders for every target on
   ...
   closure, no `requiresDoc` gate, no mandatory-versus-toggleable partition and no per-artifact
   suppression stands between the catalog and the output plan, so adding a catalog entry changes
   every served repository's output.
...
3. `decision: retire-selection-keys` The `skills`, `agents`, `docs`, `targets` and `docsDir` keys
   ...
   parsing rather than honoured, on the same footing as the severity keys ADR-0183 retired.
...
5. `decision: retire-local-artifacts` The project-local artifact channel is retired. Every skill,
   ...
   which is the surviving customization path.
...
6. `decision: selection-key-migration` A schema generation strips the retired keys from a config
   ...
   shape of their own generation, and this generation removes the keys afterward.

7. `decision: historical-config-forward-ported` Every retired key continues to parse on the
   historical path, so that a staged check or an audit over a commit range predating this record
   neither fails nor honours the retired value. Retiring a key from the live schema is not
   retiring it from history.

## State changes

- add `config/configuration:config-expresses-repo-facts-only`
- add `config/configuration:no-artifact-selection-surface`
- add `rendering/catalog-and-targets:full-catalog-render`
- add `rendering/catalog-and-targets:targets-fixed-claude-pi`
...
- update `config/migrations-and-locks:singleton-doc-migration-relocates-parts`
...
- update `rendering/doc-outputs:layout-docs-enabled-only`
...
The `enable`/`disable` and target commands lose the arrays they edit, and `awf new skill|agent|doc`
loses the local channel it scaffolds into. Retiring those commands and settling the resulting CLI
grammar is a separate decision and is not made here.
...
| Retire selection but keep `docsDir` and `targets` as repo facts | The owner confirmed neither differs across the repositories served; a key nobody varies is the surface this record exists to remove. |
