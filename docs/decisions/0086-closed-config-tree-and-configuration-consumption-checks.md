...
- `awf init` deliberately seeds `vars:` with every catalog template's referenced vars —
  including artifacts outside the scaffolded enable set — so a later `awf add` renders
