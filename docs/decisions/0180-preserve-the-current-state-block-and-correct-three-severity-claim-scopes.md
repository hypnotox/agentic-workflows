...
an operation against a claim ADR-0179 already applied, and ADR-0179 is Implemented, so its meaning is
...
circumstantial one: no awf-produced tree can arrive at generation 23 with a block these two removals
...
That filter is also the shape ADR-0179 item 2 argued against. Its selection split exists so that a
...
4. `migrate.ConfigForCurrentSchema`'s generation-24 branch stays a pure removal and does not seed. It
   ...
   the working-tree and staged-after paths too, and both do evaluate coverage. The generation-24 branch
   fires only for a side whose own lock is below 24, and `gate` and `gateStaged` in `cmd/awf/gate.go`
...
loud. It is also the narrower reading of ADR-0179 item 2 applied to awf's own call site.
...
than this correction, and ADR-0179 did not take it.
