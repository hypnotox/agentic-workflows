...
1. awf will make constrained Markdown current-state topics the sole source of authority for current project reality. Each topic belongs to exactly one domain, remains focused on one area of current behavior, and contains individually addressable normative claims. An explicit global topic scope carries the intentionally small set of repository-wide claims without creating multi-domain ownership. Domain documents become overview and navigation surfaces over their topics.
...
- `unbacked-invariant: current-state-sole-active-authority`: Normal context retrieval and invariant enforcement consume current-state topic claims, not Implemented historical ADR prose. **Verify:** in an adopter fixture containing claim provenance and an Implemented ADR, run `awf context <matching-path>` and confirm it emits the active claim but no historical ADR; then run `awf topic <claim-id> --history` and confirm it emits the ADR.
...
- `unbacked-invariant: topic-scope-is-domain-bounded`: Every topic has one owning domain and its effective path scope is the intersection of its selectors and that domain's ownership. **Verify:** configure a topic selector that also matches a path outside its parent domain and confirm topic context includes only the domain-owned match while coverage reports the other path under its actual owner.
