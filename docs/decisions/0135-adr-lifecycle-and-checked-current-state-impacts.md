...
11. At the one project-atomic upgrade boundary, staged validation uses a migration-only cross-schema adapter: it reads only ADR identity/status, legacy invariant declaration and retirement records, proof and touches markers, and the format-cutoff baseline from the legacy HEAD, then validates the index entirely with the new topic engine. While those legacy HEAD facts remain closed, the adapter may consume the migration-only authored `.awf/current-state-migration.yaml` approval file from the prepared tree. That file records reviewed mapping results for upgrade preflight; it is neither topic authority nor runtime authority, and it may only confirm an independently derived mapping, never select or disambiguate candidate claims. The adapter never assembles legacy context, supersession authority, or current guidance and remains only as upgrade-preflight support.
