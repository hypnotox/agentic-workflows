...
5. Proposed ADRs are editable and non-authoritative. Accepted freezes the Decision and State changes and is normative only as instruction for executing the pending change; it never overrides the current claims describing project reality. Normal context selects an Accepted notice only when an operation targets a topic applicable to the query. Temporary implementation authority ends at Implemented or Abandoned.

6. `## Status history` is append-only bookkeeping. Scaffolding writes `- YYYY-MM-DD: Proposed`. Every transition appends `- YYYY-MM-DD: <status>` in the same commit as the frontmatter change. An Abandoned entry requires ` - <nonempty rationale>`. Dates never descend, adjacent statuses follow the legal matrix, and the final log status equals frontmatter. Accepted freezes all prior body meaning except appending this log and mechanically permitted schema normalization. Implemented and Abandoned freeze the complete record.

7. In the pre-implementation state, every `add` target is absent and every `update` or `remove` target exists. An Abandoned ADR retains its intended operations as history, but they remain unapplied; partial work must be reverted or reconciled before abandonment. An Implemented add has one active claim whose Origin is that ADR unless a later Implemented remove exists. An Implemented update appears in the active claim's Revised-by sequence unless a remove exists. An Implemented remove has no active claim. Each identity has at most one add and one terminal remove across new-format history; each active Origin or revision has the inverse ADR operation.

8. The implementation transition is one checked Git transaction. `awf check --staged` compares HEAD with the index. A transitioned add must be absent before and present after with matching Origin; update must exist on both sides, change at least one canonical claim field other than formatting or provenance alone, and append the ADR to Revised-by; remove must exist before and be absent after. Every staged claim mutation belongs to exactly one ADR becoming Implemented, and every declared operation has its matching mutation. A direct Proposed-to-Implemented transition follows the same rules.

9. Static `awf check` validates the resulting lifecycle, operation history, claim provenance, and absence rules without claiming to replay semantic history. The rendered pre-commit hook runs `awf check --staged`. The explicit-range audit and implementation review reuse the same snapshot-diff engine over the selected commit range. Configuration, topic inputs, ADRs, and markers are read entirely from each compared snapshot rather than mixed with the working tree.

10. awf will replace `<docsDir>/decisions/ACTIVE.md` with generated `<docsDir>/decisions/INDEX.md`. Its In flight section lists Proposed and Accepted ADRs. Its History section lists Implemented and Abandoned ADRs by number, title, and status only. It renders no supersession chains, anchor annotations, partial-currentness claims, or domain ADR indexes.

11. ADR authoring, lifecycle, review, planning, implementation, audit, and retrospective skills will use the new format and authority language. Every status transition runs sync and commits the generated decision index plus affected current-state topics and generated domain/topic navigation. The `.awf/` sources for AGENTS.md, the ADR guide and template, workflow guidance, reviewer lenses, domain current state, config reference, and adopter documentation change with the implementation rather than leaving generated instructions on the old lifecycle.

## Invariants

- `unbacked-invariant: adr-status-enum-and-matrix`: Every new-format ADR has a recognized status and every observed transition follows the five legal edges. **Verify:** exercise every status pair in static and snapshot transition fixtures and confirm only the declared matrix succeeds.
...
- `unbacked-invariant: update-requires-substance`: An update changes a canonical claim field other than formatting or provenance alone. **Verify:** compare staged fixtures containing whitespace-only, provenance-only, normative prose, reference, and backing changes and confirm only the substantive cases satisfy an update.
...
The staged check introduces explicit Git index semantics into the blocking gate. It must share snapshot loading with range audit to avoid two interpretations. This expands hook and fixture complexity but catches removal and substantive-update facts that a current-tree-only check cannot prove.
