...
6. Execute prepared actions in selected declaration order and return one structured outcome per attempted step, retaining step identity and error. The caller explicitly chooses stop-on-failure or continue-on-failure. `Prepared.Run` returns those outcomes and a separate execution-level error: cancellation observed before the next action stops execution and returns `ctx.Err()` without inventing an outcome for an unattempted step; cancellation observed after an attempted action retains that action's returned outcome and also returns `ctx.Err()`, so neither fact masks the other. The package does not render text, recover panics, choose an error aggregation policy, or assign an exit code. Direct one-step check children consume stop-on-failure; the repo aggregate consumes continue-on-failure, so both exported policies have concrete production consumers.

7. Make `awf check repo` the first adopter. Working config and scanner enablement are its foundation. The command composition root constructs a `project.Loader` whose operation-local `LoadConfigTree` dependency returns that already prepared config for the selected root; `Loader.Open` remains the owner of project validation, target and catalog derivation, resident-root resolution, and Project construction without re-reading config bytes. Drift conditionally requires the resulting one opened `Project` and one complete `CheckReport`; state requires the same opened project and one working `CurrentStateReport`; enabled prose and memory steps require one shared stage-0 index snapshot; disabled scanners request no index and still execute their disabled-note action. Direct children and the aggregate select from the same definitions and use the same prepare-bind-execute path.

8. Preserve the three source capabilities as distinct typed values: working project/filesystem inputs for drift and advisories, the working snapshot owned by current-state evaluation, and the stage-0 index for prose and memory. No common tree bag or fallback crosses them. Scanner-only selections continue to use the foundation's config directly rather than opening a Project, and disabled scanners continue to avoid Git entirely. The prepared-config loader is a one-operation injected dependency, not a second project-opening path or a cache on `Project`; direct standalone project operations keep their existing config-loading entry points.
...
12. Author all five added claims in their application transactions with this ADR as `Origin`, `Backing: test`, and matching proof annotations in the files selected by their owning topics. The three `code-design/execution-planning` claims are proven by focused `internal/execution` tests covering selection, both validation barriers, exactly-once preparation, ordering, binding, failure policy, cancellation, and structured results. `tooling/cli:repo-check-capability-plan` is proven by command integration tests covering direct and aggregate selection, one project open from the prepared config, one index capture, conditional no-Git behavior, universe isolation, stable output, zero-action preparation failure, and continued execution after action errors. `rendering/project-output-plan:check-report-single-plan` is proven in `internal/project` by one-output-plan behavior and a structural check preventing `CheckReport` consumers from reconstructing it. Update the exact project-open and operation-state structural baselines. Introduce no package-global test seam.
