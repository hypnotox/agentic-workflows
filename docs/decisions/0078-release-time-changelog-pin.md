...
retires_invariants: []
superseded_by: ""
tags: [changelog, release, gate, tooling]
related: [41, 49, 73]
domains: [tooling]
---
...
2. **The exact pin moves to the release gate.** A repo-local `cmd/releasecheck` (the
   ...
   (a) the newest entry's version equals `project.Version` and (b) the `[Unreleased]`
   section's body is empty after `strings.TrimSpace` — the standing header with surrounding
   blank lines counts as empty. Since the parser discards `[Unreleased]` by design, the
   emptiness half reads the raw embedded bytes with its own section walk (prior art:
   `repoaudit`'s `unreleasedSection`, which is `gitFunc`-bound and not reusable).
...
3. **Changelog promotion becomes a release-time act.** `[Unreleased]` accumulates entries all
   ...
   tagging. `docs/releasing.md` step 2 and the pitfalls entry describing the forced mid-cycle
   promotion are updated/retired accordingly; a mid-cycle `project.Version` bump (ADR-0049
   Decision 4 is unchanged) now touches only the const and the lock.
...
- `inv: release-changelog-pin` — `cmd/releasecheck` exits non-zero unless the newest embedded
  changelog entry equals `project.Version` and the `[Unreleased]` section is empty modulo
  whitespace, and the Release workflow runs it before building artifacts.
