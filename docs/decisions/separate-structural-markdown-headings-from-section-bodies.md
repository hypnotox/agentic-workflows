...
2. `decision: heading-body-assembly` A surviving section assembles in the order edit pointer, optional structural heading, then the winning body source. Template defaults, convention parts, section-default reinjection, and in-place read-back replace or preserve only the body. A dropped section emits none of its pointer, heading, or body. Structural markers remain consumed and absent from rendered output.
