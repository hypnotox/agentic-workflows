...
1. **The changelog-conformance verdict becomes a Warning.** In `cmd/repoaudit`, the `changelogRule`
   ...
   blocks the review. This **partial-supersedes ADR-0073 Decision item 2** (`refines: ADR-0073#2`; "emit one
   Error finding") and **item 4** (`refines: ADR-0073#4`) ("Error blocks the review from concluding") for this rule only; ADR-0073 stays
