...
2. `decision: registered-worktree-boundary` An explicit verification checkout resolves relative to
   ...
   aliases. It must identify either the project root or an existing registered linked worktree of
   that repository. Empty, missing, non-Git, foreign-repository, and unregistered identities refuse
   before child dispatch with an actionable diagnostic. Registered identity discovery is NUL-safe
   so valid worktree paths are not reinterpreted by display-oriented parsing.
...
4. `decision: explicit-caller-selection` Parent-facing Pi workflow guidance selects
   ...
   cross-runtime implementer role contract gains no Pi-only checkout metadata or routing duty.
...
must enumerate and canonicalize registered worktrees before explicit dispatch, adding Git and
filesystem validation to the parent-side precondition. This decision does not bind ordinary
mutations to an effort worktree, prove the child's reported commit hash, or strengthen the existing
clean-tree and commit-parent policy.
...
| Accept any Git checkout path | Permits a typo or unrelated clone to become the monitored authority instead of constraining verification to this repository's registered worktrees. |
