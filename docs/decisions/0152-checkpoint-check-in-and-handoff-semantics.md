...
8. Working memory remains optional for small, checkpoint-less efforts. A non-trivial brainstormed effort becomes memory-backed when its first settled decision is persisted; from that point its boundaries carry the protocols above. This trigger's canonical text lands in the updated `rendering/workflow-skill-templates:memory-checkpoint-chain-coverage` claim so it survives as current-state authority.

9. Implementation sequences against the post-ADR-0149 architecture: the governed Pi workflow loader, the `direct` route, and protocol-2 phase transitions are the surfaces this decision's Pi guidance lands on, and files under concurrent ADR-0149 implementation are not modified until the relevant 0149 batches are on the base branch. The same serialization applies to the claim operations: this decision's updates to `rendering/workflow-skill-templates:memory-checkpoint-chain-coverage`, `rendering/pi-workflows:pi-session-handoff-workflow`, and `rendering/pi-workflows:pi-session-handoff-lifecycle` are Applied only after ADR-0149's pending updates to those same claims are Applied.

10. Documentation travels with the change: the rendered workflow doc's checkpoint prose, the agent guide's working-memory section, and the rendering domain overview's checkpoint narrative are rewritten to the two-protocol semantics in the same implementation batches as the claims they narrate, so no rendered surface keeps calling a routine summary an intervention point.

11. Tests classify skills by boundary class. Mandatory-boundary tests prove the explicit approval request, the stop, post-approval memory persistence, and only then target-native continuation. Routine tests prove the attention branch stops with a check-in while the clear branch continues without ending the turn. Implementation-skill tests scope their assertions to the per-task checkpoint sections so a distant terminal handoff cannot satisfy them. The added `rendering/workflow-skill-templates:mandatory-approval-boundaries` claim is an invariant with `Backing: test`, proven by the mandatory-boundary tests above. Rendered Pi and non-Pi outputs, current-state backing, publication-safe empty-variable rendering, sync and drift checks, staged checks, and the gate are all required.
...
The Pi handoff extension widens: the post-queue recovery path adds visible failure UI, editor recovery content, and matching deterministic tests. Non-Pi targets change only in prose. Embedding the complete routine protocol at every per-task checkpoint section duplicates that text across the rendered implementation skills; the single shared partial keeps authoring centralized, but the rendered surface still grows at each point of use. ADR-0145 stays frozen history; this decision updates the claims it established rather than editing it. Implementation is serialized behind the concurrent ADR-0149 work on the shared Pi workflow surfaces, which delays landing but avoids cross-effort file conflicts.
...
- 2026-07-23: Applied; operations: add `rendering/workflow-skill-templates:mandatory-approval-boundaries`, update `rendering/workflow-skill-templates:memory-checkpoint-chain-coverage`, update `rendering/pi-workflows:pi-session-handoff-workflow`
- 2026-07-23: Applied; operations: update `rendering/pi-workflows:pi-session-handoff-lifecycle`
- 2026-07-23: Implemented; content-sha256: c62ee8b393fb06f3921cef27340588b1e7dfed269ee62db39d9ad798747db982
