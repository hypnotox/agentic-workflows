...
5. `decision: fixed-guide-budgets` awf adopts three fixed, non-configurable byte bounds: a representative shipped guide remains at or below 8 KiB, this repository's self-hosted guide remains at or below 10 KiB, and ordinary aggregate `awf check` emits a structured advisory when an adopter's deterministic expected managed `AGENTS.md` exceeds 12 KiB. The advisory reports observed and allowed bytes, does not change a warning-only zero exit, includes adopter overrides and data, and is absent when the agents document is local because awf neither renders nor owns it.
6. `decision: simple-budget-observation` Guide size is observed on the already-produced expected output and presented through the existing check advisory boundary. awf does not add configurable thresholds, section or provenance attribution, generic document metrics, a new diagnostics framework, or compatibility plumbing for the removed roster; those mechanisms would cost more conceptual surface than this bounded prose signal warrants.
