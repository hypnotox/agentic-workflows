...
4. Let a group child declare a gating classification weaker than its parent's, and have the driver honour the child's. `check` is `Gated`; `check prose`, `check memory`, and `check commit` stay `Ungated` so a hook keeps invoking them without a version gate. `TestGroupChildrenCarryNoGating` is replaced by a test asserting the resolved gating, and the driver reads gating from the resolved node rather than from `top`. A child that sets no gating continues to inherit its parent's, so `metrics` and `new` are unaffected.
   ...
   `GatedCommandNames()` projects the gated top-level commands plus only those children whose resolved gating **differs** from their parent's, so the inherited-gated children of `new`, `metrics`, and `check` never enter the list. Today it returns thirteen names. After this decision the gated top-level set is twelve, because `invariants` is no longer top-level and `sync` is spelled `render`: `render`, `check`, `audit`, `metrics`, `doctor`, `list`, `config`, `context`, `topic`, `new`, `enable`, `disable`. The three differing children (`check prose`, `check memory`, `check commit`) are rendered as named exclusions beside that list rather than as members, because they are the only per-child facts a reader needs and they subtract from `check` rather than adding to the set. The `gated-commands-generated` update is worded to that rule.
...
- `gated-commands-generated` stops being a top-level-only projection without the published list bloating: Decision 4 admits only children whose resolved gating differs from their parent's, so the agent guide shows twelve gated top-level names plus three named exclusions rather than a dozen inherited entries. The literal list lives in `internal/clispec/clispec_test.go`'s `TestGatedCommandNames`, which pins today's thirteen names and fails until it is edited; the marker-carrying `TestGatedCommandsDisplay` derives its expectation from `GatedCommandNames()` and would pass unchanged, so the literal test is the one an implementer must update.
