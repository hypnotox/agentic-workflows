...
10. Do not rewrite retained history: `docs/decisions/**`, `docs/plans/**`, and the released version sections of `changelog/`. A retained record naming `awf sync` states what the command was called when that decision was made, and a released changelog entry states what shipped under that version; rewriting either falsifies it. The changelog's `## [Unreleased]` section is not retained history and does gain an entry for this break. This ADR is the forward correction; the current-state corpus carries the present names.
