...
2. **Two ingestion seams, and only those two.** The strip runs (a) over template source
   ...
   an authoring comment does not count as present. `TemplateHash` and the part-byte
...
3. **Fences preserve; unclosed openers fail.** Inside a fenced code block, directive
   lines (and unclosed openers) are preserved verbatim, so parts and templates can
   demonstrate the syntax. Outside a fence, a whole line beginning with the directive
   opener but lacking its same-line `-->` is a hard render error naming the source
   (template id or part path): the directive is new and namespaced, no legacy corpus
   exists, an unclosed opener has no legitimate use, and passing it through would leak.
...
- `invariant: authoring-comment-unclosed-fails`, a whole line outside a fence that opens
  with the directive literal but does not close with `-->` on the same line fails the
  render with an error naming the source; inside a fence it is preserved.
...
- `invariant: parts-raw-except-authoring-comments`, a convention part body is never
  passed through `text/template` and appears verbatim in rendered output, except that
  whole-line `awf:comment` lines are removed at ingestion (successor to `parts-raw`,
  ADR-0034).
...
- Downstream doc obligations for the implementing effort: `docs/working-with-awf.md`
  ...
  `docs/architecture.md` render-flow note, the rendering domain narrative, and the
  glossary (authoring comment).
