...
1. **The agent-guide `## Invariants` section holds only cross-cutting, every-change rules.** A rule belongs iff a commit could violate it *no matter which files it touches* — process, gate, and commit-hygiene rules, the flagship rendering guarantee, and the meta-rule about how invariants themselves are declared and backed. Every path- or subsystem-specific invariant is **contextual**: it stays authoritative in its owning ADR and is reached on demand — via `awf context` and the ACTIVE.md index where a project configures domains, or the ADR index otherwise.
...
Textual contracts (no machine-checkable slug; this is a guidance decision that adds no gate):

- The agent-guide `## Invariants` section lists only cross-cutting, every-change rules; a path- or subsystem-specific invariant lives in its owning ADR, not the guide.
