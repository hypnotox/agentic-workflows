...
An `awf effort` command asks native Git for the invoking checkout, absolute common directory, and authoritative primary checkout. The invoking checkout remains tracked configuration authority, while the primary checkout's confined `.awf/efforts/`, `.awf/memory/`, and `.awf/assignments/` roots hold repository-wide local orchestration state. New allocates a lowercase UUIDv4 and creates normalized memory by default; rename and lifecycle operations atomically replace one schema-1 record under the repository lock; and repair changes only confined filesystem facts it can prove. The separate schema-1 assignment authority maps each bounded safe Pi session ID to exactly one effort ID. `effort assign <id> --session <session>` explicitly creates or reassigns that current association, including for terminal efforts, `unassign` removes it, and `assignments` lists it in session order. These operations atomically replace only the assignment map, never change effort lifecycle state or create adoption history; list and show derive sorted session IDs from it. Missing or malformed resident state is refused without rewriting the evidence. No effort record governs tracked project truth.
...
an atomic staging rename. Protocol 2.1 readers recognize `effort_created`, `effort_adopted`, and
`detour_started` as the complete creation-kind set while preserving protocol-2.0 history without a
rewrite. Readers retain malformed or illegal evidence without applying its state effect, but an
