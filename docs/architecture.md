...
- `internal/audit` owns one immutable historical operation per invocation: one collected range, commit and aggregate rules, live cleanliness, transition replay, stale-merge replay, and a shared revision-state and error cache. Its first implementation derives those states from complete committed snapshots; later pipeline phases narrow the representation.
