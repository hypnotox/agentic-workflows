...
two-extension-factory runtime seam that proves real-request context-line refresh after active-branch compaction, and the pinned repository-runtime cache and launcher tests. There is no slower tier to reach for; the whole gate is fast enough to run before every commit. `./x gate timings` runs that identical sequential transaction while reporting each stage's elapsed wall time. The release-only real interactive Pi smoke remains the manual unbacked verification documented in the test layout; it is not mislabeled as a deterministic gate tier.
