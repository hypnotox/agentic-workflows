...
| `currentState.sources` | list of {globs, marker, close} mappings | none | 1 sources | A current-state source family is scanned for named invariant proof markers during topic validation. | Consumed by current-state topic validation and the staged check. |
...
| `currentState.testGlobs` | string list | none | 1 globs | Duplicate-free anchored path globs identifying proof-eligible test files for current-state invariant claims. | Consumed by current-state topic validation and the staged check. |
