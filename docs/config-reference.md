...
| `audit.allowedScopes` | list of scope entries (bare string, or {name, meaning}) | accept any scope | 7 scopes | The project's Conventional Commits scope taxonomy: the single home for commit scopes; rendered prose quotes it from here. Absent = accept any scope; entries are enforced by `awf check staged commit`/`awf audit` and editing them reflags referencing rendered artifacts. | Read by `awf check staged commit`, `awf audit`, and every rendered artifact quoting the scope list. |
