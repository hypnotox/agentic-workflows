...
| `currentState.maxClaimsPerTopic` | positive integer | 20 | 20 | Advisory maximum number of current-state claims in one topic before awf check emits a non-failing split note; it never truncates query output. | Consumed by the working-tree current-state check; the staged check does not emit this advisory. |
| `workflowTelemetry.retention.maxCompletedEffortAgeDays` | non-negative integer | 90 | 90 | Maximum age in days for terminal effort metrics; zero disables age-based retention. | Consumed by workflow metrics retention. |
| `workflowTelemetry.retention.maxCompletedEffortCount` | non-negative integer | 100 | 100 | Maximum number of terminal efforts retained repository-wide; zero disables count-based retention. | Consumed by workflow metrics retention. |
| `workflowTelemetry.widget.enabled` | bool | true | true | Shows the compact active-effort Pi widget when the dashboard target is installed. | Consumed by the Pi workflow dashboard. |
| `workflowTelemetry.widget.showCost` | bool | true | true | Shows cost in the compact widget without changing collection or export. | Consumed by the Pi workflow dashboard widget. |
| `workflowTelemetry.diagnostics.heuristicsEnabled` | bool | true | true | Enables heuristic workflow findings; exact diagnostics remain enabled when false. | Consumed by workflow diagnosis. |
| `workflowTelemetry.diagnostics.minimumBaselineSamples` | positive integer | 10 | 10 | Minimum comparable completed-effort sample count required for historical heuristic baselines. | Consumed by workflow diagnosis. |
| `workflowTelemetry.diagnostics.baselinePercentile` | integer from 1 through 100 | 95 | 95 | Nearest-rank percentile used for historical heuristic baselines. | Consumed by workflow diagnosis. |
| `workflowTelemetry.diagnostics.thresholds.phaseReentryCount` | positive integer | 2 | 2 | Inclusive absolute threshold for reentries into a named phase. | Consumed when heuristic diagnostics are enabled. |
| `workflowTelemetry.diagnostics.thresholds.phaseDurationSeconds` | positive integer | 14400 | 14400 | Inclusive absolute threshold for phase duration in seconds. | Consumed when heuristic diagnostics are enabled. |
| `workflowTelemetry.diagnostics.thresholds.phaseTokens` | positive integer | 200000 | 200000 | Inclusive absolute threshold for tokens used in one phase interval. | Consumed when heuristic diagnostics are enabled. |
| `workflowTelemetry.diagnostics.thresholds.compactionCount` | positive integer | 3 | 3 | Inclusive absolute threshold for compactions in one effort. | Consumed when heuristic diagnostics are enabled. |
| `workflowTelemetry.diagnostics.thresholds.handoffCount` | positive integer | 3 | 3 | Inclusive absolute threshold for handoffs in one effort. | Consumed when heuristic diagnostics are enabled. |
| `workflowTelemetry.diagnostics.thresholds.toolFailureCount` | positive integer | 3 | 3 | Inclusive absolute threshold for categorized tool failures in an implementation segment. | Consumed when heuristic diagnostics are enabled. |
| `workflowTelemetry.diagnostics.thresholds.gateFailureCount` | positive integer | 2 | 2 | Inclusive absolute threshold for failed gates in an implementation segment. | Consumed when heuristic diagnostics are enabled. |
| `workflowTelemetry.diagnostics.thresholds.cacheReadPercentBelow` | integer from 0 through 100 | 10 | 10 | Inclusive lower threshold for cache-read percentage when the token denominator is positive. | Consumed when heuristic diagnostics are enabled. |
| `workflowTelemetry.diagnostics.thresholds.subagentQueueWaitSeconds` | positive integer | 60 | 60 | Inclusive absolute threshold for one subagent queue wait in seconds. | Consumed when heuristic diagnostics are enabled. |
| `workflowTelemetry.diagnostics.thresholds.implementationReworkCount` | positive integer | 2 | 2 | Inclusive absolute threshold for returns to implementation after implementation review. | Consumed when heuristic diagnostics are enabled. |
...
| `runner.enabled` | bool | false (key absent); awf init and awf upgrade seed it true | n/a | Renders the pure awf wrapper `awf` at the repo root: a fully awf-owned forwarder that execs the configured (`awfInvokeCmd`) or bootstrap-resolved awf with all arguments forwarded verbatim. Absent and false both mean: do not render. | Always. |
