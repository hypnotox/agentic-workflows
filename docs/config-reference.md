...
| `docs` | string list | empty at init (the always-on docs are not listed here) | 6 enabled | Enabled toggleable docs (architecture, testing, development, ...). The always-on docs (the agent guide, workflow, this reference) render regardless and are not listed here. | Always. |
| `domains` | string list | none | 5 configured | Freeform domain keys. Each renders a generated `<docsDir>/domains/<name>.md` doc (its per-domain decision index plus your `current-state` convention part) and can declare a file territory via the domain sidecar's `paths:`. | Always. |
| `targets` | string list | claude | `claude`, `cursor` | Enabled adapter runtimes. Skills and agents render once per target into that runtime's layout; docs are runtime-neutral and render once. | Always. |
