...
| `contextIgnore` | string list | none | 15 patterns | Anchored doublestar globs for tracked paths that context and coverage should exclude (config source, docs, top-level non-code files). Matching paths are ineligible for directory expansion and coverage, including staged queries, alongside awf's own generated outputs. An empty or absent list adds no exclusion. | Always; consulted by working and staged context path expansion and coverage. |
