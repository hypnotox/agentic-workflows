...
- `awf context <path>...`: report topic-grouped current-state orientation: each applicable topic renders once per invocation with its selectors, matched-path count and `--coverage` drilldown, uncapped claim-ID roster, and the full detail of claims directly selected by exact-path state/touches/proof markers (with a detail-omission line when any rostered claim's detail is not shown), while effective paths carry classification, owning domains, topic attribution, and known-artifact navigation. Every effective path has one of the closed primary classifications `covered`, `eligible-unowned`, `context-ignored`, `generated-output`, `nested-adopter`, `symlink`, `not-found`, or `outside-repository`; classification does not erase safely matchable ownership, except that a user-typed query containing glob metacharacters that resolves to a missing or context-ignored path reports no ownership: globs are never expanded, and the text rendering says so. Use `awf context --full <path>...` for every applicable current claim, backing and `Verify` contract, marker site, direct reference ID, and pending operation, once per topic; neither projection recursively expands references or unrelated ADR history. Explicit ADR paths report lifecycle-derived operation progress and label prose as pending intent or decision history, never current authority. `--json` serializes only the selected projection for machine consumption (prefer the text form when reading), so concise JSON has no full block. Both forms are read-only and support `--staged` or `--range <a>..<b>`. `--uncovered` retains the coverage-gap report and refuses `--full`; `contextIgnore` continues to exclude matching paths from directory expansion and coverage.
