...
First adoption seals `initializedWithVersion` plus the next ADR identity and gaps in the lock. Older migrated adopters legitimately omit initialization provenance while retaining their permanent cutoff and explicit gaps; sync and forced re-init preserve either encoding rather than backfilling history.
