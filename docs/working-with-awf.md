...
- `awf context <path>...`: report request-oriented impact. Exact files and sorted Git selections remain individual; each directory reports included/excluded counts and groups descendants by complete semantic context, disclosing members only through three. Default authority contains bounded topic, applicable-invariant, direct-rule, and pending summaries once globally. Repeat `--show` with `all-rules`, `evidence`, `selectors`, `references`, `pending`, or `artifacts`; `--full` is their union and never restores a census. `--json` is removed. Human output through 8,192 bytes writes unchanged; larger normal, static, or uncovered reports securely spill exact bytes outside the repository and return a two-line notice whose file the successful caller must delete.
...
vars are set, what consumes them, and what enabling would activate. `currentState.maxClaimsPerTopic`
is a positive topic-size advisory with an effective default of 20: working-tree `awf check` prints a
non-failing note for a topic strictly above it, while equality is quiet, staged checks suppress the
...
project with no permanent cutoff is unattested and cannot cross this destructive boundary. After the
...
cancellation. Long implementations should favor sequential
