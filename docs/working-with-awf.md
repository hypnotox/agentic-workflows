...
- `awf add <kind> <name>` / `awf remove <kind> <name>` — toggle a skill, agent, doc, domain, target, bootstrap, or hooks artifact. For a skill/agent/doc, add enables the full requirement closure in one edit and remove refuses while enabled artifacts still require the target — `--with-dependents` removes them together, `--dry-run` previews either plan without changing the config (ADR-0081).
