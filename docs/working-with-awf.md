...
Pi association stays at repository root. `using_effort` directly attaches with an effort slug or detaches with `{detach:true}` and never changes CWD or transfers a conversation. Attached model calls receive the relative owned memory path and, when the fixed directory exists, the managed-worktree path. Activity and Remote Pi metadata or temporary naming are advisory only, never authority or a lock; restart begins detached and local lifecycle degradation is silent.
