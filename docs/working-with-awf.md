...
<!-- awf:template-source templates/docs/working-with-awf.md.tmpl -->
## Commands
Workflow judgment precedes this command: discovery creates no effort, and `effort-workflow` initiates first creation only when durable continuity materially helps by selecting a faithful outcome, title, and canonical short slug. Existing efforts resume only for work inside their outcome; a new outcome cannot silently reuse or replace an active effort. The CLI does not verify workflow judgment. For a continuity-backed outcome, run `./awf effort new --slug <slug> "<title>"`; the caller-supplied immutable 1-through-32-byte slug identifies `.awf/efforts/<slug>/state.json`, its always-owned `.awf/efforts/<slug>/memory.md`, optional opaque `.awf/efforts/<slug>/scratch/`, `.awf/worktrees/<slug>/`, and the `awf/<slug>` branch. Creation never scaffolds or manages scratch, and awf validates only that direct child as a safely owned real directory without traversing descendants. Existing residents through 63 bytes remain selectable and finishable. New, active-only list, and active-only show use readable text only; activity remains the JSON-only protocol. Creation makes the managed worktree by default (`--no-worktree` opts out; `--base <ref>` selects the base); flags may appear before or after the single outcome-title positional, and `awf effort worktree add <slug>` remains the standalone operation for efforts created without one. Git topology, not effort state, owns integration and removal facts. After every managed path, registration, and branch is absent, finish moves the complete unchanged resident to `.awf/effort-archive/<uuid>-<slug>`, releases the slug, and reports the exact path. Before the archive move, retry by slug is safe; after the move, any durability uncertainty directs inspection of the reported source and destination instead of blind retry. The ignored archive is local, unmanaged, non-authoritative, manually disposable, and potentially present in backups or local disclosure; awf provides no archive inventory, restore, prune, analysis, or retention command.

Pi's `handoff_session` accepts exactly `{kickoff}`: bounded prose for a replacement session. For effort-backed work, the kickoff is exactly `Continue with effort <slug>.`; skill and effort authority own association, reorientation, boundary logging, and autonomous continuation. The runtime only replaces the session without selecting an effort or mutating lifecycle state. A matched replacement sends one visible default-rendered `agent-handoff` custom message with `Agent-authored handoff context; this is not user input:` followed by two newlines and the unchanged kickoff, using `triggerTurn:true`; Pi's current provider adapter still supplies that custom content in a user-role message. After it queues the continuation, it emits Remote Pi's optional continuation disposition so a compatible push integration does not report the parent run as terminal; an absent or failing listener never blocks the handoff.

`./awf new doc <name> <description> [--title <title>]` declares, renders, and reports `docs/<name>.md`. It accepts exactly the name and one-line description positionals plus one nonrepeatable `--title` value. Without `--title`, awf derives a title from the final kebab-case name segment (`api-v2` becomes `Api V2`); use the flag for acronyms or specialized spelling. Edit the rendered document body, then run `./awf check`; remove its declaration and render to recover the complete old document from its `.awf-bak` sibling, and the same recovery occurs during uninstall.
