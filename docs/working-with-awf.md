...
First adoption seals `initializedWithVersion` plus the next ADR identity and gaps in the lock: cutoff 1 for an empty decision corpus, or highest-plus-one with explicit lower gaps for validated brownfield history. Older migrated adopters legitimately omit initialization provenance while retaining their permanent cutoff and explicit gaps; sync and forced re-init preserve either encoding rather than backfilling history. An older adopter with neither permanent authority nor a bridge attestation is refused before mutation.
