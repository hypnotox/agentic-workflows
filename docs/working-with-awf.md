...
`awf context internal/project/context.go` is a representative concise orientation query: it attributes each effective path, shows directly marked claims, and names omitted topic-wide claims with drilldowns. Use `awf context --full internal/project/context.go` whenever every applicable authority claim is required. `awf topic tooling/cli --coverage` drills into the owning-domain and topic selectors, their both-must-match rule, current matched paths, and marker sites. `--json` serializes the same selected projection; concise JSON omits the full block. Explicit ADR paths show lifecycle-derived operation progress without treating ADR prose as current authority. Run `./x` with no command to print the metadata-derived forwarded awf verbs alongside the project-owned runner verbs.
