...
case "$command" in
  ...
  fmt)
    ...
    ;;
  render|check|resolve|docs|effort|adr|plan|version)
  ...
  install)
    ...
    ;;
  *)
    echo "usage: ./x <gate|test|lint|fmt|render|check|resolve|docs|effort|adr|plan|version|build|install> [args]" >&2
    exit 2
    ;;
esac
