...
This skill may run without an effort. If continuity independently fired, validate the fixed effort identity and exact `.awf/efforts/<slug>/memory.md`, preserve one user-managed writer, and use that context read-only for children; otherwise omit effort and memory fields. If changed facts newly make continuity useful, invoke `awf-effort-workflow` before further mutation. Repository sources and current-state documentation remain authoritative, and standalone memory is forbidden.
