...
<!-- awf:template-source templates/skills/subagent-driven-development/SKILL.md.tmpl#positioning -->
...
<!-- awf:template-source templates/skills/subagent-driven-development/SKILL.md.tmpl#per-task-review-note -->
...
<!-- awf:template-source templates/skills/subagent-driven-development/SKILL.md.tmpl#when-to-invoke -->
<!-- awf:edit when-to-invoke: default; create .awf/skills/parts/subagent-driven-development/when-to-invoke.md to override -->
<!-- awf:template-source templates/skills/subagent-driven-development/SKILL.md.tmpl -->
## When to invoke

Invoke when a plan phase benefits from delegated implementation. A plan may mix modes; delegated execution never delegates transaction ownership, so hand inline phases to `awf-executing-plans`.

<!-- awf:template-source templates/skills/subagent-driven-development/SKILL.md.tmpl#procedure-resolve-plan -->
...
<!-- awf:template-source templates/skills/subagent-driven-development/SKILL.md.tmpl#procedure-raise-concerns -->
...
<!-- awf:template-source templates/skills/subagent-driven-development/SKILL.md.tmpl -->
4. For a `subagent-driven` phase, the parent verifies a known clean and green baseline. A fresh parent may run `./awf read plan <plan> <P>` for the executable closure. A task projection's generated scope notice, Phase close, and Advances/Completes outcomes are phase-owner context only; they never transfer commit, review, checkpoint, handoff, helper, or outcome authority; projection changes neither ownership nor checkpoint boundaries. Call one commit-disabled implementation child alone in its parent tool batch Then provide the complete phase, explicitly identify the parent-supplied approved boundary, and, when an effort exists, provide its slug and exact owned memory path. The child is the `implementer` agent and returns its closed focused-evidence receipt; it never edits parent memory. Choose the smallest reliable tier - `small` (narrow, mechanical), `standard` (substantive but bounded), or `large` (broad, intricate, cross-cutting, or high-consequence) - escalating after uncertainty, failed reasoning, or widened scope; select the smallest reliable target-native model explicitly, or use the harness default and note in the dispatch brief that explicit selection is unavailable. Full tier definitions: docs/workflow.md. The parent independently inventories the checkout, rejects writes outside the delegated boundary, confirms the reported unchanged HEAD and end state, and integrates the result. The parent stages the complete transaction explicitly and creates the declared phase-closing commit only after `./awf check staged` and `./x gate` pass; a wired pre-commit hook enforces both at commit time, and the parent runs them manually first only in a clone without wired hooks (checkable with `git config core.hooksPath`; when in doubt, run both manually). The parent also owns terminal exhaustive verification. A child receipt never substitutes for a parent gate. Never dispatch concurrent implementation children.
<!-- awf:template-source templates/skills/subagent-driven-development/SKILL.md.tmpl#procedure-status-handling -->
...
<!-- awf:template-source templates/skills/subagent-driven-development/SKILL.md.tmpl#per-task-review -->
...
<!-- awf:template-source templates/skills/subagent-driven-development/SKILL.md.tmpl#final-task-adr-flip -->
...
<!-- awf:template-source templates/skills/subagent-driven-development/SKILL.md.tmpl#red-flags -->
