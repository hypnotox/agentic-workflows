...
5. Before review, verify that the child worked in the explicitly intended worktree, confirm its branch, resolve the reported commit as that branch tip, and require `git status --short` to be empty. Then dispatch one report-only phase review. Choose the smallest reliable tier - `small` (narrow, mechanical), `standard` (substantive but bounded), or `large` (broad, intricate, cross-cutting, or high-consequence) - escalating after uncertainty, failed reasoning, or widened scope; select the smallest reliable target-native model explicitly, or use the harness default and note in the dispatch brief that explicit selection is unavailable. Full tier definitions: docs/working-with-awf.md. The parent owns focused settlement commits, remains the memory writer, and checkpoints only after findings resolve; the report-only reviewer never edits memory.
