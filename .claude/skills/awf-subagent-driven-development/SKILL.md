...
<!-- awf:template-source templates/skills/subagent-driven-development/SKILL.md.tmpl -->
4. For a `subagent-driven` phase, the parent verifies a known clean and green baseline. A fresh parent may run `./awf read plan <plan> <P>` for the executable closure. A task projection's generated scope notice, Phase close, and Advances/Completes outcomes are phase-owner context only; they never transfer commit, review, checkpoint, handoff, helper, or outcome authority; projection changes neither ownership nor checkpoint boundaries. Call one commit-disabled implementation child alone in its parent tool batch Then provide the complete phase, explicitly identify the parent-supplied approved boundary, and, when an effort exists, provide its slug and exact owned memory path. The child is the `implementer` agent and returns its closed focused-evidence receipt; it never edits parent memory. The receipt identifies assigned scope, canonical checkout, starting and ending HEAD and `git status --short`, changed paths, each exact focused command with cwd, argv, exit status, and actual result, completed and remaining work, deviations, separately routed blockers, and applicable generated-output or fixture evidence. Choose the smallest reliable tier - `small` (narrow, mechanical), `standard` (substantive but bounded), or `large` (broad, intricate, cross-cutting, or high-consequence) - escalating after uncertainty, failed reasoning, or widened scope; select the smallest reliable target-native model explicitly, or use the harness default and note in the dispatch brief that explicit selection is unavailable. Full tier definitions: docs/workflow.md. The parent independently inventories the checkout, rejects writes outside the delegated boundary, confirms the reported unchanged HEAD and end state, and integrates the result. It reuses focused evidence only while checkout identity, relevant authority, and overlapping paths remain unchanged; lost or unverifiable evidence, changed relevant authority, or later overlapping mutation invalidates the affected receipt. The parent stages the complete transaction explicitly and creates the declared phase-closing commit only after `./awf check staged` and `./x gate` pass; a wired pre-commit hook enforces both at commit time, and the parent runs them manually first only in a clone without wired hooks (checkable with `git config core.hooksPath`; when in doubt, run both manually). The parent also owns terminal exhaustive verification. A child receipt never substitutes for a parent gate. Never dispatch concurrent implementation children.
