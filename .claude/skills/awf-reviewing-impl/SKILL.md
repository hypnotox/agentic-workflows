...
8. **Route settled terminal review through optional worktree integration.** Repository sources and current-state documentation remain authoritative over checkpoint prose; standalone memory is forbidden.
   ...
   - Only after the applicable terminal review has zero findings, land the deferred flip transaction: apply any final V2 batch with exactly its claim mutations and flip the linked ADR(s) and plan to `Implemented` per `awf-adr-lifecycle`.
