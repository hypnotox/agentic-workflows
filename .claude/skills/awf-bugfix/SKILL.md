---
name: awf-bugfix
description: Use when applying a bug fix in awf with a known root cause. Establishes the strongest practical durable oracle, fixes the root cause, verifies via the gate, commits, and evaluates review need. Self-contained.
---
...
1. **Establish the oracle before the fix.** Select evidence under the durable-oracle rule above and record any concrete reason the preferred automated red-then-green path is impractical. Invoke `awf-tdd` to establish and exercise that evidence before implementation. First run `./awf context <the implementation and verification paths>`.
