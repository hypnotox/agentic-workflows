...
- **Batch tasks:** retain an exact representative, exact edge unless identical everywhere, exhaustive affected-site set, and deterministic post-check. Optional helper partitions assign every site to the parent or exactly one helper; helper subsets are exhaustive and path-disjoint, shared files remain parent-owned, and focused mutating commands are command-confined to the assigned subset. Helpers are sequential and commit-disabled.
