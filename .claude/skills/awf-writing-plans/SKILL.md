...
- **Gate cost:** `go test ./... && go vet ./...` (~~10s) runs on every code-touching commit. Batch closely-related same-shape changes that share one rationale into a single commit; keep genuinely independent concerns separate. Docs-only commits outside the decisions directory skip the gate automatically. See `AGENTS.md` "Commit granularity vs gate cost".
