...
**section-taxonomy**: the plan follows plan-v1: `format: plan-v1` plus `date`/`adrs`/`status` frontmatter, a `# Plan:` H1, nonempty Goal and Architecture summary, sequential phase and task headings, exact execution modes, one final Phase close and commit fence per phase, required Definition of done plain bullets, and optional Notes; File structure, Verification, and task checkboxes are not plan-v1 declarations
