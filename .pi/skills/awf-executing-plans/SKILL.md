...
**Runtime detail.** When operations intentionally target a supplied managed-worktree path, pass that path as `verificationCheckout` and keep actual mutation paths explicit in the task. The validated checkout is both the implementation child CWD and commit-policy identity. Omit `verificationCheckout` for root work intentionally. Parent mutations still name the managed-worktree path explicitly because child CWD is not filesystem confinement; parent and child Pi CWD do not become a shared session CWD. Parent and child Pi CWD do not become a shared session CWD. Choose the smallest reliable tier - `small` (narrow, mechanical), `standard` (substantive but bounded), or `large` (broad, intricate, cross-cutting, or high-consequence) - escalating after uncertainty, failed reasoning, or widened scope; omit the `model` field to use configured role routing, overriding deliberately with the tier's exact `provider/model-id`. Never pass `default`, `auto`, or `inherit parent` as a model value. Full tier definitions: docs/workflow.md.
