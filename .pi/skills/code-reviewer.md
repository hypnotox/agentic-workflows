...
**invariant-proof-exercises-its-claim**: an `invariant:` proof marker must exercise the whole claim its slug states, not one instance of it: try mutating each clause the slug asserts and confirm the test fails. Two markers in the ADR-0127 session were nominal rather than real (`audit-empty-range-clean` sat on a merge-commit test that never built an empty range; `audit-reports-evaluated-scope` claimed every verdict but asserted only the clean line, so stripping the scope suffix from the warning and error paths left the suite green). A marker that cannot fail is worse than none: it reads as backing while checking nothing
