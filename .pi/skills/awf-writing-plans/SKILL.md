...
- If planning surfaces a new load-bearing (ADR-worthy) decision, pause: invoke `awf-proposing-adr` (or amend a still-`Proposed` ADR via `awf-adr-lifecycle`), let it settle, then re-review the linked plan before implementation.
