...
3. Treat that summary as the user's intervention point. In the next tool batch, invoke `handoff_session` alone with the exact memory path and a kickoff that states the immediate successor action. Continue automatically in the fresh session unless the user cancels during the five-second window. The file skeleton and ground rules live in the agent guide's working-memory section.
