...
- The ADR stays `status: Proposed` through the implementation sequence. It flips to `Accepted` (design final, implementation follows) or directly to `Implemented` (design and implementation land together) in a later commit; that is handled by `awf-adr-lifecycle`, not this skill. The claim operations apply only in the `Implemented` transaction.
