...
<!-- awf:template-source templates/skills/subagent-driven-development/SKILL.md.tmpl -->
4. For a `subagent-driven` phase, verify a known clean and green baseline. A fresh phase owner may run `./awf read plan <plan> <P>` for the executable closure. A task projection's generated scope notice, Phase close, and Advances/Completes outcomes are phase-owner context only; they never transfer commit, review, checkpoint, handoff, helper, or outcome authority; projection changes neither ownership nor checkpoint boundaries. Call one implementation child alone in its parent tool batch with `allowCommits: true`. Effort-backed pre-integration implementation dispatches supply the managed-worktree path as `verificationCheckout`, while effort-free or root-owned work intentionally omits it. The validated checkout is both the child CWD and commit-policy identity. The main Pi session remains at the project root, so parent mutations name managed-worktree paths explicitly. Child CWD alignment is not filesystem confinement. Governed primary-checkout lifecycle transitions remain at the primary checkout. Then provide the complete phase, explicitly identify the parent-supplied approved boundary, and, when an effort exists, provide its slug and exact owned memory path. The child is the `implementer` agent, whose rendered contract already governs its authority mode, scope discipline, green obligation, escalation, owner transaction, and return shape, so the brief carries the phase rather than restating that contract. The child never edits parent memory. Choose the smallest reliable tier - `small` (narrow, mechanical), `standard` (substantive but bounded), or `large` (broad, intricate, cross-cutting, or high-consequence) - escalating after uncertainty, failed reasoning, or widened scope; omit the `model` field to use configured role routing, overriding deliberately with the tier's exact `provider/model-id`. Never pass `default`, `auto`, or `inherit parent` as a model value. Full tier definitions: docs/workflow.md. The commit-capable owner performs all ordered work, stages the complete transaction, and creates the declared phase-closing commit, which requires `./awf check staged` and `./x gate` to pass; a wired pre-commit hook enforces both at commit time, and the owner runs them manually first only in a clone without wired hooks (checkable with `git config core.hooksPath`; when in doubt, run both manually). Never dispatch concurrent implementers or helpers as phase owners.
