...
export type SubagentRole = "grounding" | "explore" | "review" | "implement";
...
export interface PreferenceStoreDependencies {
...
export function parseExactModelReference(value: unknown): { provider: string; id: string } | { reason: "malformed" | "overlong" } {
  ...
  return value.length < 3 || slash <= 0 || slash === value.length - 1 || /\s/.test(value)
    ? { reason: "malformed" }
