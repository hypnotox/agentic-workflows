...
const SLOT_GUIDANCE: Array<{ key: PreferenceField; guidance: string }> = [
  ...
  pi.events?.on?.(RESULT_EVENT, (result: any) => { result = result as ProfileRegistrationResult & { correlationId?: string }; if (result.registrationId !== REGISTRATION_ID || (result.correlationId !== undefined && result.correlationId !== correlationId)) return; if (result.protocolVersion !== PROTOCOL_VERSION || result.state !== "registered") report(result.reason ?? "registration rejected"); else registered = true; });
