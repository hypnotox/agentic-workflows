...
const SUBAGENT_TOOL_NAMES = new Set(["subagent_grounding", "subagent_explore", "subagent_review_adr", "subagent_review_plan", "subagent_review_code", "subagent_implement"]);
...
async function resolveVerificationCheckout(
  ...
  const descent = relative(canonicalRoot, checkout);
  if ((descent === "" && checkout !== canonicalRoot) || descent === ".." || descent.startsWith(`..${"/"}`) || resolve(canonicalRoot, descent) !== checkout) throw new Error(`verificationCheckout must be the parent session checkout or an accessible descendant: ${checkout}`);
...
const SLOT_GUIDANCE: Array<{ key: PreferenceField; guidance: string }> = [
  ...
  const reviewProfile = (spec: { id: string; toolName: string; label: string; kind: ReviewKind; relative: string }): ProfileDefinition => ({ id: spec.id, toolName: spec.toolName, label: `${spec.label} Review Subagent`, description: `Run one governed report-only ${spec.label} reviewer in fresh context. Omit model for configured or inherited routing; use an exact tier provider/model-id only for deliberate override. Omission is the only default form.`, promptSnippet: `Delegate governed ${spec.label} review to fresh context`, promptGuidelines: [`Use ${spec.toolName} with a self-contained review brief. Omit model for configured or inherited routing; use the selected tier's exact provider/model-id only for deliberate override. Omit the model field to use configured or inherited routing.`], parameters: Type.Object({ task: Type.String({ minLength: 1 }), model: Type.Optional(MODEL_REFERENCE_SCHEMA) }, { additionalProperties: false }), profileDataSchema: Type.Object({ ...ROUTING_PROFILE_DATA_FIELDS }, { additionalProperties: false }), concurrency: MAX_REVIEW_CONCURRENCY, selectModel: (c: any) => selectModel(c, "review"), beforeRun: (c: any) => routingFor(c), prepare: (c: any) => prepare(c, REVIEW_TOOLS, { relative: spec.relative, repair: `Enable the matching ${spec.kind}-reviewer agent and run ./awf render.`, noun: "reviewer", prepend: `You are the governed ${spec.kind} reviewer. You are report-only: never edit or commit.` }), afterRun: (_o: any, state: any) => ({ profileData: state }) });
