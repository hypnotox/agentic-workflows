  ...
  pi.registerCommand("awf-subagent-models", {
    ...
    handler: async (_args: string, ctx: any) => {
      ...
      try { snapshot = await deps.readFile(scope.path, "utf8"); }
        ...
        if ((error as { code?: string }).code !== "ENOENT") { ctx.ui.notify(`Cannot read ${scope.path}: ${errorText(error)}`, "error"); return; }
        snapshot = undefined;
      }
        ...
        if (workTree.code !== 0) {
          ...
          if (ignored.code !== 0) {
            ...
            try { existing = await deps.readFile(gitignorePath, "utf8"); }
              ...
              if ((error as { code?: string }).code !== "ENOENT") { ctx.ui.notify(`Cannot read ${gitignorePath}: ${errorText(error)}`, "error"); return; }
            }
      ...
      try { latest = await deps.readFile(scope.path, "utf8"); }
        ...
        if ((error as { code?: string }).code !== "ENOENT") { ctx.ui.notify(`Cannot re-read ${scope.path}: ${errorText(error)}`, "error"); return; }
        latest = undefined;
      }
      if (latest !== snapshot) {
