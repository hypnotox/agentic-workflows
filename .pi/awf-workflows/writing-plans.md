...
4. **Terminal step:** call `awf_workflow` alone with `skill: "reviewing-plan"`. The reviewer applies its lenses and reports findings; route them per the reviewing skill's procedure; fixes land as new commits on top of the committed plan.
...
- If planning surfaces a new load-bearing (ADR-worthy) decision, pause: call `awf_workflow` alone with `skill: "proposing-adr"` (or use the current-phase ADR-lifecycle support mapping for an amendment), let it settle, then resume the plan through `skill: "reviewing-plan-resync"`.
