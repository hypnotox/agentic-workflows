...
Exploration gives each child one independent information need and fans out sibling calls, selecting breadth, detail, and model tier per child. A fan-out may combine a large analysis child with small targeted paths or summary children. At most ten Pi exploration children are active per extension instance; excess calls queue FIFO, and refinement that depends on a prior result remains sequential.
