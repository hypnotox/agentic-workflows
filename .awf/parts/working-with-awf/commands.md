...
In Pi, `/awf-dashboard` explicitly refreshes canonical metrics then doctor and opens the trajectory-aware overlay. Its muted below-editor line uses `[awf:init]`, `[awf:<phase>]`, `[awf:done]`, or `[awf:abandoned]`; totals sum each unique active-branch assistant message's public usage once, cache hit is `cacheRead/(input+cacheRead)`, and public context usage supplies the current percentage and window without recharging history. A fresh session retains at most 256 provisional observations and 1 MiB in memory until `awf_workflow`, `/awf-resume-effort <effort-id>`, or external adoption settles identity. Use `/awf-resume-effort` only for a resident discovery or active effort. For a normalized external checkpoint with no resident effort, the agent invokes `awf_adopt_effort` alone with matching explicit memory path, effort, route, phase, and workflow fields; successful atomic adoption persists the association before returning the governed body. `awf_metrics` and `awf_doctor` return bounded plain-text summaries with compact details only; use the dashboard for full canonical detail. Repair and waiver re-resolve the selected finding's owning effort and current frontier; stale, cross-effort, empty-frontier, mismatched, or ineligible input appends nothing.
