...
Pi's `handoff_session` accepts only the exact repository-relative `.awf/efforts/<slug>/memory.md` path or an absolute spelling that normalizes to it. It validates the slug, confinement, ownership, bounded UTF-8 regular-file identity, and either exact legacy `Effort: <slug>` or canonical `effort: <slug>` identity without selecting an effort or mutating lifecycle state.
