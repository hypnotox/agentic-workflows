...
An active effort is `.awf/efforts/<slug>/memory.md`. `new effort` creates a small editable continuation starter, `list` lists active residents, `show` returns the path and raw memory, and `finish` moves the complete resident to `.awf/effort-archive/<slug>` without replacement. Memory and extra resident files are opaque; `finish` does not interpret readiness or content.
