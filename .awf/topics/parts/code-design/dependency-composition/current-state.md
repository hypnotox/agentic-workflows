...
The repository extraction target has one semantic owner per concern. `project.Session` owns one immutable selected universe of bound configuration, complete loaded facts, roots, repository and reader handles, and artifact-registry projection; `project.Loader` owns selection, opening, and validation; `projectmutation.Transaction` owns shared lease, confined-load, committed-reload, one-attempt synchronization, and partial/release mechanics without absorbing focused-operation policy; `publisher.Publisher` owns output planning, rendering coordination, backup decisions, and publishing; `internal/repositorycheck` owns policy-free ordered aggregation through `Compose`; `internal/currentstatecoord` owns focused current-state authority, checks, and staged-output preparation; focused application operations own command-level use cases; and `cmd/awf` owns parse, compose, invoke, result rendering, stream choice, and exit mapping. Individual check policy stays with its concern owner: `internal/generatedcheck` for generated-output conformance, `internal/referencecheck` for managed references, `internal/pitfallcheck` for pitfalls, `internal/glossarycheck` for glossary validation, `internal/configcheck` for configuration and command-spec consistency, `internal/currentstatecoord` for current-state validity, `internal/prosegate` for punctuation, `internal/memorycite` for memory citations, `internal/commitpolicy` for commit authorization, and `internal/audit` for advisory repository analysis. These are semantic owners, not a prescribed file inventory or extraction order.
