...
Repository wiring exposes one stable `CI / gate` conclusion for the exact revision. Pull requests always run complete Linux Go behavior and select applicable lanes from one typed JSON v2 result; `main` runs complete Linux/amd64 and Darwin/arm64 source assurance. The closed CI lane inventory is `go`, `platform-sensitive`, `release-archive`, and `render-template`. Release tags remain protected by that conclusion, while the release workflow constructs one credential-free candidate and publishes it only after Linux/amd64 and Darwin/arm64 native verification.
Backing: test
