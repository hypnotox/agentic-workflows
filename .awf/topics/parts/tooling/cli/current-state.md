...
Only authored plan projections, selected changelog content, effort activity JSON, owner-scoped effort memory protocol JSON, init descriptor JSON, and the context spill notice bypass the presentation tree on successful output, each under byte-exact tests. Memory protocol JSON is a required protocol-1 machine envelope selected only by mutually required `--owner` and `--json`, while owner-free memory output remains ordinary presentation. The exact `writeRendererFailure` terminal mechanism diagnostic is separate, is reachable only after presentation rendering fails, and is not an alternate successful renderer.
Origin: ADR-0234
Revised-by: ADR-add-associated-pi-effort-memory-tools
Backing: test
