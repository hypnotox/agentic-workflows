...
The containerized pinned Pi 0.81.1 fixtures are the deterministic gate for every Pi extension, including protocol TypeScript parity, generated ownership, adoption into every legal mapping, start, transition, continuation, nested detour completion and abandonment return at every fault boundary, pending-return retention exclusion, compact query byte/detail bounds, and dashboard full detail. Release readiness additionally requires subagent, router, structured replacement, handoff, and dashboard smoke runs on the exact compatible fork for Pi 0.81.1 or a later build verified to expose the required queued-command, persisted-session, custom-entry, widget, overlay, and shutdown APIs. The manual smoke covers router-enforced phase transitions and recovery, explicit effort resume through session replacement, memory-identity association across a parent handoff, dashboard refresh, widget and overlay behavior, durable lifecycle writes, shutdown drain, and visible degraded operation when canonical binary resolution is unavailable.
Origin: ADR-0148
