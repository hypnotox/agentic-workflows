...
| `./x test-affected [--staged\|--range <base>..<head>]` | Report deterministic changed-owner, affected-caller, and declared meta-suite reasons, then run the selected behavioral targets once through bounded isolated workers without coverage. Working-tree evidence is the default; shared or uncertain changes widen or refuse visibly. |
