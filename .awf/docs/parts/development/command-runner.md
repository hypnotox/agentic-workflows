...
| `./x test-affected [--staged\|--range <base>..<head>]` | Report deterministic changed-owner, full reverse-dependent, test-only importer, and declared meta-suite reasons, then run each selected behavioral target once through bounded isolated workers without coverage. Working-tree evidence is the default; shared or uncertain changes widen or refuse visibly. |
