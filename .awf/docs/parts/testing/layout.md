...
shared delivery path. `internal/contextspill/log_test.go` and fault cases pin exact notice recognition, POSIX quoting, owner-only no-follow paths, locked concurrent durable appends, and primary-error preservation; `cmd/contextspilllog/main_test.go` pins the private helper grammar; and `internal/project/context_wrapper_test.go` runs a copied `x` against a fake sibling `awf` to prove byte/status preservation, warning-only degradation, concurrency, and the non-failing check advisory. JSON parity fixtures are intentionally absent because context is human-only.
...
fixture advertises 2.1 because the generated telemetry extension requires a compatible reader before registration. A repository-resident preflight test refuses automatic cleanup when any protocol-1 effort exists and proves it changes no resident evidence. Telemetry tests cover confined owner-only storage, leases, tombstones, serialized drain, explicit versus passive failure, active-branch association, bounded provisional creation/discard/overflow/graceful/crash behavior, ordered retry recovery, structured replacement resume and cancellation, trajectory navigation, closed lifecycle schemas, unique-entry public-usage accounting, restored and nested usage, muted below-editor badge rendering after successful explicit actions, complete repository-path privacy exclusion, and no-spawn rendering. They also prove the absence of canonical reads, refresh, launcher fallback, dashboard commands, query tools, overlays, and dashboard maintenance controls. Workflow-loader tests cover the closed semantic enum, exclusive trustworthy batches, catalog-mapped route, phase, activity, mode, and terminal effects, one-event chain transitions, causal predecessor rejection, winner/conflict retries, fixed-body identity after durable acknowledgement, and retrospective completion, failure, cancellation, shutdown, and recovery. Runner tests cover
...
result-middleware error patches, producer observations, router loading, structured replacement, memory-identity association copying before kickoff, normalized external adoption, nested detour return recovery, widget registration, lifecycle durability, retrospective settlement, shutdown drain, and current-leaf mixed-implementation blocking survive the real runtime seam without entering model-visible content. Compatibility tests reject an official-0.81.1-shaped API that lacks any required queued-command, persisted-session, custom-entry, widget, or shutdown surface. Handoff tests cover the closed schema, path and symlink boundaries, exact memory `Effort:` parsing, independently validated association match, exclusivity, pending token, countdown cleanup and cancellation, pending-window identity revalidation, parent-linked replacement with setup-time association, restore-before-kickoff ordering, teardown failures, kickoff, and editor fallback. The container type-check and c8 coverage globs include every extension TypeScript file and require 100% statements, branches, functions, and lines. Reachable telemetry, handoff, and subagent branches use injected dependencies and behavioral tests; handoff and subagent carry no exclusions, while telemetry retains only its reasoned exclusions for closed-invariant or platform runtime guards. The Go current-state proofs invoke the focused container contract harness and require named behavioral test results rather than source-token presence. Unit tests cover exact all-role model routing and rejection, inherited
...
Release readiness also requires the unbacked real-Pi smoke on `hypnotox/pi` `fork-v0.81.1-awf.3` or a later API-verified build. In a new session, load `brainstorming` through `awf_workflow`, load its routed successor, and verify one transactional phase transition. Cancel and retry `/awf-resume-effort <effort-id>` into a replacement; create matching memory carrying `Effort: <id>`, hand off, and verify association is present before kickoff. Interrupt and retry one provisional or transition settlement to verify idempotent recovery, then fork/resume, verify the local widget after a successful explicit action, complete, shut down, and inspect retained history. Record the exact commands and observed states in the release work; this manual smoke has no invariant proof marker.
