...
Live authority starts at schema 50. A below-floor or retired layout is refused before decoding or mutation; first use AWF 0.44 to reach schema 50, then restore or adopt the supported `.awf/` control pair. If `.awf/current-state-upgrade.journal` is present, run only `./awf upgrade --recover`; precommit recovery rolls back and postcommit recovery cleans residue without reverting authority.
