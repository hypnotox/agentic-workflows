...
- **`internal/audit/`**: go-git-backed collection of the branch's commits plus the advisory
  ...
  (ADR-0017, ADR-0036).
- **`internal/effort/` and `internal/worktree/`**: own schema-1 lightweight effort records, normalized optional memory, a single session-assignment map, and strict native-Git managed worktrees. They allocate lowercase UUIDv4 IDs, resolve repository-wide resident roots through native Git, serialize mutations under an owner-only repository lock, atomically replace current-state JSON, and preserve corrupt inputs. The assignment map is the one authority for a bounded safe Pi session ID's current effort: it supports explicit reassignment and terminal-effort assignment without lifecycle mutation, while logical records derive sorted session IDs from it. Managed worktrees use the fixed `.awf/worktrees/<id>/` path and `awf/<id>` branch, validate topology and cleanliness before explicit fast-forward, merge, or manual integration, retain disposition until explicit removal, and never let force override hard path, identity, ownership, or conflict safety.
