...
- Replace the governance and generic authoring platform with a fixed documentation projector, lexical topic routing, and filesystem-only effort memory. The public CLI is now `init`, `render`, `check`, `resolve`, `effort`, and `version`; effort operations are `new`, `list`, `show`, and `finish`.

- Replace the former config, catalog, parts, domains, rendered topics, and lock with `.awf/project.md` plus `.awf/topics/**/*.md`. Generated ownership uses explicit file markers. AWF no longer deletes retired files: `render` reports them successfully and `check` fails until the adopter deletes the file or removes its marker.

- Remove Git integration, managed worktrees, migrations, upgrade behavior, hooks, repository policy checks, audit, changelog extraction, and persistent decision machinery from AWF.

### Features

- Generate fixed Pi and Claude topic and effort entrypoints, the root wrapper, and verified bootstrap unconditionally.

- Seed new projects with an editable concise AGENTS authoring guide in `.awf/project.md`.
...
- Select `internal/projectmutation` changes for native platform assurance, remove obsolete authority selectors, and document the v0.46 registry and transaction exports.

## [0.46.0] - 2026-09-01

### Others

- Complete the internal behavioral-contract liberation while retaining public behavior and generated bytes for the same inputs: unify effort operations behind one application boundary; establish the canonical artifact registry; make one authoritative `project.Session` own project selection with no `projectstate` bridge; make Publisher a one-pass, single-use operation that detects collisions before rendering and preserves byte parity; replace the generic repository-check execution DAG with typed aggregate, direct, advisory, and tracking lanes; and narrow `projectmutation` to mechanics with focused policy owners.

- Separate code-owned historical audit vocabulary from config-backed live commit scopes, remove all production dead code and obsolete compatibility authority, and close the resolved authority and known-issue cleanup without widening the public surface.

- Keep schema generation and the live floor at 50 with the existing AWF 0.44.0 minimum, and retain native Linux/amd64 and Darwin/arm64 assurance. The nine managed adopters remain same-schema synchronization work for the v0.46 rollout; each is planned to take the release through its governed upgrade, render, check, and repository-specific verification rather than being represented as already synchronized.
...
- Render now retains only current-state authority, streams bounded marker-source lines, and excludes resident effort trees, preventing project inputs from multiplying into gigabytes of memory.
