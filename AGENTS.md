...
- **Binary-version gate.** Every gated command (`sync`, `check`, `invariants`, `audit`, `list`, `config`, `enable`, `disable`, `new`) refuses to run when the binary is behind the project on schema generation or lock `awfVersion`; `config` degrades to a static catalog reference outside an adopted tree instead of refusing. (ADR-0039)
