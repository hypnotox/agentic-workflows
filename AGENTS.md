...
- **Staged authority and green gate before every commit.** Stage the complete transaction; every commit requires `awf check staged` and `./x gate` to pass. A wired pre-commit hook enforces both, but awf never activates hook wiring; resolve the invoking worktree before delegating so absolute or relative `core.hooksPath` does not select another checkout. Run them manually before committing only in a clone without wired hooks, and treat remote branch policy as the final boundary.
...
Implementation agents use their selected workflow's shared authority boundary to resolve compliant detail autonomously.
