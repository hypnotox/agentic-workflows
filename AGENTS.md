...
- **Toggle a target** — add or remove its name in the enable arrays (`skills`, `agents`, `docs`, `hooks`) in `.awf/config.yaml`. The workflow-chain skills reference one another by name, so disable them as a unit rather than piecemeal, or a handoff will point at a skill that isn't enabled.
