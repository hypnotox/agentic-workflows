...
- **Context Tier 1 governs.** `awf context <paths>` reports as Tier-1 "governing" ADRs exactly the Implemented, non-retired ADRs declaring an invariant slug present as a marker under a queried path — the intersection of the path-present slugs with the shared one-to-one `slug → declaring ADR` join (`invariant: context-tier1-governs`). (ADR-0104)
