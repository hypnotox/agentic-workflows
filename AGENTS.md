...
- **Context Tier 1 governs.** `awf context <paths>` reports as Tier-1 "governing" ADRs exactly the Implemented, non-retired ADRs declaring an invariant slug present as a marker under a queried path — the intersection of the path-present slugs with the shared one-to-one `slug → declaring ADR` join (`inv: context-tier1-governs`). (ADR-0104)
- **Context Tier 2 topical.** A non-Tier-1, non-Superseded ADR is Tier 2 iff it shares a tag with the Tier-1 precise set (the Tier-1 tags minus any tag naming a configured domain) or is `related:`-linked from a Tier-1 ADR; a pitfall is Tier 2 iff it shares such a tag; each artifact appears in at most one tier (`inv: context-tier2-topical`). (ADR-0104)
- **Context Tier 3 collapsed.** A domain-membership ADR in neither Tier 1 nor Tier 2 is reported only as part of a collapsed background count with the domain current-state pointers, never individually enumerated (`inv: context-tier3-collapsed`). (ADR-0104)
- **Context surfaces tiered plans.** `awf context` surfaces a plan iff its `adrs:` links a Tier-1 or Tier-2 ADR, on the single `ContextResult` preserving read-only/parity/fallback (`inv: context-surfaces-tiered-plans`). (ADR-0104)
- **Context surfaces tiered pitfalls.** `awf context` surfaces a pitfall iff it shares a tag with the Tier-1 precise set, on the single `ContextResult` preserving read-only/parity/fallback (`inv: context-surfaces-tiered-pitfalls`). (ADR-0104)
