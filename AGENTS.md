...
- **Context Tier 2 topical.** A non-Tier-1, non-Superseded ADR is Tier 2 iff it shares a tag with the Tier-1 precise set (the Tier-1 tags minus any tag naming a configured domain) or is `related:`-linked from a Tier-1 ADR; a pitfall is Tier 2 iff it shares such a tag; each artifact appears in at most one tier (`invariant: context-tier2-topical`). (ADR-0104)
